...
\begin{frame}
  \frametitle{Attack 1 on naive stack and return capabilities }
  \centering
  \begin{tikzpicture}[scale=.45, every node={scale=.5}]
    % recurrent parts
    \stdstackstart
    ...
    \begin{onlyenv}<+>
      ...
      \draw (18,8.25) edge[->,color=myred,very thick,out=235,in=0] node[sloped, below] {read/write} (6,8);
      \draw [decorate,decoration={brace,amplitude=10pt,mirror,raise=4pt},yshift=0pt]
...
\begin{frame}
  \frametitle{\stktokens{} prevent attack 1}
  ...
  \begin{tikzpicture}[scale=.45, every node={scale=.5}]
    ...
    \begin{onlyenv}<+>
      ...
      \draw node[right=0cm of sp1] { stack pointer };
      \draw (r1) edge[->,myred,thick,bend left,dashed] (sp1);
    \end{onlyenv}
    ...
    \begin{onlyenv}<+>
      ...
      \draw (r2) edge[->,myred,thick,bend left, dashed] (sp2);
      ...
      \draw (r1) edge[->,myred,thick,bend left,dashed] (usp);
    ...
    \begin{onlyenv}<+>
      ...
      \draw [decorate,decoration={brace,aspect=0.2,amplitude=10pt,mirror,raise=4pt},yshift=0pt]
      (6.5,5) -- (6.5,16) node[draw=black] (usp) [black,pos=0.2,xshift=0.8cm] {};
      \draw (r1) edge[->,myred,bend left,dashed] (usp);
      \draw node[right=0cm of usp] { stack pointer };
    ...
    \end{onlyenv}
    \draw<.-> (r2) edge[->,myred,bend left,dashed] (sp2);
...
\begin{frame}
  \frametitle{Attack 2 on naive stack and return capabilities }
  \begin{tikzpicture}[scale=.5, every node={scale=.5}]
    % recurrent parts
    \stdstackstart
    ...
    \begin{onlyenv}<.-.(2)>
      ...
      \draw node[right=0cm of sp1] { stack pointer };
    \end{onlyenv}
    \draw<+->[fill=white] (0,13) rectangle (6,13.5) node[pos=.5,color=myred] {\footnotesize copy of sp};
    \draw<.> (6,13.25) edge[myred,thick,->,in=45] (sp1);
    \begin{onlyenv}<+->
      \draw [decorate,decoration={brace,amplitude=10pt,mirror,raise=4pt},yshift=0pt]
        (13.5,4) -- (13.5,16) node[draw=black] (spold) [black,midway,xshift=0.8cm] {};
      \draw (6,13.25) edge[myred,thick,->,in=45] (spold);
    \end{onlyenv}
...
%    \draw<+->[fill=gray!50] (0,4) rectangle (6,7) node[pos=.5] {\footnotesize callee stack frame};
    ...
    \begin{onlyenv}<.-.(1)>
      ...
      \draw node[right=0cm of sp1] { stack pointer };
    \end{onlyenv}
    \draw<+> (spold) edge[->,color=myred,very thick,out=235,in=0] node[sloped, below] {read/write} (6,8);
  \end{tikzpicture}
\end{frame}

\begin{frame}
  \frametitle{\stktokens{} prevent attack 2}
  \begin{tikzpicture}[scale=.5, every node={scale=.5}]
    % recurrent parts
    \stdstackstart
    ...
    \begin{scope}
      ...
      \foreach \x in {1,2,...,4}
      {
        \draw[fill=white] (16,\x) rectangle (17,\x+1) node[pos=.5,color=mygreen] {};
      };
    \end{scope}
    \node (r1) at (16.75,4.5) {};
    \node (r2) at (16.75,3.6) {};
    ...
    \begin{onlyenv}<+>
      ...
      \draw (6,13.25) edge[myred,thick,->,in=45,dashed] (usp);
      \draw node[right=0cm of usp] { stack pointer };

      \draw[fill=white] (0,13) rectangle (6,13.5) node[pos=.5,color=myred] {\footnotesize sp};
      \inactsf{(0,2.5)}{(6,5)}{caller stack frame} 
      \actadv{(0,5)}{(6,8)}{callee stack frame}
    \end{onlyenv}
...
%    \draw<+> node[mygreen,cloud,cloud puffs=10.8,cloud puff arc=110, aspect=3, draw, fill=white, align=center] () at (4,10) { Stack pointer\\essentially lost.}; 
    ...
    \begin{onlyenv}<+->
      ...
      \begin{scope}
        ...
        \draw (6,0) -- (6,15);
      \end{scope}
      \draw[fill=gray!50] (0,13) rectangle (6,13.5) node[pos=.5,color=myred,dashed] {\footnotesize sp};
      \inactsf{(0,2.5)}{(6,5)}{caller stack frame} 
      \inactadv{(0,5)}{(6,8)}{callee stack frame}
      \actsf{(0,8)}{(6,10.5)}{callee (2) stack frame} 
  \end{onlyenv}

  \draw<+>[mygreen,very thick] (-0.5,7.5) -- (6.5,11);
  \draw<.>[mygreen,very thick] (-0.5,11) -- (6.5,7.5);
  \draw<.> node[mygreen,cloud,cloud puffs=10.8,cloud puff arc=110, aspect=3, draw, fill=white, align=center] () at (14,7) { No stack pointer\\for callee (2)!};  
...
\begin{frame}
  \begin{tikzpicture}[scale=.5, every node={scale=.5}]
    \frametitle{Attack 3 on naive stack and return capabilities}
    \begin{scope}
...
\note{Callee stores the return pointer they got from caller.}
    \begin{onlyenv}<+->
      \draw[fill=white] (13,1) rectangle (18,1.5) node[pos=.5] {\scriptsize {\color{myred} copy of ret-ptr}};
      \draw (13,1.25) edge[->,myred,thick,bend left] (ret1);
    \end{onlyenv}
...
\note{Callee calls callee (1)}
    ...
    \begin{onlyenv}<+->
      ...
      \draw[fill=gray!30] (13,1) rectangle (18,1.5) node[pos=.5] {\scriptsize {\color{myred} copy of ret-ptr}};
    \end{onlyenv}
...
\note{Callee (2) has access to ret-ptr (the return pointer caller passed to callee)}
    \begin{onlyenv}<+->
      \draw [decorate,decoration={brace,amplitude=2pt,mirror,raise=4pt},yshift=0pt]
      (18.1,1) -- (18.1,1.5) node[draw=black] (copy) [black,midway,xshift=0.5cm] {};
      \draw (18,8.25)  edge[->,myred,thick,bend left] (copy);
      \draw[fill=white] (13,1) rectangle (18,1.5) node[pos=.5] {\scriptsize {\color{myred} copy of ret-ptr}};
    \end{onlyenv}
...
\note{Callee (2) uses ret-ptr to return}
    ...
    \begin{onlyenv}<+->
      \draw node [ellipse, fit=(retptr1) (retptr2), draw, myred] { };
      \draw node[myred,cloud,cloud puffs=10.8,cloud puff arc=110, aspect=3, draw, fill=white, align=center] () at (12,10) { These were skipped!};  
    \end{onlyenv}
  \end{tikzpicture}
\end{frame}

\begin{frame}
  \frametitle{\stktokens{} prevent attack 3}
  \begin{tikzpicture}[scale=.5, every node={scale=.5}]
    % recurrent parts
  \scope
    \clip (-.1,-.1) rectangle (6.1,15.1);
...
\note{Callee saves ret-ptr-d on the heap.}
  \begin{onlyenv}<+->
    \draw (13,4.75) edge[myred,bend right,thick,->,dashed] (ret1);
  \end{onlyenv}
\note{Callee saves ret-ptr-c on the heap.}
  \begin{onlyenv}<+->
    \draw (13,4.75) edge[myred,bend right,thick,->] (ret1c);
  \end{onlyenv}
...
\note{Callee (2) has a capability for Callee and can thus access the stored ret-ptr-d}
  \begin{onlyenv}<+->
    ...
    \draw (18,11) edge[myred,bend left,thick,->] (calleecap);
  \end{onlyenv}
...
\note{Callee (2) uses ret-ptr-d to return}
  ...
  \begin{onlyenv}<+->
    \draw[mygreen,very thick] (6.4,4.4) -- (7.4,5.4);
    \draw[mygreen,very thick] (7.4,4.4) -- (6.4,5.4);
    \draw node[mygreen,cloud,cloud puffs=10.8,cloud puff arc=110, aspect=3, draw, fill=white, align=center] () at (10,6) { Splice fails!};  
  \end{onlyenv}
...
\note{Splicing: fails because ends of capabilities don't meet. callee stack frame (1) cannot be empty (it will at least contain 42).}
  % \begin{onlyenv}<+->
  %   \draw node[mygreen,cloud,cloud puffs=10.8,cloud puff arc=110, aspect=3, draw, fill=white, align=center] () at (8,7.5) {Caller tries to splice sp \\and ret-ptr-d, but fails!};  
  % \end{onlyenv}
...
\begin{frame}
  ...
  \begin{tikzpicture}[scale=.5, every node={scale=.5},distance=1cm]
    ...
    \draw<1-> node[draw=black,rounded corners,below=of source] (mca) {Machine code w/ many abstractions};
    \draw<1-> node[draw=black,rounded corners,below=1cm of mca] (mcir) {...};
    \draw<1-> node[draw=black,rounded corners,below=1cm of mcir] (mcs) {Machine code w/ built-in stack};
    \draw<1-> node[draw=black,rounded corners,below=of mcs] (target) {Machine code};
...
% "Useful for reasoning" is intended to contrast our approaches in
% ESOP18 (logical relation) and POPL19 (fully abstract overlay
% semantics) against Abadi's use of control flow graphs.
%   The idea is that if you have a concrete program whose correctness
% depends on WBCF and LSE, then one should be able to rely on the
% formalization of WBCF and LSE of the calling convention to prove
% correctness of the program.
%   This intuitive desirable property is satisfied for our ESOP18 paper
% (you can use the LR to reason about your program) and our POPL19 paper
% (you can reason in oLCM where WBCF and LSE hold, using a separate LR
% or other tool if needed) but not for Abadi's property about "control
% flow stays withing the static control flow graph" (how would you use
% this for reasoning?, time-insensitive).
...
% "Arguably complete" is intended to contrast our POPL19 approach
% against the ones in ESOP18 and Abadi's CFG approach.
%   It is intended to mean that the formalization of WBCF and LSE should
% arguably apply to "all properties that follow from WBCF and LSE".
%   For POPL19, this is satisfied because it applies to any property
% that holds in oLCM.
%   For Abadi's CFGs, this is not satisfied, for one reason because it
% is not timing-sensitive.
%   For ESOP18, I don't see any reason to think that any result provable
% using WBCF and LSE should also be provable using our LR, even though
% there is also no reason to think it isn't.
