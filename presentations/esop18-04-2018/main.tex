...
\begin{frame}
  ...
  \item New \textit{write-local} authority
...
\begin{columns}
  \begin{column}{0.5\textwidth}
    \begin{itemize}
      \note[item]<1-3>{Compared to standard assembly language, capabilities executing code has access to represent bound.}
    \item<1-> Capabilities represent bounds on executing code
      \note[item]<1-3>{That is, the capabilities the executing code has access to.}
      \note[item]<1-3>{What exactly is the bound on. In the system we have considered, no I/O, so memory invariants.}
      \note[item]<1-3>{Take what the program does into account - allows more fine-grained then simply ``read/write''}
    \item<2-> World, $W$
      \note[item]<2-3>{The collection of invariants is what we call the world (and it can bethought of as  a model of the memory) }
      \note[item]<2-3>{Here colored region represents invariant. A simple invariant could be a specific address contains 42.}
      \begin{itemize}
      \item Collection of invariants
      \end{itemize}
      \note[item]<3-7>{We are interested in all the words on the machine that preserve these invariants, so}
    \item<4-> Predicate for safe values w.r.t world, $\stdvr(W)$
      \note[item]<4-7>{We define a predicate with theses values. Whether a capability violates invariants obviously depend on the invariants, so the predicate has to be world-indexed.}
      \note[item]<4-7>{Let's see some examples:}
      \note[item]<5-7>{If the capability on the slide has \emph{read} authority, then it cannot violate the simple invariant.}
      \note[item]<6-7>{If the capability on the slide also has \emph{write} authority, then it can violate the invariant by simply overwriting that address with a different number. \emph{Not safe}.}
      \note[item]<7-7>{Generally speaking a capability that can read is safe when it only can read safe words. What happens when it is stored at an address that it has authority over itself?}
      \note[item]<7->{It is safe only if it can read only safe values which requires it to be safe.}
      \begin{itemize}[<9->]
      \item Recursively definition
        \note[item]<8->{Need to take a fixed-point. Made possible by step-indexing.}
        \note[item]<9->{Related to similar issue for languages with \emph{recursive types} and \emph{ML-like references}.}
      \end{itemize}
    \end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
  \begin{tikzpicture}[scale=.5, every node={scale=.5}]
    ...
    % One capability
    \draw<1-7> [decorate,decoration={brace,amplitude=3pt,mirror,raise=-2pt},yshift=0pt]
    ...
    \fill<3-7>[black, fill opacity=0, text opacity=0.7] (0,4) rectangle (6,4.5) node[pos=.5] {\footnotesize 42};

    \draw<5> (6,7.25) edge[black,->,in=45] node[right] {\footnotesize \textsc{r}} (sp1)   ;
    \draw<6-7> (6,7.25) edge[black,->,in=45] node[right] {\footnotesize \textsc{rw}} (sp1)   ;
    \draw<7>[very thick,color=red] (6.5,6.5) -- (8.5,4.5)
    (6.5,4.5) -- (8.5,6.5);

    \draw<8-> [decorate,decoration={brace,amplitude=3pt,mirror,raise=-2pt},yshift=0pt]
    (6.25,2) -- (6.25,5) node[draw=black] (sp1) [black,midway,xshift=6pt] {}; % ,shape=coordinate,xshift=1pt
    \path<8->  (6,4.25) edge[black,out=30,in=135] node[above right]  {\footnotesize \textsc{r}} (8,5) 
...
\onslide<1-4>
  \begin{itemize}
\note[item]<1->{Like languages with ML-references, memory changes over time. Example, if we are in this memory with two invariants and more is memory allocation. World need to cope with this.}
  \item<1-> Memory evolves over time
    \note[item]<2->{Notion called \emph{future worlds}. Allow us to \emph{add invariants}.}
  \item<3-> Add invariants in future worlds
    \note[item]<3->{Memory may be repurposed. Satic invariants don't model this. Alle invariants have an internal state machine. \emph{State machine progress in future worlds}.}
  \item<4-> Invariants as state machines
    \note[item]<4->{Each state is associated with a predicate of all memories that respect the invariant.}
    \note[item]<4->{\emph{Safety monotone wrt worlds}, \emph{Kripke logical relation}}

  \end{itemize}
  \onslide<5-6>
  \begin{itemize}
  \item<5-> Each state contains a predicate of accepted memory segments
    \note[item]<5->{ We want to express ``all memories with safe values''. World dependent, so the predicate needs to be world indexed. }
  \item<6-> World indexed
    \note[item]<6->{ Worlds with invariants with state machines with predicates that are world indexed - circular definition. }
    \note[item]<6->{ Resolved using standard techniques from the litterature (essentially advanced step-indexing). }
  \end{itemize}
  \[
...
\begin{frame}[fragile]
  \frametitle{Local Capabilities}
  \note[item]<1>{Now consider how local capabilities affect all this.}
  \note[item]<1>{Consider this simple example, first f is called with capability c as an argument. Then f is called with unit.}
%  \note[item]<1>{What do we need to argue about c in the first call for it to be valid?}
  \note[item]<1>{What may we assume about c in the second invocation of f?}
  \note[item]<1>{Depends on c!}
  \note[item]<2-3>{(Cont) Depends on c!}
  ...
  \item<2-> \texttt{c} global $\Rightarrow$ available in second invocation of \texttt{f}
    \note[item]<2-3>{If \emph{c is global}, then it can be \emph{stored on the heap}, so it needs to remain safe for the remainder of the execution. When f is invoked \emph{c must still be safe}.}
%    \note{when c global, second invocation in a world where c is safe}
  \item<3-> \texttt{c} local $\Rightarrow$ not available in second invocation of \texttt{f}
    \note[item]<3>{If \emph{c local}, then \emph{CC dictates clear} all the places c may reside, so in the second invocation \emph{c need not remain safe}.}
...
%    \note{c essentially revoked, so the invariants it relies on need not hold.}
  \end{itemize}
  \note[item]<3>{Need two future world relations. In both, global capabilities must remain safe. In one local capabilities need not. We have public and private future world relation.}
  \note[item]<4->{This double monotonicity lemma expresses the assumptions we can make, namely in \emph{public future world all capabilities} remain valid and in \emph{private future worlds local capabilities need not remain valid}.}
  \note[item]<4->{In the example this means that if \emph{c is local}, then it is okay to invoke f a second time in a \emph{private future world} as c need not be safe anymore. }
  \note[item]<4>{On the other hand, if \emph{c is global}, then the invokation of f must be in a \emph{public future world}, so c remains safe.}
...
\onslide<4->{
  \begin{lemma}[Double monotonicity of safety predicate]
    \begin{itemize}
    ...
    \end{itemize}
  \end{lemma}}
\note[item]<4>{Related to public private future worlds, state machines with pub/priv transitions}
...
\begin{frame}
  ...
  \begin{theorem}[Fundamental theorem (simplified)]
      ...
      \]
  \end{theorem}
  \note[item]<1->{Now for the formal statement about guarantees}
  \note[item]<1->{readCond: only safe values in the interval.}
  \note[item]<1->{E-relation: when capability used as the pc with register file and memory respecting the world, then the execution respects the memory invariants.}

  \note[item]<1->{In other words, take an arbitrary capability that only has access to safe values, then the memory invariants are preserved when we use it for execution.}
  \note[item]<1->{The instructions it execute don't matter only the authority it can use.}
...
let x = ref 0 in
  (*$\lambda$*)f.(x := 0;
    ...
    \end{lstlisting}
  \end{tabular}
  \note[item]{known from the litterature}
  \note[item]{in ML difficult to reason about as callback f can be the closure it self. (so x can be either )}
  \note[item]{assert may fail if calls not well-bracketed}
  \note[item]{can do more things to attack well-bracketedness low-level. Context need not follow CC, so well-behaved code cannot rely on behavior of untrusted code. }
  \note[item]{ We have proven a faithful translation correct. That is the assert never fails. Notice, dynamic checks, so machine can fail, but we set it up, so we can distinguish this from assertion failure.}
  \note[item]{ More semantic statement of guarantees allow us to prove it. }
