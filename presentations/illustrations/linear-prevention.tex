...
\begin{frame}
  ...
  % Note: the return pointer actually spans all of the available stack, but for space reasons, we only draw part of it here.
  \begin{onlyenv}<+->
    \begin{scope}
  ...
  \begin{onlyenv}<+>
    \draw[teal,very thick] (9,4.9) -- (12.5,5.9);
    \draw[teal,very thick] (9,5.9) -- (12.5,4.9);
...
\note{Callee (2) uses ret-ptr-d to return}
  \begin{onlyenv}<+->
    \draw[fill=white] (13,1) rectangle (18,3) node[pos=.5] {\footnotesize caller code};
    \draw[fill=gray!30] (13,10) rectangle (18,12) node[pos=.5] {\footnotesize callee code (2)};
    \actsf{(0,1.5)}{(6,4.5)}{caller stack frame}
