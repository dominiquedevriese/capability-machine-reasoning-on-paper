...
\begin{frame}
  ...
  \begin{itemize}
    \item Execute capabilities allow for execution at an arbitrary point in the
      range of authority.\todo{and they are transparent, i.e. no encapsulation
        of local state}
  \end{itemize}
...
\begin{frame}
  ...
  \todo{mention (``without a trusted stack'')?}
  \todo{mention return pointers}
  \todo{emphasize the non-obvious points (callbacks must be global and clear
    part of the stack the callee gains control over)}
  \begin{itemize}
  \item Stack
    \begin{itemize}
...
\begin{frame}
  ...
  \todo{I would try to show a few crucial parts (e.g. the value relation as a
    big union, with one case shown in detail, the observation and expression relation)}
  \todo{Show worlds first?}
  \begin{itemize}
...
\begin{frame}
  ...
  \end{itemize}
  \todo{mention public/private transitions}
\end{frame}
...
\begin{frame}
  ...
  \begin{theorem}[FTLR (essence)]
    ...
    \todo{I would show the technical statement.}
  \end{theorem}
  \begin{itemize}
  \item 
  \end{itemize}
