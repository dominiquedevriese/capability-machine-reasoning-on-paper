...
\institute{\inst{1}Aarhus University \and \inst{2}KU Leuven}
\date{SCM, January 15, 2016}
...
\begin{frame}
  ...
  \begin{itemize}
  \item CHERI
  \item Cheap compartmentalisation
  \item Interesting compilation target
    % Compeling target for high-level language compilation
  \item C-like calling convention
  \item How do we formally reason about it
    % One can program anything?
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{What are capability machines?}
  %Permissions
  % - enter (like scall)

  %Temporal revocation - local/global pwl/nwl 
  %  Temporal information flow control
\end{frame}

\begin{frame}
  \frametitle{Formalisation highlights}
  \begin{itemize}
  \item Simplified
  \item Capabilities
  \end{itemize}
...
\begin{frame}
  \frame{Operational semantics}
  \begin{itemize}
    \item Capability aware instructions 
    \item Operational semantics of store
  \end{itemize}
\end{frame}
