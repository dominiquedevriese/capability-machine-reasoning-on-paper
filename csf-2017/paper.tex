...
\begin{figure}
  \begin{tikzpicture}
    \draw[fill=gray!20,draw=none] (0,4) rectangle (4,3.5);
    \draw[fill=gray!20,draw=none] (0,0.5) rectangle (4,1);
    \draw (0,0.5) -- (0,4);
    \draw (4,0.5) -- (4,4);
    \draw (0,3.5) rectangle (4,3) node[pos=.5] {\footnotesize previous program counter};
    \draw (0,3) rectangle (4,2.5) node[pos=.5] {\footnotesize previous stack pointer};
    \draw (0,2.5) rectangle (4,1) node[pos=.5] {\footnotesize restore instructions};
    \draw (-2,2.25) node (rp) {return pointer};
    \draw[->] (rp.east) -- (-.2,2.25);
  \end{tikzpicture}
...
construct the activation record as depicted in Figure~\ref{fig:activ-rec-struct}.

* return pointer as enter pointer to activation
record on the stack. * activation record contains previous pc and stack pointer
