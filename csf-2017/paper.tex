...
prove. The intere
sted reader can find all the technical details in the
techincal report.

\subsection{Encapsulation of local state} 
The program \texttt{f1} in \figurename~\ref{fig:prog-f1} simply pushes 1 to the
stack, jumps to an adversary using \texttt{scall} and finally asserts
the top value on the stack is 1. If this program is linked up
with an arbitrary adversary that has access to malloc but otherwise only
consists of instructions, then we can show that the assertion never
fails. This shows that the local state of $\texttt{f1}$ is
encapsulated.
\begin{figure}[htbp]
  \centering
  \begin{lstlisting}
...
\todo[inline]{give a taste of a proof by giving high-level proof here?
Maybe later for one of the other examples}

In \figurename~\ref{fig:prog-2}, we have \texttt{f2}. \texttt{f2} is
very similar to \texttt{f1}, but instead of using the stack for local
state and the stack-based calling convention, it allocates new memory
for the local state and use a calling convention based on activation
record. Despite the different approach to handle local state and
calls, we are still able to show a similar property to that of
\texttt{f1}. This illustrates that the logical relation is not
specific to the stack-based calling convention as it can be used to
reason about programs that follow a different calling convention.
\begin{figure}[htbp]
  \centering
\begin{lstlisting}
f2: malloc $r_l$ 1
