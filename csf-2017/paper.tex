...
capability for it) as the stack capability and a global
return capability. If our local state contains no local capabilities,
...
macros that uses the stack assumes a stack capability in
register $r_\stk$. 
...
pushes local state, stack capability and in code return capability to
the stack, sets up the necessary on stack return code and creates a
return capability, restricts stack capability, clears the unused part
