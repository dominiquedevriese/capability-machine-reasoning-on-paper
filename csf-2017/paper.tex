...
%
% Intuition
The logical relation is a semantic models of all the words on the
capability machine that are definitely safe to give to an
adversary. Whether a capability is safe to give to an adversary
depends heavily on the contents of the memory as instructions as well
as capabilities may reside in memory. The memory will be modeled using
worlds where a world can be seen as a collection of protocols that
parts of the memory must satisfy. The memory changes during an
execution as memory is allocated and part of memory changes
purpose. The model needs to be able to express these changes to the
memory which is captured by the future world relations.
%
...
\subsubection{Discussion/comparison with related Dreyer-Neis-Birkedal and with Hur-Dreyer}
