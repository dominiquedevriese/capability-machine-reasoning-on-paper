...
\textbf{Reusable macro instructions} With the high-level approach in
mind, we define the following macro instructions: 
\texttt{fetch $r$ $\var{name}$} fetches the capability related to
$\var{name}$ from the linking table and stores it in register
$r$. 
\texttt{push $r$}/\texttt{pop $r$} works as one would expect. All
macros that uses the stack assumes a stack pointer in
register $r_\stk$. 
\texttt{prepstk $r$} prepares the stack by checking it has permission
$\rwlx$ and making it conform to our stack discipline. 
\texttt{scall  $r$($\overline{r_{\var{args}}}$,$\overline{r_{\var{priv}}}$)}
Jumps to the capability in register $r$ in a safe manner. That is it
pushes local state, stack pointer and in code return pointer to
the stack, sets up the necessary on stack return code and creates a
return pointer, restricts stack pointer, clears the unused part
of the stack, clears the necessary registers and jumpsto $r$. Upon
return, the private state is restored.
\texttt{mclear $r$} clears all the memory the capability in register
$r$ has authority over.
\texttt{rclear $\var{regSet}$} clears all the registers in $\var{regSet}$
\texttt{reqglob $r$} checks whether the word in register $r$ is a
global capability.
\texttt{crtcls $\overline{(x_i,r_i)}$ $r$} allocates a closure pair
where the capability in register $r$ is used to point to the program
and a new environment is allocated where the contents of $\overline{r_i}$ is
stored. In the program referred to by $r$, an implicit fetch
