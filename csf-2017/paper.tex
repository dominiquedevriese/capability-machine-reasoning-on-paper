...
The precautions the stack discipline must take have now been introduced and motivated. Summarized, the stack discipline is:
\begin{description}
...
\item[Before returning to the adversary] clear the registers (with words we do not want to pass to the adversary) and erase any part of the stack we have used.
...
\item[Before invoking an adversary call] make sure it is a global capability.
\item[When having a call back invoked by the adversary] make sure the alleged stack pointer has permission $\rwlx$.
\end{description}
