...
this. Program \texttt{f1} assumes it has access to a stack pointer
...
well-bracketed. If the adversary is able to store the return
...
assertion to fail. The return pointer is passed as a local
capability, so the only means the adversary has to store it is the
stack pointer. The stack pointer itself is a local capability,
so the adversary can only store it on the stack. This means that after
the adversary jumps to the return pointer, \texttt{f3} can
revoke it (by erasing it which happens as part of the
\texttt{scall}). In other words, the adversary cannot store the
return pointer for the second call, and it is possible to
prove that the assertion never fails. The correctness lemma for \texttt{f3} is similar to the two previous lemmas:
\begin{figure}[t]
  \centering
  \begin{lstlisting}
f3: push 1
...
more time. If the adversary can save the return pointer in
the second callback and use it in the first, then the assertion will
fail because $x$ is $0$ before the first callback. The return
...
adversaries original stack pointer breaking the encapsulation of
...
\item A general well-bracketed control flow result?
  ...
  \end{itemize}
\item Relation to local parameters in Scala, Algol/Pascal second-class function
  parameters? 
\item Stack clearing realistic?
\item Non-modularity of heap allocation requirement for adversary callbacks
\end{itemize}

It is worth mentioning that local capabilities are quite similar to features
that have been proposed for high-level languages. Specifically,
\citet{osvald_gentrification_2016}'s second-class or local values in Scala are
remarkably similar and we expect they will support similar reasoning patterns.
