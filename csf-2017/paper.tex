...
language properties relied on by software authors, such as control flow
...
local state encapsulation necessarily rely on techniques like machine code
rewriting (e.g.~\cite{wahbe_efficient_1993,abadi_control-flow_2005}) or
verification (e.g.~\cite{morrisett_system_1999}), virtual machines with a native
stack (e.g.~the JVM~\cite{lindholm_java_2014}) or
randomization~\cite{forrest_building_1997}. Such techniques for retrofitting
protection techniques to commodity hardware have disadvantages in terms of
performance, complexity and/or security.

\emph{Capability machines} are a type of processors, intended to remediate these
limitations by presenting a better security model at the hardware
level~\cite{Watson2015Cheri,Woodruff:2014:CCM:2665671.2665740,Carter:1994:HSF:195473.195579}.\dominique{todo:
  more?} \lars{can we add that there has been renewed interest and
  refer to crash-safe project ?}
 Such hardware tags every word (in the register file and in memory) to
enforce a strict separation between numbers and capabilities, a kind of pointers
carrying some kind of authority. Memory capabilities carry the authority to read
and/or write to a range of memory locations. Additionally, the details vary but
all capability machines offer some form of \emph{object capabilities}, which
...
If a $\temp$ region is not masked by a region, then it behaves like a
$\perm$ region. The $\perma$ regions stay permanent and they are
...
it has revoked.
\todo[inline]{can we make this crisper ?}
...
\begin{align*}
  &\memSat{\ms}{W}
    ...
    \left\{\begin{aligned}
        ...
        &\quad (\hs = \biguplus_{r\in\activeReg{W}}P(r) \land \\
...
\todo[inline]{write something about single orthogonal closure?}  The
