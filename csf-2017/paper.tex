...
\emph{At program startup} a local $\rwlx$-capability (the stack pointer) resides in register $r_\stk$. The system does not hand out global write-local capabilities.
\emph{Before returning to the adversary} clear the registers (with words we do not want to pass to the adversary) and erase any part of the stack we have used.
...
\emph{Before invoking an adversary call} make sure it is a global capability.
\emph{When having a call back invoked by the adversary} make sure the alleged stack pointer has permission $\rwlx$.
