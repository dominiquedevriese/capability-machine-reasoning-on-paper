...
$g$ is global.
The read condition uses one of two standard regions found
in~\ref{fig:standard-regions}. $\iota^\pwl_{\start,\addrend}$ requires
all the words in the range $[\start,\addrend]$ to be capability
safe. $\iota^\nwl_{\start,\addrend}$ requires the words to be
non-local and capability safe.
