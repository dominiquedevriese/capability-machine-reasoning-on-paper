...
real challenge comes when \texttt{f4} returns and we need to establish
that the protocols of the adversaries local state has not been
violated. This can be done by reinstating all of the adverseries old
regions, but we can only do this if the old adverserial regions are
not masked by permanent regions. As illustrated in
\figurename~\ref{fig:worlds-in-pf-g1}, we know this must be the
case. Whenever we are given a world, we get a public future world
where it is not allowed to use permanent regions for masking and in
the private future worlds we construct, we know it is not done
either. The return pointer \texttt{f4} gets from the adversary is
valid in future worlds of $W_1$, so in order to use it to argue it is
safe to return $W_6$ needs to be a public future worlds of $W_1$. We
can construct this as a private future world of $W_5$, but only
because we can reinstate the temporary regions of $W_1$ with the same
region names.
...
\end{tikzpicture}
  \caption{Worlds in the proof of Lemma~\ref{lem:correctness-g1}}
