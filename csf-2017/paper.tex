...
\begin{figure*}
  ...
  \begin{equation*}
    \Phi  \rightarrow
    \begin{cases}
      \sem{\decode(\memheap(\addr))}(\Phi) & \text{if $\memreg(\pcreg) = \stdcap$ and $\start \leq \addr \leq \addrend$ and $\perm \in \{ \exec,\rwx, \rwlx \}$ } \\
  ...
  \begin{tabular}{|c|p{3.7cm}|p{10.5cm}|}
    \hline
...
jumps ($\jmp{}$), conditional jumps ($\jnz{}{}$), a move instruction that copies
a word from one register to another ($\move{}{}$), load and store instructions
for reading from and writing to memory ($\load{}{}$ and $\store{}{}$) 
