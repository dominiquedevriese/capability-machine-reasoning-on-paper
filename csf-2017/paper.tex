...
that, while we claim that our calling convention enforces control-flow
correctness, we do not prove a general theorem to show that this is the case.
The reason for this is that it is not clear what such a theorem should look
like. Formulations in terms of a control-flow graph, like the one by
...
is not made very explicit. As future work, we are considering to look at a more
...
calling convention is fully abstract~\cite{abadi_protection_1998}, which roughly
means that the lower-level machine does not allow interactions with the compiled
code that do not correspond to an interaction with the original code in the
high-level machine.

\textbf{Performance and the requirement for stack clearing} It is important to
consider the performance of our proposed calling convention.
...
\textbf{Modularity} An important aspect of our calling convention is that it is
modular. By that, we mean that the requirements we have on callbacks, return
...
return pointers are necessarily local capabilities because they must point to
...
themselves against us in the same way as we protect against them.
