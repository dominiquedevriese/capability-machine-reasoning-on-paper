...
\begin{abstract}  
  ...
  provably enforce control-flow correctness, encapsulation of local
  ...
  control-flow correctness and encapsulation of local state on it. To prove
...
\item We define a novel stack management discipline for enforcing control-flow
  correctness and encapsulation of local state on the stack in
  Section~\ref{sec:stack-and-return-pointer}. The discipline relies solely on
  local capabilities (used to represent stack and return pointers) and does not
  require OS support in the form of a trusted shadow stack or special call and
  return instructions. It also does not require separate per-component stacks
  and instead works with a single, fairly standard, contiguous stack. It
  supports higher-order cross-component calls (e.g., (potentially)
  cross-component function pointers) and can be efficient assuming only one
...
\item We demonstrate the power of our results in Section~\ref{sec:examples} by applying them to challenging
  ...
  and control-flow correctness guarantees in the presence of cross-component
...
management scheme that enforces control-flow correctness in a way that is provably
...
We define a number of reusable macros. The macros capture the stack
management discipline and other conveniences. The macros are:
...
\textbf{Formulating control flow correctness} The attentive reader may notice
that, while we claim that our stack and return pointer discipline enforces
control-flow correctness, we do not prove a general theorem to show that this is
...
like the well-bracketedness that Example~\texttt{\footnotesize{g1}} relies on. In fact, our
examples show that our logical relation imply a form of control-flow correctness
