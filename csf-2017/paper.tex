...
\item[Before returning to the adversary] clear the registers (with words we do not want to pass to the adversary) and erase any part of the stack we have used (any part of the stack that has been passed to an adversary must also be erased).
\item[Before invoking the adversary] push an activation record to the stack, create a local $\entry$-capability for the restore instructions of the activation record (the return pointer) and leave it in a register, leave a copy of the stack capability restricted to the unused part in $r_\stk$, erase the part of the stack we release control over, and clear the non-argument registers.
