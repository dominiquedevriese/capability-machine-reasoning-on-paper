...
The public future regions relation has the following properties:
% - permanent and temporary regions stay the same type
% - public transitions
The $\perma$ and $\temp$ regions stay the same type of region and they
are allowed to take a transition in the public part of their
transition systems.

% - revoked regions can be reused as temporary regions
The $\revoked$ regions can either stay $\revoked$, or they can become
any $\temp$ region. When a region is $\revoked$ it serves as a mask
for a revoked $\temp$ region. We use masks rather than deleting the
region altogether for monotonicity reasons, but we might as well let a
new $\temp$ region serve as the mask. We do not allow $\revoked$
regions to become a $\perma$ region because we want the private future
region relation to be able to reuse any region it has revoked.
...
relations differ in the way existing regions are allowed to evolve. In
a \emph{public future world}, the regions are allowed to evolve
according to the \emph{public future region} relation. In a
\emph{private future world}, the regions are allowed to evolve
according to the \emph{private future world} relation. The future
world relations are defined as follows:
\begin{mathpar}
