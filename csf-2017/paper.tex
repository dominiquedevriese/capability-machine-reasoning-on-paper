...
\item The above theorem is formulated in terms of a unary step-indexed logical
  ...
  \cite{Dreyer:jfp12} to express well-bracketedness
...
We define the syntax of our capability machine in Figure~\ref{fig:syntax}. We
...
Figure~\ref{fig:perm-hier}: the permissions include null permission ($\noperm$),
...
$\readCond{}$. This condition makes sure the read capability can only
be used to read capability safe words. To make sure this is the case,
the region that governs the part of memory the capability grants
authority over must at least require all the words to be in the value
relation.
Generally speaking, global capabilities should only depend on
permanent regions. The function $\var{localityReg}(g,W)$ projects out
all active regions when $g$ is local, but when $g$ is global, then it
is just the permanent regions.

For capabilities with a write permission, $\writeCond{}$ must be
satisfied for the capabilities range of authority. Generally speaking,
...
must allow any capability safe word.
...
capability safe words. For permit-write-local capabilities, the
...
differs only slightly. Executable capabilities can be updated to point
anywhere in memory, so they must be capability safe as a program
counter no matter where they point in memory. Contrarily, the enter
...
read-execute.
In both conditions, the capability must be capability safe as a
program counter certain future worlds. If the capability is global,
...
\begin{theorem}[Fundamental theorem of logical relations]
  ...
  If $\perm = \exec$ and $\npair{(\start,\addrend)} \in \readCond{}(\gl)(W)$ or
        $\perm = \rwx$ and $\npair{(\start,\addrend)} \in
        \readCond{}(\gl)(W)$ and $\npair{(\start,\addrend)} \in
        \writeCond{}(\iota^\nwl,\gl)(W)$ or
        $\perm = \rwlx$ and $\npair{(\start,\addrend)} \in \readCond{}(\gl)(W)$ and $\npair{(\start,\addrend)} \in \writeCond{}(\iota^\pwl,\gl)(W)$, then
