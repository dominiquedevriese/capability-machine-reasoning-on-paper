...
\begin{abstract}  
  Capability machines provide security guarantees at machine level
  which makes them an interesting target for compilation schemes that aim to
  provably enforce control-flow integrity, encapsulation of local
  state, and such properties.
  We provide a formalisation of a representative capability machine
  and propose a novel stack management discipline for enforcing
  control-flow integrity and encapsulation of local state on it. To prove
  these properties, we provide a logical relation that semantically
  captures capability safety. The capability safety defined by the
  logical relation goes beyond the
  proposed stack management discipline and can be used to reason about
  any capability safe program.
\end{abstract}
...
\figurename~\ref{fig:syntax}. We assume an infinite set of addresses
$\Addrs$ and define machine words as either integers or capabilities
of the form $((\perm,\gl),\var{base},\var{end},\addr)$. Such a capability
represents the authority to execute permissions $\perm$ on the memory
array $[\var{base},\var{end}]$, together with a current address $\addr$ and
a tag $\gl$ which indicates whether the capability is global or
local. The capability machine has no notion of pointers by itself, so
we refer to capabilities as pointers and capabilities
interchangeably. The available permissions and their ordering are
depicted in \figurename~\ref{fig:perm-hier}: the permissions include
null permission ($\noperm$), readonly ($\readonly$), read/write
($\readwrite$), read/execute ($\exec$) and read/write/execute ($\rwx$)
permissions. Additionally, there are three special read/write-local
($\readwritel$), read/write-local/execute ($\rwlx$) and enter
...
but when $c$ is jumped to, the data becomes available to the closure's code through
the updated version of $c$ that is now in the $\pcreg$\footnote{Note that the
  capability in the $\pcreg$-register can be used to load from.}.
% \lau{As we
...
section, but we refer to the technical appendix~\ref{XXX} for further
details.
...
abstraction we put on a piece of the memorey. This stack pointer must be local and it must carry $\rwlx$ permission. It is crucial for
...
clearly need to clear that data before returning to the adversary, to prevent
the adversary from gaining access to it.
...
store-local. The adversary could take advantage of this by using the unused part
...
prevent this, before returning, we need to clear (i.e. overwrite with zeros) the
entire part of the stack that the adversary has had access to, not just the
...
most problematic aspect of our discipline in terms of efficiency, since it may
consist of a large part of memory. However, we believe there are ways to
optimize it that we discuss in Section~\ref{sec:discussion}.
...
\textbf{Stack management discipline}
The precautions the needed to safely interact with an adversary have now been introduced and motivated. The stack discipline comprises the precautions. Summarized, they were:
\begin{description}[font=\normalfont\itshape]
\item[At program startup] a local $\rwlx$-capability (the stack pointer) resides in register $r_\stk$. The system does not hand out global write-local capabilities.
\item[Before returning to the adversary] clear the registers (with words we do not want to pass to the adversary) and erase any part of the stack we have used (if any part of the stack has been passed to an adversary, then that part must be erased).
\item[Before invoking the adversary] push an activation record to the stack, create a local $\entry$-capability for the restore instructions of the activation record (the return pointer) and leave it in register $r_0$, leave a copy of the stack capability restricted to the unused part in $r_\stk$, erase the part of the stack we release control over, and clear the non-argument registers.
\item[Before invoking an adversary callback] make sure it is a global capability.
\item[When called by an adversary] make sure the alleged stack pointer has permission $\rwlx$.
\end{description}
\textbf{Reusable macro instructions} 
We define a number of reusable macros. Some of them capture the stack
management discipline and others make the examples in
Section~\ref{sec:examples} readable. The macros are:
\texttt{fetch $r$ $\var{name}$} fetches the capability related to
$\var{name}$ from the linking table and stores it in register
$r$. \texttt{push $r$}/\texttt{pop $r$} works as one would expect. All
macros that uses the stack assumes a stack pointer in register
$r_\stk$.  \texttt{prepstk $r$} is used when a callback is invoked by
the adversary and prepares the received stack pointer by checking that
it has permission $\rwlx$ and making it conform to our stack
discipline. \texttt{scall
  $r$($\overline{r_{\var{args}}}$,$\overline{r_{\var{priv}}}$)} Jumps
...
the private state is restored. \texttt{mclear $r$} clears all the
memory the capability in register $r$ has authority
over. \texttt{rclear $\var{regSet}$} clears all the registers in
$\var{regSet}$. \texttt{reqglob $r$} checks whether the word in
register $r$ is a global capability. \texttt{crtcls
...
In Section~\ref{sec:examples}, we use the above macros and some
more. The technical appendix~\ref{XXX} contains detailed descriptions
of all the macros.
...
\dominique{I would try to motivate the LR defs by saying that the LR is
  ...
  respect them itself too.} The logical relation defines semantically what it
means to be capability safe.\dominique{We have not introduced capability
  safety..} In large parts, capability safety is defined by when a capability
can be safely given to an untrusted piece of code. In the following paragraphs,
...
relation is found in \figurename~\ref{fig:logrel},
\figurename~\ref{fig:standard-regions}, and \figurename~\ref{fig:perm-conds}.
Some definitions are omitted and explained when needed. The precise definitions
can be found in the technical appendix~\cite{XXX}.
...
$\var{localityReg}(g,W)$, which projects out all active regions when $g$ is
local, but only the permanent regions when $g$ is global. The read condition
uses $\iota^\pwl_{\start,\addrend}$, a standard regions defined in
Figure~\ref{fig:standard-regions}, which requires all the words in the range
$[\start,\addrend]$ are safe.
...
word, so the region governing the relevant part of memory must (at least) allow
any capability safe word to be stored. However, this is not entirely true
because of the distinction between write and permit-write-local capabilities.
Only permit-write-local capabilities need to allow arbitrary safe values to be
stored, while for write capabilities, it is sufficient to allow storing values
that are both non-local and safe. The write condition uses $\var{localityReg}$
to pick an appropriate region for its locality in the same way the read
condition does. The condition for the region to be address-stratified is there
...
\texttt{f1} in \figurename~\ref{fig:prog-f1} and \texttt{f2}
\figurename~\ref{fig:prog-f2} demonstrate this. They are very similar: both
...
Program \texttt{f1} assumes it has access to a stack pointer and uses the stack
to store its local state. It also uses the stack-based calling convention
\texttt{scall} to call the untrusted code. On the other hand, \texttt{f2} uses
...
f2: malloc $r_l$ 1
    ...
    halt
\end{lstlisting}
  \caption{Simple program that uses a calling convention not reliant
    on the stack}
  \label{fig:prog-f2}
\end{figure}
...
configuration is safe after some reduction steps. We then use a general lemma
for reasoning about \texttt{scall}, by which it suffices to show that (1) the
configuration that \texttt{scall} will jump to is capability safe and (2) that
the configuration just after \texttt{scall} is done cleaning up is capability
...
In \texttt{f3} there are two calls to an adversary and in order for the
assertion in the middle to succeed, they need to be well-bracketed. If the
adversary were able to store the return pointer from the first call and invoke
it in the second call, then \texttt{f3} would have $2$ on top of its stack and
...
adversary is cleared before the second invocation. IThe following lemma shows
...
The final example, in \figurename~\ref{fig:prog-g1}, is a faithful translation
of a tricky example known from the
literature~\citep{pitts_operational_1998,Dreyer:jfp12}. It consists of two
parts, \texttt{g1} and \texttt{f4}. \texttt{g1} is a closure generator that
generates closures with one variable $x$ set to $0$ in its environment and
\texttt{f4} as the program. \texttt{f4} expects one argument, a callback. The
program sets $x$ to $0$ and calls the callback. When it returns, $\texttt{f4}$
...
(it must have the correct permission and fit with our stack discipline) and (2)
require that the adversary callback be global.
...
the assertion can likely be made to fail too. However, our stack and return
pointer discipline guarantees that there is no way any of this can happen, as we
...
first with a capability for the closure creator \texttt{g1}. This means we need
...
execution of \texttt{f4}. During the execution of \texttt{f4} and the callbacks,
the adversary should not be able to rely on its local state from before the call
of \texttt{f4}. This is easily done by revoking all the temporary regions of the
world given at the start of \texttt{f4}. However, when \texttt{f4} returns, the
...
the return pointer that \texttt{f4} receives from the adversary may be local,
and the adversary is allowed to allocate the activation record on the stack
(just like we do) so that he can store and recover his old stack pointer after
\texttt{f4} returns.
...
local return pointer received by \texttt{f4} from the adversary is safe in
...
memory) for our local stack as well as the adversaries local stack. The given
...
state he/she might have stashed away.
...
\textbf{Formulating control flow integrity} The attentive reader may notice
that, while we claim that our stack and return pointer discipline enforces
control flow integrity, we do not prove a general theorem to show that this is
the case. The reason for this is that it is not clear what such a theorem should
look like. Formulations in terms of a control flow graph, like the one by
\cite{abadi_control-flow_2005}, do not take into account temporal properties,
like the well-bracketedness that Example~\texttt{g1} relies on.\footnote{It is a
  bit unfortunate that the general term ``control flow integrity'' is used for a
  property that only captures partial correctness of control flow.} In fact, our
examples show that our logical relation imply a form of control flow integrity
that is stronger than such formulations, although this is not made very
explicit. As future work, we are considering to look at a more explicit and
useful way to formalise well-bracketed control flow. The idea would be to define
a variant of our capability machine with built-in call and return instructions
and well-bracketed control flow built-in to the operational semantics, and then
prove that compiling such code to our machine using our stack and return pointer
discipline is fully abstract~\cite{abadi_protection_1998}, which roughly means
that the lower-level machine does not allow interactions with the compiled code
that do not correspond to an interaction with the original code in the
high-level machine.

\textbf{Performance and the requirement for stack clearing} It is important to
consider the performance of our proposed stack and return management discipline.
Generally, the additional security measures described in
Section~\ref{sec:stack-and-return-pointer} obviously impose an overhead w.r.t.
an approach that does not provide security. However, they generally consist of a
...
exception is the requirements that we have for stack clearing, in two
...
the stack that we have used ourselves, but we need to clear the entire stack,
except the part that we are using ourselves. This means that on every crossing
of the boundary between trusted and adversary code, a potentially large region
of memory needs to be zeroed out entirely. We believe that capability machines
like CHERI should consider to provide special support for this requirement: a
highly-optimized instruction for zeroing out a potentially large block of
memory. We have no experience in processor design, but we imagine this might be
optimized as a vector instruction or even further, by having the processor keep
track of a range of memory for which loads should pretend that it has already
been cleared, by returning zero irrespective of the actual contents of the
memory.

\textbf{Modularity} Our return pointers and callbacks satisfy the checks we
apply to the ones we get from the adversary (return pointers are allowed to be
local, callbacks must be global), i.e. we do not claim special status and what
we propose is modular.

\textbf{Local capabilities in high-level languages} It is worth mentioning that
local capabilities are quite similar to features that have been proposed for
high-level languages. Specifically, \citet{osvald_gentrification_2016}'s
second-class or local values in Scala are remarkably similar: they are a kind of
values that can be provided to other code for immediate use without allowing it
to store the value in a closure or reference for later use. We believe reasoning
about such values will require very similar reasoning techniques than the ones
we have used for local capabilities.
...
traces), we do not require their notion of effect parametricity.
...
hand-written and self-modifying assembly code), but they do assume code that is
non-adversarial and respects the calling convention of the high-level language
(for example, the code does not store return pointers and invoke them after
their intended lifetime). As a result, they can treat return pointers as
evaluation contexts (continuations) in a lambda calculus and use biorthogonality
to define the LR~\cite{pitts_operational_1998}. In our setting, there is no
guarantee that potentially adversarial code will respect the calling convention,
so, as described in Section~\ref{sec:logical-relation}, we do not use
biorthogonal closure but just model return pointers as continuations using a
...
nature of these capabilities can be seen as a generalisation of the
syntactically special, unstoreable status of evaluation contexts in lambda
calculi (without control effects). Closely related to this is a study by
\citet{Dreyer:jfp12} (DNB) of the impact of higher-order state and control
effects on local relational reasoning. DNB formally capture the special status
of evaluation contexts using Kripke worlds with public and private future world
relations. Our future world relations are inspired by theirs and they are at
times remarkably similar. For example, the state-transitition system in the
proof of our assembly version of the awkward example, is exactly the same as the
one used by DNB for the original awkward example. However, in our logical
relation, public and private future worlds turn up in an entirely different
place in the logical relation: rather than using public future worlds for the
special syntactic category of evaluation contexts, they are used in the value
relation depending on the depending on the locality of the capability at hand.
Additionally, our worlds are a bit more complex because, to allow store-local
capabilities, they can have temporary regions that are only monotonous w.r.t.
public future worlds, while DNB's worlds are entirely permanent.
