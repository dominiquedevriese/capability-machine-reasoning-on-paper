...
\begin{figure}
  \begin{tikzpicture}[scale=.7, every node={scale=.7}]
    ...
    \draw (0,1) rectangle (6,1.5) node[pos=.5] {\footnotesize previous program counter};
    \draw (0,1.5) rectangle (6,2) node[pos=.5] {\footnotesize previous stack pointer};
...
local state (the contents of registers $\overline{r_{\var{priv}}}$), return
pointer and stack pointer to the stack, sets up the necessary on stack return
...
first with a capability for the closure creator \texttt{\footnotesize{g1}}. This
means we need to check that all arguments are safe to use and that we clean up
before returning in the end. The inversion of control makes it challenging to
reason about the adversary's local state during the execution of
\texttt{\footnotesize{f4}}. During the execution of \texttt{\footnotesize{f4}}
and the callbacks, the adversary cannot use their local state from before the
call of \texttt{\footnotesize{f4}}. This is modeled by revoking the temporary
regions of the world we get at the start of \texttt{\footnotesize{f4}}. However,
when \texttt{\footnotesize{f4}} returns, the adversary's old local state becomes
available again, since they are allowed to allocate their activation record on
the stack (just like we do) so that they can store and recover their old stack
pointer after \texttt{\footnotesize{f4}} returns. This means that we need to
restore validity of the adversary's old local state, which we achieve
essentially by clearing the stack.

In Figure~\ref{fig:worlds-in-pf-g1}, we illustrate how we accommodate this in
the proof of Lemma~\ref{lem:correctness-g1} by constructing suitable worlds for
all situations where we pass control to the adversary. The potentially local
return pointer received by \texttt{\footnotesize{f4}} from the adversary is safe
in $W_1$, so it can only be used in public future worlds. Let us see how $W_6$
is constructed as a private future world of $W_5$ but also a public future world
of $W_1$. The worlds $W_2$ and $W_4$ are constructed by revoking all temporary
regions and adding a $\iota^\pwl$ region for the stack we pass away and static
temporary regions (which accept exactly one memory) for our local stack as well
as the adversary's old local stack. The given worlds $W_3$ and $W_5$ are public
future worlds in which the adversary chooses to return to us. None of these
worlds can have replaced the temporary regions of $W_1$ with permanent regions.
Therefore, by revoking the temporary regions of $W_5$ and reinstating the
temporary regions of $W_1$, we can construct $W_6$ as a private future world of
$W_5$ but also a public future world of $W_1$. Because of the latter, it is safe
to use the local return pointer to return to the adversary, since we have
...
\caption{Illustration of the worlds in the proof of
  Lemma~\ref{lem:correctness-g1}. In the proof, the above row of worlds are
  constructed by us, while the below worlds are given. $W_6$ is constructed such
...
to formalise control flow correctness. The idea would be to define a variant
...
calling convention without security guarantees. However, most security measures
...
allowed to be local too. Adversary callbacks on the other hand are required to
...
how we model the special nature of local capabilities using public/private
...
effects. To reason about code that relies on this (particularly, the original
awkward example), \citet{Dreyer:jfp12} (DNB) formally capture the special status
of evaluation contexts using Kripke worlds with public and private future world
relations. Essentially, they allow relatedness of evaluation contexts to be
monotone with respect to a weaker future world relation (public) than
relatedness of values. This formalises the idea that it is safe to make
temporary internal state modifications (private world transitions, that
invalidate the continuation, but not other values) while a term is performing
internal steps, as long as it returns to a stable state (i.e. transitions to a
public future world of the original) before returning. We generalise this idea
to reason about local capabilities: validity of local capabilities is allowed to
be monotone with respect to a weaker future-world relation than other values,
which we can exploit to distinguish between state changes that are always valid
(public future worlds) and changes that are only valid if we clear local
capabilities (private future worlds). Our future world relations are similar to
\citet{Dreyer:jfp12}'s (for example, our proof of the awkward example uses the
exact same state transition system), but they turn up in an entirely different
...
relation depending on the locality of the capability at hand. Additionally, our
worlds are more complex because, to allow local memory capabilities and
write-local capabilities, they can contain (revokable) temporary regions that
are only monotonous w.r.t. public future worlds, while DNB's worlds are entirely
permanent.
...
Finally, we summarise how our work relates to previous work. We do not
