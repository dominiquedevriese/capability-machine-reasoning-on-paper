...
\begin{specification}[Malloc]
  ...
  \end{itemize}
then there exists addresses $b'$ and $e'$ where $b'-e' = m$ such that for all future worlds $W' \future W$ where $\heapSat{\hs'}{n-j}{W'}$ for $\hs' = hs[b' \mapsto 0, \dots, e' \mapsto 0]$ we have $(\reg,\hs) \step[j] (\reg',\hs')$ for some register file $\reg'$ that has the following constraints:
\begin{itemize}
