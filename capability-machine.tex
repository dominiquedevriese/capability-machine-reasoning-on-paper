...
Assume the instructions of some adversary resides in the memory starting at the memory location marked with $\codelabel{adv}$.
\label{sec:tick-disp}
\[
  \begin{array}{r l}
    \init: & \move{\refreg{r_1}}{\size} \\
           ...
           & \store{\refheap{r_1}}{0} \\
           & \lea{\refreg{r_1}}{-7} \\
           & \restrict{\refreg{r_1}}{\refreg{r_1}}{(\encodePerm{(\entry)})} \\
           & \move{\refreg{r_0}}{\refreg{\pcreg}} \\
           & \lea{\refreg{r_0}}{4} \\
...
\begin{lemma}
 Given $\Phi$, if $\memheap$ contains the ticket dispenser program starting at the address labelled $\init$, $\memreg(\pcreg) = (\exec,\init,\init+26,\init)$, and $\Phi \rightarrow^* (\halted,\Phi')$, then if $i$ was the address returned by the call to $\malloc$, then $\refheap(i+8)$ is even and positive.
\end{lemma}
The magic number 26 is the size of the ticet dispenser program (minus one), so the $\Phi$ is assumed to have an execute permission for the area of the heap where the program resides.
...
\begin{align*}
  ...
  \stder(W) \defeq & \{ \npair{\reg} \mid \\
                   ...
                   & \qquad \quad (\reg\update{r_0}{c},\heap \uplus \heap_f) \in \observations(W) \}
\end{align*}
