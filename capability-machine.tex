...
\begin{specification}[Malloc v.2]
  \begin{align*}
    ...
    &\qquad \exists i, \FS \ldotp \\
    &\qquad \quad W (i) = \iota_{\malloc}^{\FS} \land \\
    ...
    &\qquad \qquad \qquad W(i) = \iota_{\malloc}^{\FS \setminus [b',e']} \land \\
    &\qquad \qquad \qquad [b',e'] \subseteq \FS  \land \\
...
\[
\begin{gathered}
  \iota_{\malloc}^{\FS} \defeq (\FS, \phi_{\malloc}, H_{\malloc}) \\
  \FS \subseteq \Addrs \\
  \forall \FS_1, \FS_2 \ldotp (\FS_1,\FS_2) \in \phi_{\malloc} \text{ iff } \FS_1 \supseteq \FS_2 \\
  H \; W \; \FS = {\npair{\hs} \mid \dom(\hs) = \FS}
\end{gathered}
\]
...
\exists R \ldotp 
\begin{gathered}
  \dom(W) \rightarrow \HeapSegments \ldotp hs = \biguplus_{r \in \dom(W)} R(r) \\
  \text{and}\\
...
\]
\begin{align*}
  \iota_{\var{base},\var{end}}(W) & \defeq (s,=,H_{\var{base},\var{end}}) \\
  H_{\var{base},\var{end}}\; s\; W         & \defeq \{\npair{\hs} \mid \forall \addr \in [\var{base},\var{end}] \ldotp \hs(\addr) \in \stdvr(\roll \; W) \\
...
\]
\begin{align*}
  \iota_{\var{start},\var{end}}^0\; s\; W & \defeq (s,=,H_{\var{base},\var{end}}^0) \\
  H_{\var{base},\var{end}}^0\; s\; W     & \defeq\{ \npair{\heapseg} \mid \dom(\heapseg) = [\var{start},\var{end}] \land \\
...
\begin{lemma}
  \[
    \forall n \ldotp \forall W \future W' \ldotp \forall \reg \ldotp \forall \hs \ldotp \npair{(\reg,\hs)} \in \observations(W) \Rightarrow \npair{(\reg,\hs)}(W')
  \]
\end{lemma}
...
\begin{lemma}
  \[
    \forall n \ldotp \forall W' \future W \ldotp \forall \pc \ldotp \npair{c} \in \stder(W) \Rightarrow \npair{c} \in \stder(W')
  \]
\end{lemma}
...
\begin{lemma}[Write condition uniformity]
\[
