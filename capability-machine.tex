...
\begin{specification}[Malloc v.2]
  \begin{align*}
    ...
    &\qquad \exists i, \var{FS} \ldotp \\
    &\qquad \quad W (i) = \iota_{\malloc}^{\var{FS}} \land \\
    ...
    &\qquad \qquad \qquad W(i) = \iota_{\malloc}^{\var{FS} \setminus [b',e']} \land \\
    &\qquad \qquad \qquad [b',e'] \subseteq \var{FS}  \land \\
...
\begin{gathered}
  \exists R \dom(W) \rightarrow \HeapSegments \ldotp hs = \biguplus_{r \in \dom(W)} R(r) \\
  \text{and}\\
  \forall r \in \dom(W) \ldotp \forall n' < n \ldotp \npair[n']{R(r)} \in W(r).H(W(r).s)(\roll\; W)
\end{gathered}
\]
Where $\iota.s$ and $\iota.H$ project out the state and the predicate of the region respectively.
\end{definition}
