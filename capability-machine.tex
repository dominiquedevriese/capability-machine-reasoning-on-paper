...
\label{lem:reg-dc}
  \begin{align*}
    &\forall \stdvr, n' < n \ldotp \forall W \ldotp \forall \reg \ldotp\\
...
\label{subsubsection:continuation-relation}
...
\label{lem:stdkr-dc}
  \begin{align*}
    &\forall \stdvr \ldotp \forall n' < n \ldotp \forall W \ldotp \forall c \ldotp\\
...
\label{subsubsection:expression-relation}
...
\label{lem:stder-dc}
  \begin{align*}
    & \forall \stdvr \ldotp \forall n' < n \ldotp \forall W \ldotp \forall \pc \ldotp\\
...
\begin{align*}
  ...
  H_{\var{std}} & : (\Worlds \monnefun \UPred{\Words}) \nefun \States \nefun \Worlds \monnefun \UPred{\HeapSegments} \\
  H_{\var{std}} \; \stdvr \; (\var{base},\var{end})\; W &  \defeq \left\{ \npair{\hs} \middle|
                                               \begin{aligned}
...
\label{lem:rc-mono-world}
  \begin{align*}
    &\forall \stdvr, n,W,W',\start, \addrend \ldotp \\
...
\label{lem:rc-ne-world}
  \begin{align*}
    & \forall \stdvr, b,e, n, W, W' \ldotp \\
    & \quad (b,e,W) \nequal (b,e,W') \Rightarrow \\
    & \qquad \readCond{}(\stdvr)(b,e,W) \nequal \readCond{}(\stdvr)(b,e,W')\\
  \end{align*}
\end{lemma}
...
\label{lem:ec-mono-world}
  \begin{align*}
    &\forall \stdvr,n,W, W',\start, \addrend, \perm \ldotp \\
...
\label{lem:ec-ne-world}
  \begin{align*}
    &\forall \stdvr, W_1, W_2,n,\start,\addrend,\perm \ldotp \\
    &\quad (\start,\addrend,\perm,W_1) \nequal (\start,\addrend,\perm,W_2) \Rightarrow \\
    &\qquad \Rightarrow \execCond{}(\stdvr)(b,e,p,W_1) \nequal \execCond{}(\stdvr)(b,e,p,W_2)
  \end{align*}
\end{lemma}
...
\begin{lemma}[Entry condition monotone in world]
  ...
  \begin{align*}
    &\forall \stdvr,n,W, W',\start, \addrend, \addr \ldotp \\
...
\label{lem:en-ne-world}
  \begin{align*}
    &\forall \stdvr, W_1, W_2,n,\start,\addrend,\addr \ldotp \\
    &\quad (\start,\addrend,\perm,W_1) \nequal (\start,\addrend,\perm,W_2) \Rightarrow \\
    &\qquad \Rightarrow \execCond{}(\stdvr)(b,e,p,W_1) \nequal \execCond{}(\stdvr)(b,e,p,W_2)
  \end{align*}
\end{lemma}
...
\begin{align*}
  ...
  & \{ \npair{i} \mid i \in \ints \} 
    \union \\
       ...
       \end{aligned}
\end{align*}
Using the Banach fixed-point theorem, we can find a fixed point for $\stdvr$ We will use this fixed point in the next section to define the logical relation we use in practice.
...
\begin{lemma}[$\iota_{\start,\addrend}^0$ is well-defined]\forcenewline
...
In order to define the value relation concisely using the standard regions from Subsection~\ref{subsec:standard-regions}, we define the following conditions:
\begin{align*}
...
\begin{align*}
  ...
  \stder(W) \defeq & \{ \npair{\pc} \mid \\
                   ...
                   & \qquad \qquad \quad \npair[n']{(\reg\update{r_0}{c}\update{\pcreg}{\pc},\hs)} \in \observations(W) \}
\end{align*}
\lau{Added $n' \leq n$ to be able to prove Lemma~\ref{lem:stder-dc}}
...
Let $\stdvr$, $n$,$W$, $W'$, $b$, and $e$ be given where $W' \future W$. Let $n \in \readCond{}(\stdvr)(\start,\addrend,W)$, this gives an $r$ and $[b',e'] \supseteq [b,e]$ such that $W(r) \nsubeq \iota_{b',e'} (r) = ((b',e'),=,H_{\var{std}})$. This gives
\begin{align*}
  &W(r).\phi \equiv = \\
  &W(r).s    = (b',e') \\
...
\[
  (b,e,W) \nequal (b,e,W')
\]
which implies $W \nequal W'$. Now let $(n,\hs) \in \readCond{}(\stdvr)(b,e,W)$ be given. This gives us $r$ and $[b',e'] \supseteq [b,e]$ such that 
\[
  W(r) \nsubeq[n-1] \iota_{b',e'}(\stdvr)
\]
Using $r$ and $[b',e']$, we need to show
\[
  W'(r) \nsubeq[n-1] \iota_{b',e'}(\stdvr)
\]
Let $W(r) = (s,\phi,H)$, $W(r) = (s',\phi',H')$, and $\iota_{b',e'} = (s_\iota,\phi_\iota,H_\iota)$.
From the $n$-equality of our worlds, we get $W(r) \nequal W'(r)$. From this we conclude
\[
  (s',\phi') = (s,\phi) = (s_\iota,\phi_\iota)
\]
It remains to show
\[
  \forall \hat{W} \in \Wor \ldotp H' \; s \; W \nequal[n-1] H_\iota \; s \; W
\]
Given $\hat{W}$ we know from $n$-equality of the worlds
\[
   H' \; s \; \hat{W} \nequal H \; s \; \hat{W}
\]
Given $(k,\hs) \in H' \; s \; W$ where $k < n$ and $\hs$ is a heap segment, by above we have $(k,\hs) \in H \; s \; W$ and from the $\nsubeq[n-1]$ assumption, we get $(k,\hs) \in H_\iota \; s_\iota \; W$.
\end{proof}
...
\begin{proof}[Proof of lemma~\ref{lem:ec-ne-world}]
  Assume $(\start,\addrend,\perm,W_1) \nequal (\start,\addrend,\perm,W_2)$ and thus $W_1 \nequal W_2$. Given $n'<n$, $W_1' \future W_1$, and $\addr$, show
\[
  \npair[n']{(\perm,\start,\addrend,\addr)} \in \stder(\stdvr)(W_1')
\]
by lemma~\ref{lem:nequal-and-future-world} there must exist $W_2'$ such that $W_2' \future W_2$ and $W_2' \nequal W_1'$. By assumption $n \in \execCond{}(\stdvr)(\start,\addrend,\perm,W_2)$ we have
\[
\npair[n']{(\perm,\start,\addrend,\addr)} \in \stder(\stdvr)(W_2')
\]
and by non-expansiveness of $\stder(\stdvr)$ (lemma~\ref{lem:stder-ne-worlds}), we get the desired result.
\end{proof}
...
\label{lem:wc-implies-rc}
  ...
  \end{align*}
\end{lemma}[Proof of lemma~\ref{lem:wc-implies-rc}]
\begin{proof}
  Follows directly from the definition.
\end{proof}
...
\end{lemma}
\begin{proof}
  Follows from uniformity of $\readCond{}$, $\writeCond{}$, $\execCond{}$, and $\entryCond{}$ in the worlds. That is Lemma~\ref{lem:rc-mono-world},~\ref{lem:wc-mono-world},~\ref{lem:ec-mono-world}, and~\ref{lem:en-mono-world}.
\end{proof}
...
\section{Proof of Lemmas in Section~\ref{subsec:further-standard-regions}}
