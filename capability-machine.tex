...
let h = fun adv =>
          ...
          return
\end{verbatim}
In Dreyer's example, the issue arises as the context can provide the result of evaluating $\mathtt{e_1}$ (the equivalent here is \texttt{g})) to \texttt{f} (the equivalent here is \texttt{adv}). In our system, the context cannot capture this, so in the above program, \texttt{adv} is given explicit access to \texttt{g}. The lemma we would want to prove here is that the assertion does not fail.

To show this, we will probably need public/private transitions.
...
\begin{align*}
  ...
  \observations (W) \defeq & \lambda W \ldotp 
                             ...
                             \begin{aligned}[t]
                               ...
                               & \qquad \Rightarrow
                               \begin{aligned}[t]
                                 & \exists W' \futurestr W \ldotp\exists \hs' \ldotp\\
                                 & \quad \heap' = \hs' \uplus \heap_f \land \\ 
