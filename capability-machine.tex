...
\begin{specification}[Malloc]
  ...
  \end{itemize}
then there exists a configuration $\Phi'$, a future world $W[r \mapsto \iota_{b',e'}] \future W$ for some addresses $b'$ and $e'$\lau{I had this to be exists future world $W'$, but it seemed useless as I couldn't argue that I could change the heap}, a natural number $i$ such that $\heapSat[{\memheap[\Phi']}]{n-i}{W'}$, $\dom(\memheap[\Phi']) = \dom(\memheap) \uplus [b',e']$  and for all $a \in \dom(\memheap)$ the heap is unchanged, i.e., $\memheap(a) = \memheap[\Phi'](a)$\footnote{This is probably not the best assumption, as $\malloc$ probably need to keep track of the allocated memory, but this is a simple way to make $\malloc$ trustworthy in the sense that it cannot give the same capability to the adversary code.}. Further the register file contains the following
\begin{itemize}
