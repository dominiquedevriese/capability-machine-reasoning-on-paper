...
% Save capability for counter two addresses after jump to adv.
           & \move{\refreg{r_3}}{\refreg{\pcreg}} \\
           & \lea{\refreg{r_3}}{17} \\
           & \store{\refheap{r_3}}{r_0} \\
           & \move{\refreg{r_3}}{0} \\
% Save counter
           & \store{\refheap{r_1}}{0} \\
% Sent pointer to start of ticket dispenser code
           & \lea{\refreg{r_1}}{-8} \\
% Set io to -1
           & \store{\refheap{\rio}}{-1} \\
...
% jump to adv
           & \jmp{\codelabel{adv}} \\
           & 0 \quad (\text{this address will be overwritten})\\
           & 0 \quad (\text{this address will be overwritten})\\
...
% retieve counter capability
           & \lea{r_0}{1} \\
           & \load{\refreg{r_2}}{\refheap{r_0}} \\
% load the value of the counter
           & \load{\refreg{r_0}}{\refheap{r_2}} \\
...
\begin{lemma}
 Given $\Phi$, if $\memheap$ contains the ticket dispenser program starting at the address labelled $\init$, $\memreg(\pcreg) = (\exec,\init,\init+56,\init)$, and $\Phi \rightarrow^* (\halted,\Phi')$, then the I/O address (that is $\memheap[\Phi'](0)$) contains either $-1$ or a number that is $\geq 0$ and even.
\end{lemma}
...
\begin{align*}
  ...
  \observations (W) \defeq & \{ (n,\reg,\heap) \mid (\reg,\heap) \diverge \}
\end{align*}
\begin{align*}
...
\end{definition}
\begin{lemma}[FTLR]
  For all $n \in \nats$, $W \in \Worlds$, and register files $\reg$, if for all $r \in RegName$ $(n,\reg(r)) \in \stdvr(W)$, then $(n,\reg) \in \stder(W)$
\end{lemma}
