...
Assume the instructions of some adversary resides in the memory starting at the memory location marked with $\codelabel{adv}$. Assume that the register $\rio$ initially contains a capability for the address in memory where I/O is written to (we assume this address is 0). The following is a test program for a ticket dispenser:
\[
...
\label{lem:tckt-disp}
 Given $\Phi$, if $\memheap$ contains the ticket dispenser test program starting at the address labelled $\init$, $\memreg(\pcreg) = (\rwx,\init,\init+\var{proglen}-1,\init)$, $\memreg(\rio) = (\readwrite,0,0,0)$, the remaining registers of $\memreg$ contains 0, and $\Phi \rightarrow^* (\halted,\Phi')$, then the I/O address (that is $\memheap[\Phi'](0)$) contains either $-1$ or a number that is $\geq 0$ and even.
\end{lemma}
...
\begin{specification}[Malloc]
  For all worlds, $W$, configurations, $\Phi$, natural numbers, $n$, and addresses $b$, $e$, and $return$ where $\heapSat[\memheap]{n}{W}$ if
  \begin{itemize}
  \item $\memreg(r_0) = (\entry,b,e,\var{return})$,
  \item $\memreg(r_1) = m$ where $m$ is some positive natural number\footnote{We do not include 0 as malloc in C returns a null pointer when malloc(0) is called.}, and
  \item the remaining registers contain 0,
  \end{itemize}
then there exists a configuration $\Phi'$, a future world $W' \future W$, a natural number $i$ such that $\heapSat[{\memheap[\Phi']}]{n-i}{W'}$ and $\dom(\memheap[\Phi']) \supseteq \dom(\memheap) \uplus [b',e']$\footnote{notice that $\malloc$ is allowed to allocated memory for its own purposes} for some addresses $b'$ and $e'$. Further the register file contains the following
\begin{itemize}
\item $\memreg(\pcreg) = (\rwx,b,e,\var{return})$,
\item $\memreg(r_0) = (\entry,b,e,\var{return})$,
\item $\memreg(r_1) = (\rwx,b',e',b')$, and
\item $\memreg(r) = 0$ for all other registers $r$.
\end{itemize}
\end{specification}
\begin{proof}[Proof (Lemma~\ref{lem:tckt-disp})]
Let configuration $\Phi$ be given. Assume that the ticket dispenser test code resides in memory starting at address $\init$. Further assume that the register file contains the following:
\begin{enumerate}
...
\end{enumerate}
Further assume $\Phi \rightarrow^* (\halted,\Phi')$.
\end{proof}
...
\begin{align*}
  ...
  \stdkr(W) \defeq & \{ \npair{c} \mid \\
                   ...
                   & \qquad  \forall W' \future W\ldotp %on the whiteboard, we had public future world.
                     \forall \heapSat[\heap]{n'}{W'} \ldotp \forall \reg \in \stdrr(W') \\
...
\begin{align*}
  ...
  \stder(W) \defeq & \{ \npair{\pc} \mid \\
                   ...
                   & \qquad \quad  \forall \heapSat[\heap]{n}{W}, \heap_f \ldotp \\
