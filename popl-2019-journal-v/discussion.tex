...
\begin{jversion}
  ...
  \dominique{Yes, but what if we want exceptions, can StkTokens be modified to support it? What about continuations? Also: should we mention the limited additional control flow we seem to get if we omit the stack base check (where  adversaries can use a kind of non-preemptive multi-threading)? Perhaps the answer to some of these question is that we don't know but we should still give it.}
  ...
  \dominique{I find this ``vertical extensionality'' both vague and unrealistic. Should we really discuss this?}
  ...
  \dominique{The word ``however'' is wrong in the next sentence.  IIUC, you're explaining *why* we require the previous compiler phase to have ensured reasonability instead of enforcing reasonability in StkTokens.
  This is very confusing.}
  \dominique{Also: I'm not sure it's a good idea to lump together reasonability and well-formedness.
    In my understanding, well-formedness is a set of *syntactic* constraints and the requirement that the linker has distributed seals properly.
    As such, the discussion below seems to be only about reasonability, not well-formedness.
  }
