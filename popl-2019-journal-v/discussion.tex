...
\begin{jversion}
  A compiler is fully abstract when it enforces all high-level properties of a high-level language which means that the high-level language that dictates what we need to enforce.
  In other words to figure out what properties we want, we need to figure out what high-level language we want.
  % TODO is this all we have to say about high-level properties? Maybe something about address hiding and the potential issues. Also other notions of control-flow, e.g. call/CC
  % TODO say something about horizontal vs vertical composition.

  % TODO new paragraph: conditional full-abstraction

  % TODO new paragraph: Machine checked proof. MANY details, any future work should be machine checked.

  % TODO new subsection on construction of overlay semantics:
...
% TODO passing pointers for stack allocated memory is not possible with linear stack
