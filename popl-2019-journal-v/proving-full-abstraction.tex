...
The future world relation can also be thought of as the model of memory changes over time.
...
\[
  ...
  \end{array} \right.
\]
We make a similar change to $\Regionh$, and we make the seal invariant monotone as well.

Kripke future world relation must be defined such that future worlds are extensions of the past world.
...
Specifically, if we have the only linear capability for a piece of memory, then we can be sure that there are no other capabilities for the same memory which makes it sure to repurpose the memory.
In terms of the world, this corresponds to dropping a region and replacing it with a new one.
Due to the fact that future worlds must be an extension of the past world, we cannot drop a region.
Instead, we mark it as revoked which for all intends and purposes corresponds to dropping the region. 
...
As we will see later, the logical relation makes disjoint splits of ownership, so each capability gets its own ownership to depend on.
A linear capability that is repurposed will get an entirely new world to depend on.
After all, a future world would have to respect the old assumptions, but a repurposed capability needs to depend on new assumptions.
The new world constructed for the repurposed linear capability would have a revoked region in place of the $\spatialo$-region, but the new world would not be a future world of the world it depended on before.
While the repurposed capability gets a new world to depend on, all other capabilities in the system must remain valid in a future world to the one they depend on.
...
\end{scope}
  ...
  \draw[gray, fill=gray, fill opacity=.4] (3,5) rectangle (5,0.5) node[fitting node] (w2l) { };
  ...
  % TODO do something about the capabilities
  \node[draw,rectangle, black, minimum size=2mm] (w1r) at (-2,5) {  };
  ...
  \node[draw,rectangle, black, minimum size=2mm] (w2r) at (6.5,5) {  };
...
\lau{TODO: insert monotonicity lemma}
