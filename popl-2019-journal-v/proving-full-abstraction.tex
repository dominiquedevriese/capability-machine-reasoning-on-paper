...
\todo[inline]{Dominique: I would try to formulate this more positively. It sounds too much like ``In this version, we copy-pasted boring stuff from the TR to try and fill more pages.''. We should say \emph{why we think those details are interesting} to the reader. Perhaps something like ``In the previous version, we were forced to omit many aspects of the logical relation, and especially the way it deals with the linking model.  This was unfortunate and therefore the current version takes a bit longer to explain those aspects. Readers who are looking for a shorter summary with less detail may ...''}
...
\todo[inline]{Dominique: Maybe it would be clearer to rename spatial\_owned/spatial to spatial/spatial\_shadow or something similar. Spatial\_owned is really the default meaning of spatial, and what we've called spatial is a peculiar thing that we use for a very specific purpose.
  This would make the discussion easier to follow, I think.}
...
\[
  ...
  \end{array} \right.
\]
\todo[inline]{Dominique: Perhaps rename pure to shared?  Or introduce the term pure earlier on?}
...
\]
\todo[inline]{Dominique: $+$?  Why not $\cup$?}
...
\todo[inline]{Dominique: step missing in the reasoning: expressing that the capability is not linear is done by stripping the world ... \emph{and then} requiring that the capability is valid w.r.t.\ this stripped world.}
...
\todo[inline]{
  I would try to link back to the discussion of worlds as collections of assumptions by first explaining that correctness of values may depend on another type of assumptions than those about the contents of memory, namely the intended use case of seals.
  Therefore, in addition to a relation on memory segments, some regions also carry a seal interpretation function that etc.
  I think this would make the role of the seal interpretation function clearer.
  }
...
This function takes a seal and returns a world-indexed relation that relates all the sealables that may be sealed with this specific seal.
That is
\[
...
During machine execution, the memory contents changes and parts of memory may be repurposed\todo[inline]{Dominique: Add something like ``, at least when we can be sure that there are no capabilities left in the system whose validity depends on the old contents of the memory.''?}.
...
\todo[inline]{Rather than introducing the future world relation as ``how memory may change over time'', then its precise definition and then monotonicity, I would explain monotonicity first and the precise definition of the future world relation second.
  IMHO, it makes a lot more sense if you think about the future world as modelling precisely those changes in assumptions that do not invalidate the relatedness of values.
  In other words, I would say something like ``Very often, it is the case that relatedness of two capabilities doesn't change if extra assumptions in the system are added. For example, two related capabilities remain related when an extra invariant is added on unrelated memory, or when a stack frame that it does not reference is dropped.
  In Kripke logical relations, this kind of assumption changes, that do not invalidate the relatedness of values, are modelled by the future world relation.
  etc.''
  }

  \todo[inline]{Maybe explain here that Kripke future world relations usually only allow for adding fresh assumptions on fresh memory or protocol steps that the system was designed to support from the start.
    However in our setting, we sometimes allow dropping assumptions, namely when linearity tells us that no value in the system depends on the capability anymore.
    By linking revocation of regions immediately to linearity, it will require less explanation later that only spatial regions can be revoked.
  }
In order to relate \trgcm{} and \srccm{}, we are mainly interested in changes to the stack as it repurposes stack frames.
Specifically, we need to introduce regions to govern new encapsulated stack frames, and we need to revoke regions for call frames that are popped from the call stack, to make place for a new frame.
...
\dominique{To be honest, I don't actually know what exactly would go wrong if we simply removed revoked regions.
  Perhaps we should think this through?
}
For technical reasons, we need the worlds to be extensional\todo{Dominique: meaning?}, so we cannot simply forget about regions that we want to remove.
...
Not all regions should be revocable.
\todo[inline]{
  Perhaps explain intuition with a phrase like ``Since regions express assumptions that the validity of other values in the system may depend on, we should only be able to revoke a region when we are sure that nothing depends on it.
  This is never the case for shared regions, as blabla copying blabla...
  However, if we have a linear capability to a block of memory, then linearity ensures that no other capability in the system can reference the same memory.''
  }
...
\todo[inline]{Dominique: it would be great if we could add an example here of two concrete capabilities, one of which is linear, the worlds in which they're valid and explain how we can revoke the linear one and repurpose the memory it addresses, but still keep the other capability valid in the new world?
}
...
In future worlds, part of the memory may change but for the memory invariant to be invariant, it should continue relating the same memory segments even when the rest of the world changes.
\dominique{This discussion might be easier if we had talked about monotonicity earlier.}
...
% TODO update terminology: memory invariant, H, and seal invariant, H_\sigma

\subsubsection{Solving the recursive domain equation}
\label{subsubsec:rec-dom-eq}
\dominique{Explain that this section provides technical details on how the recursive world equation can be solved and that readers may skip this if they like.}
% TODO consider new structure (the below)
% First introduce c.o.f.e.s and the structures you can freely use to get a solution for your recursive domain equation
% Then introduce all the specialized gunk necessary for our worlds
%
%solving the recursive domain equation means that we can present the actual definitions
% Also see the notes from Aina
...
\dominique{mention that this is a standard technique (cite the relevant papers) and an abstracted form of step-indexing (cite some more papers).}
...
\end{enumerate}
\end{definition}
\dominique{Explain how an o.f.e. can be seen as a set with extra structure.}
...
% TODO add comment about standard constructions (x, +, fun, later, etc) and fixed-points (what are the conditions? Contractive, and?). Mention that it does not suffice to define our worlds, so we need to have some specialized worlds.
...
\begin{definition}[Preordered c.o.f.e.]
  ...
  \end{itemize}
\end{definition}
With the preordered c.o.f.e.\ definition, we can define the remainder constructs necessary to define the world we need. Any c.o.f.e.\ can trivially be lifted to a preordered c.o.f.e.\ where the ordering is the identity.
...
\begin{lemma}[Spatial region preordered c.o.f.e.]
...
%% Sum
\begin{lemma}[Sum preordered c.o.f.e.]
...
\begin{lemma}[The preordered c.o.f.e.\ of partial functions]
...
\begin{lemma}[$\blater$ preordered c.o.f.e.]
  ...
  \]
  \dominique{shouldn't this be $x \nequal[n+1]_{\blater} x' \text{ iff } x \nequal[n]_X x'$}
\end{lemma}
...
The later placement does, however, affect what the logical relation definitions in Section~\ref{subsec:logical-relation} looks like, so in the end it is a matter of taste.\dominique{This reasoning is odd: it's a matter of taste because it affects what the LR looks like?}
% Solution theorem
\begin{theorem}
...
% TODO the following paragraph does not read well. Rewrite.
...
% Linear and non-linear case. Affects the future world we consider.
...
% TODO write why we have the two cases. (it allows us to have the "other" capability by the one we consider - and it feels like the only reason)
