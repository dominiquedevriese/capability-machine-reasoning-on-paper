...
\begin{definition}[The shared part of a world]
  ...
  \begin{align*}
    ...
    \purePart{W_\var{call\_stk}} &\defeq \lambda r\ldotp
                       \begin{cases}
                         ((\spatial,\var{sm}),\opc) & \text{if } W_\var{call\_stk}(r) = ((\spatialo,\var{sm}),\opc)\\
...
Theorem~\ref{thm:rec-dom-eq-sol} uses the method of~\citet{Birkedal:2011:SKM:1926385.1926401,Birkedal:tutorial-notes} to construct the solution $\Wor$ to the recursive equation.
...
\[
  H^\mathrm{sta,\square}_{\stpair{\ms}{\ms}} \; \gc \; \hat{W} \defeq \left\{ \npair{\stpair{\ms}{\ms}} \middle| 
    \begin{array}{l}
      ...
      \exists S : \dom(\ms_S) \fun \World \ldotp \xi(\hat{W}
      ) = \oplus_{\aaddr \in \dom(\ms)} S(\aaddr) \wedge\\
