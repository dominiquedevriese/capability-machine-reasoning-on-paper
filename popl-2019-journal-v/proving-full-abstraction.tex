...
The spatial region represent ownership a part of memory which ensures that a linear capability is not aliased.
...
\todo[inline]{Rather than introducing the future world relation as ``how memory may change over time'', then its precise definition and then monotonicity, I would explain monotonicity first and the precise definition of the future world relation second.
  ...
  \todo[inline]{Maybe explain here that Kripke future world relations usually only allow for adding fresh assumptions on fresh memory or protocol steps that the system was designed to support from the start.
    ...
    By linking revocation of regions immediately to linearity, it will require less explanation later that only shadow regions can be revoked.
  }
...
\]
\end{definition}
The remaining standard constructions are defined similarly.
Another important construction is the \emph{later} ($\blater$) c.o.f.e.\
The later c.o.f.e.\ construction is based on one c.o.f.e.\ which it decreases the index of.
\begin{lemma}[$\blater$ c.o.f.e.]
  ...
  where
  \[
    x \nequal[n+1]_{\blater} x' \text{ iff } \left\{
      \begin{array}{l}
        n = 0 \vee{}\\
        x \nequal_X x'
      \end{array}
  ...
  \]
\end{lemma}
The later c.o.f.e.\ is used to make sure that the index decreases before the self-reference which gives something to recurse on in order to find a solution for the domain equation.
If we only use the standard constructions to construct a self-referential equation and make sure to guard every negative occurrence of the self reference with the later c.o.f.e.\ construction, then the equation has a solution.
\todo[inline]{Insert the solution theorem here?}
...
We will need the standard construction for later, product, and sum, so we define them here.
...
The future world relation for the shared region is trivial as it is just equality
The shared regions we sketched are the easiest to define in terms of a c.o.f.e.\ as the future relation is just equality.
...
To this end, we combine the shared region preordered c.o.f.e.\ (Definition~\ref{def:pure-reg-p-cofe}) with the shared region preordered c.o.f.e.\ (Definition~\ref{def:shared-reg-p-cofe}) using the union preordered c.o.f.e.\ construction in Definition~\ref{def:sum-p-cofe}.
