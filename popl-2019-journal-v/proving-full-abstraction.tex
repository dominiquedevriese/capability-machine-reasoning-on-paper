...
\begin{figure}
  ...
  \draw[pattern=dots,draw=none] (0,0) rectangle (4.5,2);
  \draw[pattern=dots,draw=none] (0,9.5) rectangle (4.5,11);

  \draw[pattern=north west lines] (0,2) rectangle (4.5,4);
  \draw[pattern=crosshatch] (0,4) rectangle (4.5,5);
  \draw[pattern=north west lines] (0,5) rectangle (4.5,6);
  \draw[pattern=crosshatch] (0,6) rectangle (4.5,7.5);
  \draw[pattern=north west lines] (0,7.5) rectangle (4.5,8.5);
  \draw[pattern=crosshatch] (0,8.5) rectangle (4.5,9.5);

  \draw (5.75,2.25) node[anchor=west] {\tiny $\stkb$};
  \draw (5.75,2.25) edge[-latex] (4.75,2.25);
  \draw (5,9.5) edge[-latex] (5,8);
  \draw [decorate,decoration={brace,amplitude=7pt,mirror,raise=4pt},yshift=0pt] (-0.25,9.5) -- (-0.25,2) node [black,midway,xshift=-.75cm,yshift=0.05cm,rotate=45] {\footnotesize stack};

  \matrix [draw,below right] at (current bounding box.north east) {
    \node[pattern=dots,draw,label=right:{\tiny Heap ($\ms$)}] {}; \\
    \node[pattern=north west lines,draw,label=right:{\tiny Free stack ($\ms_\stk$)}] {}; \\
    \node[pattern=crosshatch,draw,label=right:{\tiny Encapsulated stack frame ($\stk$)}] {}; \\
};
