...
Words given to untrusted code may be stored by the untrusted code for later use.
This means that a word is only safe to give to untrusted code if it is safe for them to keep it.
This means that the untrusted value relation must be monotone with respect to the world, so words stay safe in any possible future world.
\begin{lemma}[Untrusted value relation monotonicity]
  \label{lem:monotonicity}
  For all integers $n$, words $w_1$ and $w_2$, and worlds $W' \future W$, if $\npair{(w_1,w_2)} \in \lrv(W)$, then $\npair{(w_1,w_2)} \in \lrv(W')$.
\end{lemma}
