...
\]
% TODO is the below notation even used?
...
Given a world, we sometimes want to express that a capability, that is otherwise valid with respect to the world, is not linear or indirectly depends on a linear capability.
...
We will also refer to the seal interpretation function as the seal invariant, and we will refer to the memory relation as the memory invariant or just invariant when it i unambiguous.
...
% TODO describe why "Spatial -> spatialo" ~ affine or at least why we have it?
...
which provide sufficient structure for us to solve recursive domain equations.
...
\begin{definition}[World disjoint union $\uplus$]
  ...
  \]
\end{definition}
The $\uplus$ world join does not guarantee that the result is a sensible world with respect to authority.
However, it does not guarantee that a join is a sensible memory specification.
...
That is, when we join the authority of two worlds, then the ownership of the two worlds should be completely separate.
...
At the top-level we have $\memSat{\ms_S,\ms_\stk,\stk,\ms_T}{W}$ which relates the source memory triple, $\ms_S$, $\ms_\stk$, and $\stk$, from \srccm{} to the target memory $\ms_T$ from \trgcm{}.
...
% TODO illustrate the protected stack ?
...
% TODO double check "co-termination" (equitermination?)
...
% TODO be more precise, what later paragraph means.
...
% TODO in the above, add something about the untrusted case where the seals should nut be in the "global" seals because the global seals are the one that we globally agree to be trusted.
...
Iff the capability had read permission, then $\readCond{}$ would make sure that the word would have to be safe\footnote{It should not be possible to obtain a capability that can be used to break invariants. After all, if such a capability was obtained, memory invariants could be broken.
...
When a write capability makes a change to memory, the change must be permitted by the regions that govern the memory it has authority over for it to not break the invariant and thus be safe.
...
The next case is capabilities that do not have a special representation on \srccm{} unlike stack pointers and return pointers.
For two capabilities to be related, they should be syntactically equal. That is, they should have same range of authority, linearity and so one.
Generally speaking, untrusted components should not have direct access to a trusted components code.
For this reason, we require that capabilities must have a range of authority outside the trusted address space if they are to be related.
...
For instance, if a capability has read-permission, then it should only by able to read safe values with it.
...
That is, if a capability has read permission, then $\readCond{}$ must be satisfied, if it has writer permission, then the $\writeCond{}$ must be satisfied, and if it has execute permission, then $\execCond{}$ and $\xReadCond{}$ must be satisfied.
If the capability has execute permission, then it must also be a non-linear capability.
Finally, the capability cannot have read/write/execute permission because that would break the write-XOR-execute assumption.
...
In order for a pair of \srccm{} and \trgcm{} seals to be in the unsafe value relation, they must be sealed with the same seal $\sigma$, and they must be related by the seal invariant $\sigma$.
% Sealed capability only useful with pairing. Sealed caps must be permitted.
Further, if we take any sealed capabilities related under $\seal$ in the seal invariant, then they should produce a safe result.
...
It should be safe to use the sealed capability both now and later.
This means that it should be safe in the world $W$, but also in future worlds $W' \future W$.
...
Further, the seals in the set should be disjoint from the return seals and trusted closure seals ($\gsigrets$ and $\gsigcloss$ because trusted code relies on having the sole access to them.
...
% TODO add approx square to relation, add gc
\begin{figure}
  \centering
  \[
  \lrv(W) =
...
Further, we need to know that they are actually capability which is why we require the $\xReadCond{}$ to be satisfied.
...
The code memory contains trusted closure seals and return seals that we cannot let untrusted code have, so the read condition would not hold.
It is, however, fine to let trusted code have access to such seals because we expect it to treat the seals reasonably.
\begin{figure}
  \centering
  \[
...
The definition of the value relation is straightforward.
It distributes the authority of the world among the registers and require each of the registers to contain a safe word with respect to the authority it is given.
The register file never takes into account the $\pcreg$ and it can leave out further registers.
We use this to not relate register content that we are going to overwrite with new values.
We write $\lrr(W)$ to mean $\lrr(\emptyset)(W)$.
That is, if we do not need to exclude extra registers, then we simply omit that argument.
The register file relation is defined in \figurename~\ref{fig:reg-file-relation}.
...
% TODO write section
The expression relation $\lre$ defines when two capabilities can be used in the $\pcreg$-register to produce related executions, i.e. construct configurations in the observation relation.
The $\lre$ relation can be used to reason about the safety of an executable capability, i.e.\ capabilities that can change the control-flow control-flow during execution when a $\tjmp{}$ instruction is executed.
...
The $\txjmp{}{}$ updates the $\pcreg$ register and the $\rdata$ register, however, the $\lre$ relation only updates the $\pc$-register, so we cannot use $\lre$ to reason about sealed capabilities.
...
In this case, related executions means that the observable effect of the executions should be permissible.
...
We cannot pick arbitrary configurations because an arbitrary configuration may contain words that can be used to break memory invariants and create observable effects that we do not permit.
...
Each of the configurations are plugged with a code capability and a data capability just like the $\txjmp{}{}$ instruction would do it and requires the resulting configurations to be in the $\lrexj$ relation.
The $\lrexj$ relation is defined in \figurename~\ref{fig:expr-rels}.
\begin{figure}
  \centering
  \begin{align*}
% TODO add approx square
...
Specifically, we say that two components are related when they are syntactically equal after all a \srccm{} component is in some sense the same as a \trgcm{} as we only see the difference during execution when a call happens and when we lift a component to a configuration which means that we need to introduce a stack pointer.
However, we cannot take arbitrary components as they could potentially break memory invariants.
For related components, we require that if the imports are satisfied with related words, then the resulting memory should be safe.
Further, related components must have safe exports.
This roughly describes when a base components are related.
A component with a main is related when the base component is related and the mains capabilities are in the exports.
...
Contextual equivalence (Definition~\ref{def:contextual-equivalence}) plugs two components with a context and requires equitermination of the resulting executable configurations.
This means that we need to lift relatedness one step further, namely to the level of execution configurations.
...
Further, the two memory and the two register files should be related.
