...
A world which contains a spatial region, represents the assumption of exclusive ownership of the represented memory fragment.
In other words, a linear capability for this memory may be valid in such a world.
However, when we do not possess exclusive ownership of a memory fragment, that does not mean we should be entirely unaware of the memory's existence.
...
When we discuss compatibility of worlds w.r.t. ownership in Section~\ref{subsubsec:joining-worlds}, we will see that a world with a shadow region can be combined with a world that has the same region as $\spatialo$.
