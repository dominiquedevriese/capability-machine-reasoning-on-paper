...
\]
The sub-worlds are partial maps from names $\RegionName$ (not to be confused with register names), modelled as natural numbers, to invariants.
In order to define what this actually means, we need to be more precise about what we mean by invariants.

The invariants are registered in the worlds as regions.
...
The spatial region represents ownership of a part of memory which ensures that a linear capability is not aliased.
Even though spatial regions give the right to reference part of memory, we still need the world to specify the remainder of the memory that may be referenced by linear capabilities.
...
Instead, the stack frame and code return point are connected by the capabilities that reference them as they are sealed with the same seal and together then constitute the sealed return pair.
...
The logical relation presented in Section~\ref{subsec:logical-relation} uses this bookkeeping to ensure that linear capabilities \emph{uniquely} reference part of memory.
...
Safety of capabilities should be defined, so that it is monotone with respect to the future world relation.
...
\end{align*}
\end{definition}
The $\oplus$ operator, like the $\uplus$ operator, does not guarantee that the resulting world is sensible.
For example, the resulting world might have several regions that impose conflicting requirements on a single memory location.
However, when we know that a certain memory satisfies the world (as a memory specification, see Section~\ref{subsubsec:mem-sat}), then we can be sure that the world's specifications are non-contradictory.
...
The free stack in between stack frames may be used by untrusted code for their local stack frames As such, untrusted stack frames are not natively protected by \srccm{}.
...
This also means that \srccm{} configurations that terminate by failing termination are related to any \trgcm{} configuration.
