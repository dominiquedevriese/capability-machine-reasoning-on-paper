...
\todo[inline]{ I would try to link back to the discussion of worlds as collections of assumptions by first explaining that correctness of values may depend on another type of assumptions than those about the contents of memory, namely the intended use case of seals.
...
For instance, return seal must only be used to seal the return pointer of one specific return point.
...
In this section we present some of the technical details on solving as well as the final world domain equation (Theorem\ref{thm:rec-dom-eq-sol}).
This section may be skipped by readers with no interest in the recursive domain equation.
