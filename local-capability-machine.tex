...
\]
for some $j$ where for some $[0 \mapsto \iota_{\malloc}] \futurewk [0 \mapsto \iota_{\malloc,0}]$
\begin{enumerate}
\item $m' = \hs_{f1} \uplus 
         \hs_\flag \uplus                
         \ms_{\var{link}} \uplus 
         ...
         m_{\malloc}' \uplus 
         \hs_{\var{frame}} $
\item $\heapSat[m_{\malloc}']{n-j}{[0 \mapsto \iota_{\malloc}]}$ \label{f1:mallocsat}
\item $\dom(\hs_l) = [l,l]$
...
\item $m_{\var{footprint}} =  m_\malloc' $
\end{itemize}
Get:
\[
  (\reg_0\update{\pcreg}{c_\malloc}\update{r_0}{c_{f1}''}\update{r_1}{\dots}\update{r_l}{c_l},m'')
...
\end{enumerate}
Contniue execution until just after the jump to $\var{adv}$.
\[
  (\reg_0\update{\pcreg}{c_{f1}''}\update{r_0}{c_{f1}''}\update{r_1}{c_{\var{ar}}}\update{r_l}{c_l},m^{(3)})
\step[k']
  (\reg_0\update{\pcreg}{\updatePcPerm{c_{\var{adv}}}}\update{r_0}{c_{ar}'}\update{r_l}{c_l},m^{(3)})
\]
for some $k'$ wher
\begin{itemize}
\item $\ms_{\var{ar}}$ contains the activation record, i.e., $c_l$, $c_{f1}^{(3)}$, and activation code.
\item $c_{ar}' = ((\entry,\local)b,e,b+2)$ where $b+2$ is the first address of the activation code.
\end{itemize}
