...
\begin{definition}
  We say that $(\reg,\ms) \text{ is looking at } [i_0,\cdots,i_n] \text{ followed by } c_{\mathit{next}}$ if
  \begin{itemize}
...
\begin{definition}
  $(\reg,\ms) \text{ has stack $\ms_\stk$ with $\ms_{\mathit{unused}}$ unused}$
...
\begin{lemma}[$\mathtt{scall}$ works]
  ...
  \item[Hyp-Callee] If
    ...
    \item $\reg'= \reg_0[\pcreg\mapsto\updatePcPerm{\reg(r)},
      ...
      \mapsto c_{\mathit{ret}}, r_\stk \mapsto \_]$ \lau{add stack capability?}
  ...
  \item[Hyp-Cont] If
    ...
    \item for all $r$, we have that:
      \begin{equation*}
        ...
        \end{cases}
      \end{equation*}
    \item $(\reg',\ms'')$ has stack $\ms_\stk$ with
        $\ms_{\mathit{unused}}''$ unused for some $\ms_{\mathit{unused}}''$
    \item $\memSat[n']{\ms''}{W''}$
    \end{itemize}
    then we have that $(n', (\reg',\ms'')) \in \observations(W'')$
  \end{itemize}
  Then 
  \begin{itemize}
