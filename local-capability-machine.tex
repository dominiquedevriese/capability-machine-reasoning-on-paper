...
\dominique{The comment immediately above is outdated, right?}
...
\lau{do we want the FTLR to also talk about everything that is trivially in the FTLR.}
  \label{thm:ftlr}
...
\begin{proof}[Proof of Lemma~\ref{lem:pwl-nsim-view}]
  ...
  \begin{align*}
    ...
    \xi (W')  =& [0 \mapsto \iota^\pwl_{\start,\addrend}]  \\ 
              & [1 \mapsto \revoked] 
  \end{align*}
  \dominique{The above still needs updating now that revoked regions remember
...
\begin{lemma}[$\mathit{writeCondition}$ downwards-closed]
  \label{lem:writecond-mono-pub}
  \[
    \forall n' \leq \ldotp \forall \iota \in \{\iota^\pwl,\iota^\nwl,\iota^{(\nwl,p)}\} \npair{\start,\addrend} \in \writeCond{}(\iota,\gl)(W) \Rightarrow \npair[n']{\start,\addrend} \in \writeCond{}(\iota,\gl)(W)
  \]
\end{lemma}
