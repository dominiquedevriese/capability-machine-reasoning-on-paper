...
\cline{2-2}
\end{tabular}
\caption{ Stack and register-register file after the clean up code has been run. The ``private'' words have been poped to their respective registers. The restore code has been poped off the stack. }
\end{figure}
...
W = \;& [0 \mapsto \iota_{\malloc,0}] \\
    ...
    & [6 \mapsto \iota^\pwl_{\start_\stk'', \addrend_\stk} (\temp)]
  \end{align*}
...
\begin{enumproof}[resume]
\item $\heapSat[\ms]{n}{W}$ \lau{The argument here is a bit informal}
  \begin{itemize}
  ...
  \item $\ms_\stk'$, here we need to split the memory segment in two and for $n' < n$ argue
    ...
    \end{itemize}
  \end{itemize}
\item $\npair{\reg_0\update{\pcreg}{\updatePcPerm{c_\adv}}\update{r_0}{c_{\var{prp}}}\update{r_1}{c_\adv}\update{r_\stk}{c_\stk''}} \in \stdrr(W)$
  \begin{enumproof}
...
\[
  \iota^\pwl_{\start_\stk'', \addrend_\stk}(\temp) \nsubeq[n-1] \iota^\pwl_{\start_\stk'', \addrend_\stk}(\temp)
\]
which is indeed the case.
        \item $\npair{(\start_\stk'', \addrend_\stk)} \in \writeCond{}(\iota^\pwl,\local)(W)$\\
This amounts to showing 
\[
  \iota^\pwl_{\start_\stk'', \addrend_\stk}(\temp) \nsupeq[n-1] \iota^\pwl_{\start_\stk'', \addrend_\stk}(\temp)
\]
which is indeed the case.
...
Similar to case~\ref{f2:rwlx-exec-case}
    ...
    \item case $\npair{c_{\var{prp}}} \in \stdvr(W)$:\\
      ...
      \[
        ...
        (\halted,m'')
      \]
      (From $\heapSat[\ms']{n''}{W'}$ we know that that $\ms'$ contains memory segments $\hs_{f2}$, $\hs_\flag$, and $\ms_\stk'|_{[\start_\stk, \start_\stk''-1]}$ (our local part of the stack). The restoration code remains unaltered, and so does the data on the stack, so the execution continues by poping the $c_\stk'$ to $r_\stk$ followed by a pop of $c_{f2}'$ to $\pcreg$. Continuing in $f2$, the restore code is poped off the stack which is the last of the $\mathtt{scall}$ instruction. Now the top word of the stack is poped to $r_1$, and thanks to the world satisfaction, we know that this word is 1. The $\mathtt{assert}$ therefore succeeds making no changes to the memory and afterwards the execution halts.)

Use $\revokeTemp(W') \futurestr W'$. From the private-future memory satisfaction lemma (Lemma~\ref{lem:priv-mono-like}) get $\ms''$ and $\ms_f$ such that
...
\begin{align*}
  &\iota_{\start,\addrend}^\pwl : \Views \fun \Regions\\
  &\iota_{\start,\addrend}^\pwl \defeq \lambda v \ldotp
    \begin{cases}
      (v,1,=,H^\pwl_{\start,\addrend}) & v \neq \revoked \\
      \bot & \text{otherwise}
    \end{cases} \\
...
\begin{align*}
    ...
    & \quad \begin{aligned}[t]
              \{ \npair{(\start,\addrend)} \mid & \;\exists r \in \var{localityReg}(g,W) \ldotp \\
                        & \;\quad \exists [\start',\addrend'] \supseteq [\start,\addrend] \ldotp \\
                        & \;\qquad W(r)\nsubeq[n-1] \iota^\pwl_{\start',\addrend'}(W(r).v) \}
            \end{aligned}
\end{align*}
...
\begin{lemma}
  \begin{align*}
    ...
    & \quad \iota^\nwl_{\start,\addrend} (v) \nsubeq \iota^\pwl_{\start,\addrend}(v)
  \end{align*}
\end{lemma}
