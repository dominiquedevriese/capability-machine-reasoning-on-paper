...
\[
\forall \iota' \futurestr \iota_{\malloc,0} \ldotp \forall W,i \ldotp W(i)=\iota' \Rightarrow \iota'.H (\iota'.s) (W) = \iota'.H (\iota'.s ([i \mapsto W(i)]))
\]
and
\[
  \iota_{\malloc,0}.v = \perma
\]
...
\begin{description}
  \item [Set up activation record]
    ...
    \item Adjust a copy of the current pc to point to the return address and save it to the activation record.
...
The stack will reside entirely in memory. There is no separation between the memory and the stack, so when we talk about the stack it is as a conceptual thing.
...
\item[\texttt{scall} $f(\bar{r}_{\var{args}},\bar{r}_{\var{priv}})$] $\bar{r}_{\var{args}}$ and $\bar{r}_{\var{priv}}$ are lists of registers. This call assumes $r_{\var{stk}}$ contains a stack capability. An overview of this call:
  ...
  \item Clear the stack we release control over
  \item Clear unused registers
  \item Jump
...
\begin{description}
  ...
  \item [Push stack pointer]
    Push the full stack pointer to the stack.
  \item [Create protected return pointer]
...
\cline{2-2}
\end{tabular}
\caption{ Stack and register-register file after the clean up code has been run. The ``private'' words have been poped to their respective registers. The restore code has been poped of the stack. }
\end{figure}
...
\item $c_{\var{ar}}' = ((\entry,\local)b,e,b+2)$ where $b+2$ is the first address of the activation code.
\end{itemize}
Define
\begin{itemize}
\item $W = [0 \mapsto \iota_\malloc']
           [1 \mapsto \iota^\nwl_{\start_\adv,\addrend_\adv}(\perma)]
           [2 \mapsto \iota_{f1}]
...
\item $\npair{(\start_{\adv},\addrend_{\adv})} \in \readCond{}(\glob)(W)$
  \begin{itemize}
  \item Show: $\mapsto \iota^\nwl_{\start_\adv,\addrend_\adv}(\perma) \nsubeq[n-1] \iota^\pwl_{\start_\adv,\addrend_\adv}(\perma)$.
  \end{itemize}
\end{itemize}
Have
\begin{enumerate}[resume]
...
\item $\heapSat[\ms]{n'}{W}$
  \begin{enumproof}
    \item Split the memory into the disjoint unions of \ref{f1:m3-disj-union} and show:
      \begin{enumproof}
      \item case: $\npair[n']{\ms_\adv} \in \iota_\malloc'.H (\iota_\malloc'.s) (W)$
        \begin{enumproof}
          \item Use $\heapSat[\ms_\malloc]{n'}{[0 \mapsto \iota_\malloc']}$ with malloc specification context independence property.
        \end{enumproof}
      \item case: $\npair[n']{\ms_\adv} \in H^\nwl_{\start_\adv,\addrend_\adv} (\start_\adv,\addrend_\adv) W$
        \begin{enumproof}
        ...
        \begin{enumproof}
          ...
          \item $a = \start_\adv$: show $((\readonly,\glob),\link,\link+1,\link) \in \stdvr(W)$\\
           SFTS $\iota_{\link} \nsubeq[n'-1] \iota^\pwl_{\link,\link+1} (\perma)$\\
    ...
    \item case: $\npair[n']{c_{\var{ar}}'} \in \stdvr(W)$.
      \begin{enumproof}
        \item Let $n'' < n'$ and $W' \futurewk W$ be given and show $\npair[n'']{\updatePcPerm{c_{\var{ar}}'}} \in \stdvr(W')$\\
          ...
          Show: $\npair[n^{(3)}]{(\reg\update{\pcreg}{\updatePcPerm{c_{\var{ar}}'}},\ms)}$\\
