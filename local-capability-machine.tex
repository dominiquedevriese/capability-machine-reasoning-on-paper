...
\begin{align*}
  \sem{\plus{\refreg{r_1}}{\rv_1}{\rv_2}}               & =
                                                          \begin{cases}
                                                            ...
                                                            \failed & \text{otherwise}
                                                          \end{cases}\\
  \sem{\minus{\refreg{r_1}}{\rv_1}{\rv_2}}               & =
                                                          \begin{cases}
                                                            \stdUpdatePc{\updateReg{r_1}{n_1-n_2}} &
                                                            \arraycolsep=0pt
                                                            \begin{array}[t]{l}
...
\begin{lemma}[Determinacy]
  ...
  If $\Phi \step[n] \Phi'$ and $\Phi \step[n'] (\halted,\mem'')$, then $n \geq n'$
...
% Suggestion for malloc impl (r_t1 and r_t2 are registers in RegName_t), r_1 should not contain important stuff
% move r_1 n
% move r pc
% getb r_t1 r
% geta r_t2 r
...
% lea r r_t1
% move r_0 r_t1
% lea r_0 3 // 3 is offset to just after jmp
...
%Load the entry $f$ from the linking table to a free register and jump to it.
              ...
              \item[\texttt{crtcls $[(x_1,r_1),\dots(x_n,r_n)]$ $r_{\var{code}}$}] \forcenewline
                ...
                \item[Activation code:] \forcenewline
                  ...
                  \end{itemize}
                \end{description}
%Possible implementation of crtcls \overline{(x,r_v)} r_code where |\overline{(x,r_v)}| = n (i_1,...,i_6 under)
% malloc r_t1 n
% store r_t1 r_v1
% lea r_t1 1
% store r_t1 r_v2
    ...
    \begin{enumproof}
      ...
      \item $\ms_\link$: governed by region 2, only this memory segment is accepted. We also need to show that the contents is safe, i.e. shoe
        \begin{enumproof}
          ...
          \item $\npair[n-i]{c_\adv} \in \stdvr(W)$: Follows from Theorem~\ref{thm:ftlr} and Lemma~\ref{lem:nwlp-subset-pwl}.
        \end{enumproof}
      \item $\ms_\adv$:
    ...
    \item $\npair{\reg_0\update{r_\stk}{c_\stk}\update{r_1}{c_{g1}}} \in \stdrr(W)$ \label{pf:g1-4}\\
      ...
      \begin{enumproof}
        ...
        \item $\npair{c_{g1}} \in \stdvr(W)$ \\
          ...
          for some $i$. Where
          \begin{itemize}
          \item $\memSat[n_2-i]{\ms_2 \uplus \ms_\malloc'}{W_1\update{0}{\iota_\malloc}} $
          ...
          For world
          \[
            W_2 = W_1\update{0}{\iota_\malloc}\update{i_1}{\iota^\sta (\perm,\ms_\env \uplus \ms_\cls)}\update{i_2}{\iota_x}
          \]
