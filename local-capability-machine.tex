...
\[
  (\reg\update{\pcreg}{c_{f3}}\update{r_\stk}{c_\stk},m) \step[i] (\reg',\ms' \uplus [\start_\stk \mapsto 1] \uplus \ms_\stk|_{\start_\stk+1,\addrend_\stk} \uplus \ms_f),
\]
where
\[
  \lookingat{(\reg',\ms')}{\scall{r}{[]}{[]}}{c_{\var{next}}}
\]
where $c_{\var{next}}$ is $c_{f3}$ adjusted to point to the next instruction. Further we have \pointstostack{\reg'}{[\start_\stk \mapsto 1]}{\ms_\stk |_{\start_\stk+1,\addrend_\stk}} and $i$ is a suitable number of steps.
...
\begin{enumproof}
  ...
  \item Hyp-Cont\\
    ...
    \item $(\reg'',\ms'' \uplus [\start_\stk \mapsto 1] \uplus \ms_{\var{unused}}'') \in \observations(\revokeTemp{W''})$\\
      As $W'' \futurestr W$, we know that the program, assertion flag, and linking table remain unchanged in $\ms''$. Given some frame $\ms_f'$, then the execution proceeds by first suceeding the assertion and then pushing 2 to the stack:
      \[
      ...
      \item $\npair[n''-k]{(\reg^{(3)},\ms'' \uplus [\start_\stk \mapsto 2] \uplus \ms_{\var{unused}}'')} \in \observations(\revokeTemp{W''})$\\
        Show this using Lemma~\ref{lem:scall-works}. So show:
        \begin{enumproof}
