...
\begin{lemma}[$\mathtt{scall}$ works]
  ...
  \item[Hyp-Cont] If
    ...
    \item $\memSat[n']{\ms''}{W''}$ 
    \end{itemize}
    then we have that $(n', (\reg',\ms'' \uplus \ms_{\var{unused}}'')) \in \observations(W'')$
  \end{itemize}
  Then 
  \begin{itemize}
...
\begin{lemma}[Execution and $\observations$]
  \begin{align*}
    & \forall n,i,\reg, \reg', \ms, \ms', W \ldotp\\
    & \quad (\forall \ms_f \ldotp (\reg,\ms \uplus \ms_f) \step[i] (\reg',\ms' \uplus \ms_f) \land \npair[n-i]{(\reg',\ms')} \in \observations(W)) \\
