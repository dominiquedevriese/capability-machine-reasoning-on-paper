...
\begin{definition}
  We say that $(\ms,\reg) \text{ is looking at } [i_0,\cdots,i_n]$ if
  \begin{itemize}
...
\begin{definition}
  $(\ms,\reg) \text{ has stack $\ms_\stk$ with $\ms_{\mathit{unused}}$ unused}$
...
\begin{lemma}[$\mathtt{scall}$ works]
  ...
  \begin{itemize}
  \item $\heapSat[\ms]{n}{W}$ 
  \item $(\ms,\reg) \text{ is looking at } \mathtt{scall}(r(\overline{r_{\mathit{arg}}},
    \overline{r_{\mathit{priv}}}))$
  \item $(\ms,\reg) \text{ has stack $\ms_\stk$ with $\ms_{\mathit{unused}}$ unused}$
  \item $(\ms\uplus \ms_{\mathit{frame}},\reg) \rightarrow^j (\halted,
    \ms')$
  \item[Hyp-Callee] If
    \begin{itemize}
    ...
    \item $\memSat[n-1]{\ms''}{W'}$
    \item $(\ms'',\reg') \text{ has stack $\emptyset$ with
        $\ms_{\mathit{unused}}'$ unused}$,
    \item $\reg'= \reg_0[\pcreg\mapsto\updatePcPerm{\reg(r)},
      \overline{r_{\mathit{arg}}} \mapsto \reg(\overline{r_{\mathit{arg}}}),r_0
      \mapsto c_{\mathit{ret}}]$
    \item $(n,c_{\mathit{ret}}) \in \stdvr(W')$
    \end{itemize}
    then we have that $(\reg',\ms'') \in \observations(W')$
  \item[Hyp-Cont] ...
  \end{itemize}
  Then $\exists W' \futurestr W\ldotp$
  \begin{itemize}
  \item $\ms' = \ms'' \uplus \ms_{\mathit{frame}}\uplus \ms_{\mathit{rev}}$
  \item $\ms'' :_{n-j} W'$
  \end{itemize}
