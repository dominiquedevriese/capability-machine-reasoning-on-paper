...
1g:  jmp $r_0$
f4:  reqglob $r1$
     reqperm $r_\stk$ $\encodePerm(\rwlx)$
     store $x$ 0
...
4f:  jmp $r_0$
\end{lstlisting}
\lau{todo: when the stack is received the first time, make sure it points to where we would expect it to point (1 - range of auth.) Make macro for this.}
