...
\begin{lemma}
  \label{lem:cap-in-vr-revoketemp-global}
  If
  \begin{itemize}
  \item $\perm \neq \noperm$
  ...
  \]
\end{lemma}
\begin{proof}[Proof of Lemma~\ref{lem:cap-in-vr-revoketemp-global}]
  Assume for contradiction $\gl = \local$. Procced by case on $\perm$:
  \begin{enumproof}
    \item $\perm = \noperm$\\
      By assumption $\perm \neq \noperm$.
    \item $\perm \in \{ \readonly, \readwrite, \exec, \rwx, \rwl, \rwlx \}$ \\
      Here 
      \[
        \npair{(\start,\addrend)} \in \readCond{}(local)(\revokeTemp{W})
      \]
      must hold, but there are no $\temp$ regions and 
      \[
        r \in \localityReg(\local,\revokeTemp)
      \]
      must be the region name of a $\temp$ region, so no such $r$ exists. (contradiction)
    \item $\perm = \entry$ \\
      % TODO!!! I am not sure it is possible to show for this case.
    \end{enumproof}
\end{proof}
  ...
  \item Case $\var{conf'} = \updateReg[\Phi']{\pcreg}{\var{newPc}}$, and additionally, one of the following
    ...
    \item For all $r \in \RegName$, $\npair[n-1]{\memreg[\Phi'](r)}\in\stdvr(W)$.
      ...
      \item $\memreg[\Phi'](r) = \var{w}$ and $\memreg(r_2) =
        ...
        $\heapSat[\memheap]{n}{W}$ and $\npair{\memreg(r_2)}\in\stdvr(W)$ which
