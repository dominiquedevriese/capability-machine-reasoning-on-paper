...
and
\begin{itemize}
\item The first address of $\hs_{f1}$ contains a global read-only capability for $\hs_\link$, the second address of $\hs_{f1}$ contains a global read-write capability to $\hs_\flag$, the rest of $\hs_{f1}$ contains the code of $f1$. \dominique{why not write this in symbols?}
...
\iota^{\nwl,p}_{\start_\adv,\addrend_\adv} \nsubsim[n-1] \iota^\pwl_{\start_\adv,\addrend_\adv}
\]
which is indeed the case\dominique{by some lemma?}. Use this to apply the FTLR to get 
\begin{enumproof}
...
\item $\heapSat[\ms]{n}{W}$ \label{lem:f2-mem-sat}\lau{The argument here is a bit informal}
  ...
  \item $\ms_\stk'$, here we need to split the memory segment in two and for $n' < n$ argue
    ...
    \item case $\npair[n']{\ms_\stk'|_{[\start_\stk, \start_\stk''-1]}} \in
      H^\sta(\ms_\stk'|_{[\start_\stk,\start_\stk''-1]})$ \dominique{apply to world?}:\\ This region matches only this memory segment.
...
Similar to case~\ref{f2:rwlx-exec-case}
    ...
    \item case $\npair{c_{\var{prp}}} \in \stdvr(W)$: \label{f2:c-prp}\\
      ...
      \]
      \dominique{part of a phrase missing here?}
...
\begin{lemma}[Correctness lemma for \texttt{f3}]
  ...
  and
  \begin{itemize}
  \item The first address of $\hs_{f3}$ contains a global read-only capability for $\hs_\link$, the second address of $\hs_{f3}$ contains a global read-write capability to $\hs_\flag$, the rest of $\hs_{f3}$ contains the code of $f3$.
  \item $\ms_\flag = [flag \mapsto 0]$
  \item $\ms_{\var{link}} = [\var{link} \mapsto c_\malloc, \var{link} + 1 \mapsto c_\adv]$
...
This proof goes like the proof of Lemma~\ref{lem:correctness-f3} except for item \ref{f2:c-prp}
\begin{enumproof}[start=3]
  ...
  \item $\npair{c_{\var{prp}}} \in \stdvr(W)$ \\
    ...
    \begin{enumproof}
      \item $n'' \leq n'$
      \item $\npair[n'']{\reg} \in \stdrr(W')$ \label{f3:mem-sat-mid}
      \item $\heapSat[\ms]{n''}{W'}$
    \end{enumproof}
    be given and show
    \[
      \npair[n'']{(\reg\update{\pcreg}{\updatePcPerm(c_\prp)},\ms)}
    \]
    ...
    now show
    \[
      \exists \ms_R,\ms', W'' \futurestr W' \ldotp m'' = \ms' \uplus \ms_R \uplus \ms_f \land \heapSat[\ms']{n''-i}{W''}
    \]
    At this point use Lemma~\ref{lem:priv-mono-like2} with \ref{f3:mem-sat-mid} to get $\ms_p$, $\ms_r$, and $R$ that such that $\ms = \ms_p \uplus \ms_r$ and $\heapSat[\ms_p]{n''}{W'}$. We can further conclude that $\ms |_{[\start_\stk,\addrend_\stk]} \subseteq \ms_r$ \lau{The needs to be spelled out - preferably in an easy to use lemma. The idea is that $R$ partitions $\ms_r$ into memory segments that each are accepted by a temporary region. The stack is governed by a temporary region, so it must be in $\ms_r$.}.

 Now consider the first steps of execution \ref{f3:mid-exec}. From \ref{f3:mem-sat-mid} we have that the private part of the stack and $f3$ is unchanged, so the recovery code gets to run which restores the stack capability and program counter, and pops the return code off the stack (there are no ``private'' registers). Next, the top word is popped off the stack. Again due to memory satisfaction, we know that this word is the integer 1 and therefore the assertion succeeds. Execution continues by pushing 2 to the stack. Now due the $\mathtt{scall}$ up to the point just after the jump to $\adv$. Figure~\ref{fig:f3-stack-before-snd-adv} shows the stack and register file at this point. The $\ms_p$ part of $\ms$ was not changed. For convenience, name the stack part of memory at this point $\ms'_\stk$.

Let $W^{(3)} = \revokeTemp{W''}\update{i_1}{\iota^\sta(\temp,\ms|_{[\start_\stk, \start_\stk''-1]})}\update{i_2}{\iota^\sta(\temp,\ms|_{[\start_\stk'',\addrend_\stk]})}$
...
\begin{align*}
  ...
  \text{ iff }
  \left\{\begin{aligned}
    &\exists R : \activeReg{W} \rightarrow \HeapSegments \ldotp \\
    &\quad \hs = \biguplus_{r\in\activeReg{W}}R(r) \land \\
...
\label{lem:priv-mono-like2}
  \begin{align*}
    & \forall \ms, n, W \ldotp \heapSat[\ms]{n}{W} \Rightarrow \\
    & \quad \exists \ms', \ms_r, R : \dom(\erase{W}{\temp}) \fun \MemSegments \ldotp \\
    & \qquad \ms = \ms' \uplus \ms_r \land \\
    & \qquad \heapSat[\ms']{n}{\wrev{W}} \land \\
    & \qquad \forall r \ldotp \forall n' < n \ldotp \npair[n']{R(r)} \in W(r).\mathrm{H}(W(r).\mathrm(s))(W)\\
    & \qquad \ms_r = \biguplus_{r \in \dom(R)} R(r)
  \end{align*}
\end{lemma}
...
\label{lem:disj-mem-sat}
  \begin{align*}
    ...
    & \quad \heapSat[\ms \uplus \ms' ]{n}{W \uplus W'}
  \end{align*}
\end{lemma}
