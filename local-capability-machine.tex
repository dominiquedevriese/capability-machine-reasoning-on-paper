...
\item[\texttt{malloc x}] Calls malloc to allocates a piece of memory. In the program $\texttt{x}$ will henceforth be used to refer to the capability returned by malloc.
\item[\texttt{hstore x n}] Fetches the capability refered to with \texttt{x} and stores \texttt{n} to it.
