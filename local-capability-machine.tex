...
\begin{lemma}[$\mathtt{scall}$ works]
  ...
  \begin{itemize}
  \item $\heapSat[\ms]{n}{W}$ 
  \item $(\reg,\ms) \text{ is looking at } \mathtt{scall}(r(\overline{r_{\mathit{arg}}},
    \overline{r_{\mathit{priv}}}))$
  \item $(\reg,\ms) \text{ has stack $\ms_\stk$ with $\ms_{\mathit{unused}}$ unused}$
  \item[Hyp-Callee] If
    \begin{itemize}
    ...
    \item $\memSat[n-1]{\ms''}{W'}$
    \item $(\reg',\ms'') \text{ has stack $\emptyset$ with
