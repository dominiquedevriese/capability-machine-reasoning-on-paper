...
\subsubsection{Downward closedness lemmas}
\label{sec:downward-closedness}
...
\begin{lemma}[$\stdvr{\cdot}$ downwards-closed]
  \label{lem:stdvr-dc}
  \begin{itemize}
  \item $n' \leq n$
  \item $\npair{\word} \in \stdvr(W)$
  \end{itemize}
  then 
  \[
   \npair[n']{\word} \in \stdvr(W)
  \]
\end{lemma}
...
\begin{lemma}[$\stdrr{\cdot}$ downwards-closed]
  \label{lem:stdrr-dc}
  \begin{itemize}
  \item $n' \leq n$
  \item $\npair{\reg} \in \stdrr(W)$
  ...
  \]
\end{lemma}
\begin{proof}[Proof of Lemma~\ref{lem:stdrr-dc}]
  % TODO
\end{proof}
...
\begin{lemma}[Fundamental theorem proof helper lemma]
  ...
  \item $\heapSat[\hs]{n}{W}$
  \item $i \leq n$
  \item $\Phi = (\reg\update{\pcreg}{((\perm,\gl),\start,\addrend,\addr)},\hs \uplus \ms_f)$
  \item $\Phi \step[i] (\halted,\Phi')$
  \end{enumproof}
...
\begin{proof}
  ...
  \item Case $\var{conf'} = (\halted,\Phi)$:
    ...
    \end{enumproof}
  \item Case $\var{conf'} = \Phi'$, and additionally, one of the following
    holds:
    \begin{itemize}
    ...
    \begin{itemize}
    \item $\memreg[\Phi'](\pcreg) = \updatePcPerm{\memreg(\lv)}$
    \item $\memreg[\Phi'](\pcreg) = \var{newPc}$ and $\var{newPc} =
      ((\perm,\gl),\start,\addrend,\addr + 1)$
      \dominique{This is wrong: the semantics allow, for example, a move into
        the PC, after which the new pc is increased by one}
    \end{itemize}
    and finally, for all $r \in (\RegName \setminus \{\pcreg\})$, one of the following
    holds:
    \begin{itemize}
    ...
    \item $\heapSat[{\memheap[\Phi']}]{n-1}{W}$.
      ...
      \item Case $\memheap[\Phi'] = \memheap[\Phi]\update{\addr'}{\var{w}}$,
        ...
        $\npair{\memreg(r_1)}\in\stdvr(W)$ and
        $\npair{\memreg(r_2)}\in\stdvr(W)$ which follows from
        $\npair{\memreg}\in\stdrr(W)$ by definition.
      \end{enumproof}
    \item $\npair[n-1]{\memreg[\Phi']}\in\stdrr(W)$.
      \begin{enumproof}
      \item Assume: $r \in \RegName\setminus\{\pcreg\}$.\\
        Suffices: $\npair[n-1]{\memreg[\Phi'](r)} \in \stdvr(W)$.\\
        By: definition of $\stdrr{\cdot}$
      \item Case $\memreg[\Phi'](r) = \memreg[\Phi](r)$:
        $\npair[n-1]{\memreg[\Phi'](r)} \in \stdvr(W)$ follows from
        $\npair{\memreg}\in\stdrr(W)$ by definition of $\stdrr(\cdot)$ using
        Lemma~\ref{lem:stdvr-dc}.
      \item $\memreg[\Phi'](r) = n$ for some $n \in \ints$.
      ...
      \item $\memreg[\Phi'](r) = \var{w}$ and $\memreg(r_2) =
        ...
        Lemmas~\ref{lem:conds-load-suffice} and~\ref{lem:stdvr-dc} using the
        fact that $\heapSat[\memheap]{n}{W}$ and $\npair{\memreg(r_2)}\in\stdvr(W)$
        which follows from $\npair{\memreg}\in\stdrr(W)$ by definition.
      \item $\memreg[\Phi'](r) = \var{c}$ and $\memreg(r_1) =
        ((\perm',\gl'),\start',\addrend',\addr')$ and $\perm' \neq \entry$ and
        $\var{c} = ((\perm',\gl'),\start',\addrend',\addr' + n)$ for some $n \in
      ...
      \item $\memreg[\Phi'](r) = \var{c}$ and $\memreg(r) =
        ...
        definition\dominique{this does not follow if $r = \pcreg$, similar
      ...
      \item $\memreg[\Phi'](r) = \var{c}$ and $\memreg(r) =
        ...
        which follows from $\npair{\memreg}\in\stdrr(W)$ by definition.
      \end{enumproof}
    \item $\npair[n-1]{\memreg[\Phi'](\pcreg)} \in \stder(W)$: TODO
    \end{enumproof}
  \end{enumproof}
\end{proof}
...
\begin{proof}[Proof of Lemma~\ref{lem:stdvr-glob-priv-mono}]
  ...
  \[
    \npair{\stdcap[(\perm,\glob)]} \in \stdvr(W')
  \]
  to this end consider all the possible cases of $\perm$:
  \begin{enumproof}
    \item $\perm = \noperm$\\
      Trivial
    \item $\perm = \readonly$ \\
