...
\begin{mathpar}
  ...
  \and
  \inferrule{ }
  { \revoked \futurestr (\temp,s,\phi_\pub,\phi,H) }
  \and
...
\begin{align*}
  ...
  \observations (W) \defeq & \lambda W \ldotp 
                             ...
                             \begin{aligned}[t]
                               ...
                               & \quad(\reg,\hs \uplus \ms_f) \step[i] (\halted,\Phi)  \\
                               & \qquad \Rightarrow
                               \begin{aligned}[t]
                                 & \exists W' \futurestr W \ldotp\exists \hs_r, \hs' \ldotp\\
                                 & \quad \memheap[\Phi] = \hs' \uplus \hs_r \uplus \ms_f \land \\ 
...
\begin{proof}[Proof of Lemma~\ref{lem:anti-red-obs}]
  ...
  \begin{equation}
    ...
    (\reg,\ms \uplus \ms_r \uplus \ms_{\var{frame}}) \step[i] 
  \end{equation}
...
\begin{lemma}
  ...
  \[
    \npair{(\start,\addrend)} \in \writeCond{}(\iota^\pwl,\local)(W) \Rightarrow \npair{(\start,\addrend)} \in \writeCond{}(\iota^\nwl,\local)(W)
  \]
\end{lemma}
...
\begin{lemma}[$\mathit{readCondition}$ monotone w.r.t $\futurewk$]
  ...
  \[
    \forall W' \futurewk W \ldotp \npair{\start,\addrend} \in \readCond{}(\gl)(W) \Rightarrow \npair{\start,\addrend} \in \readCond{}(\gl)(W')
  \]
\end{lemma}
...
\begin{lemma}[$\mathit{readCondition}$ downwards-closed]
  ...
  \[
    \forall n' \leq n \ldotp \npair[n]{\start,\addrend} \in \readCond{}(\gl)(W) \Rightarrow \npair[n']{\start,\addrend} \in \readCond{}(\gl)(W)
  \]
\end{lemma}
...
\begin{lemma}[$\mathit{writeCondition}$ monotone w.r.t $\futurewk$]
  ...
  \[
    \forall W' \futurewk W \ldotp \forall \iota \in \{\iota^\pwl,\iota^\nwl,\iota^{(\nwl,p)}\} \npair{\start,\addrend} \in \writeCond{}(\iota,\gl)(W) \Rightarrow \npair{\start,\addrend} \in \writeCond{}(\iota,\gl)(W')
  \]
\end{lemma}
...
\begin{lemma}[Fundamental theorem proof helper lemma]
...
\begin{proof}
  ...
  \item By induction on $n$. In other words, assume that the theorem already
    holds for all $n' < n$.
  \item The case that $i = 0$ is impossible, since
    $(\reg\update{\pcreg}{((\perm,\gl),\start,\addrend,\addr)},\hs \uplus
    \ms_f)\neq (\halted,\Phi)$ for any $\Phi$. Assume w.l.o.g. that $i = 1+i'$
  \item 
  \end{enumproof}
\end{proof}
...
\begin{proof}
  ...
  \item This follows by Lemma~\ref{lem:ftlr-induction-helper} using
    Lemmas~\ref{lem:readcond-dc} and~\ref{lem:writecond-dc}.
  \end{enumproof}
\end{proof}
...
\begin{lemma}[$\mathtt{scall}$ works]
  ...
  \item[Hyp-Cont] If
    \begin{itemize}
    \item $n' \leq n-2$
    \item $W'' \futurewk \revokeTemp{W}[\iota^{\sta}(\temp,\ms_\stk\uplus \ms_\act \uplus \ms_f),\iota^\sta(\temp,\ms_{\mathit{unused}}'')]$
    \item $\memSat[n']{\ms''}{\revokeTemp{W''}}$ 
    \item for all $r$, we have that:
      \begin{equation*}
        \reg'(r)
...
\end{lemma}
\begin{proof}
  Assume $n$ is sufficiently large to execute all the steps up to and including the jump of $\scall{r}{\overline{r_{\var{arg}}}}{\overline{r_{\var{priv}}}}$. If this is not the case, then given any frame the execution will not halt successfully fast enough.
  ...
  \[
    (\reg,\ms \uplus \ms_f \uplus \ms_\stk \uplus \ms_{\var{unused}} \uplus \ms_f) \step[i] (\reg_1,\ms \uplus \ms_f \uplus \ms_\stk \uplus \ms_{\var{act}} \uplus \ms_\unused' \uplus \ms_{\var{frame}})
  \]
  where
  \begin{enumproof}[resume]
  \item $i \leq n$
  ...
  \item $\reg'(r_0) = c_{\var{ret}} = ((\entry,\local),\_,\_,\_)$ where the range of authority is the same as $\reg(r_\stk)$ and it points to the first instruction of the activation code.
  \item \pointstostack{\reg'}{\emptyset}{\ms_\unused'}
  \item $\reg'(\pcreg) = \updatePcPerm{\reg(\pcreg)}$ 
  \item $\reg'(r) = \reg(r)$
  \item $\reg'(\overline{r_{\var{args}}}) = \reg(\overline{r_{\var{args}}})$
  \item $\forall r' \in \RegName \setminus \{\pcreg, r_\stk, r, \overline{r_{\var{args}}} \}$
  \end{enumproof}
  ...
  \]
  to this end use Hyp-Callee (\ref{pf:scall-works:Hyp-Callee}). To use this everything is satisfied directly by assumptions but the following:
  \begin{enumproof}[resume]
    ...
    \item $\npair[n-1]{\reg'(r_\stk)} \in \stdvr(W_1)$ \\
      Say $\dom(\reg'(r_\stk))=[\start_\stk,\addrend_\stk$. This amounts to showing
      \begin{enumproof}
      \item $\npair[n-1]{(\start_\stk, \addrend_\stk)} \in \readCond{}(\local)(W)$ \label{pf:scall-works:readcond-stack}\\
        This amounts to showing 
        \[
          \iota^\pwl_{\start_\stk, \addrend_\stk} \nsubsim[n-1] \iota^\pwl_{\start_\stk, \addrend_\stk}
        \]
        which is indeed the case.
      \item $\npair[n-1]{(\start_\stk, \addrend_\stk)} \in \writeCond{}(\iota^\pwl,\local)(W)$ \label{pf:scall-works:writecond-stack}\\
        This amounts to showing 
        \[
          \iota^\pwl_{\start_\stk, \addrend_\stk} \nsupsim[n-1] \iota^\pwl_{\start_\stk, \addrend_\stk}
        \]
        which is indeed the case.
      \item $\npair[n-1]{(\rwlx,\start_\stk, \addrend_\stk)} \in \execCond{}(\local)(W)$\\
        Using \ref{pf:scall-works:writecond-stack} and \ref{pf:scall-works:readcond-stack}, we can use Lemma~\ref{lem:stack-exec-cond-help}.
      \item $\npair[n-1]{(\rwx,\start_\stk, \addrend_\stk)} \in \execCond{}(\local)(W)$\\
        Using \ref{pf:scall-works:writecond-stack} and \ref{pf:scall-works:readcond-stack}, we can use Lemma~\ref{lem:stack-exec-cond-help}.
      \item $\npair[n-1]{(\exec,\start_\stk, \addrend_\stk)} \in \execCond{}(\local)(W)$\\
        Using \ref{pf:scall-works:writecond-stack} and \ref{pf:scall-works:readcond-stack}, we can use Lemma~\ref{lem:stack-exec-cond-help}.
      \end{enumproof}
...
\begin{enumproof}[resume]
  ...
  \item $\perm = \exec$\\
    ...
    \begin{enumproof}
      \item $\npair[n']{(\start,\addrend) \readCond{}(\local)(W')}$ \\
        Follows from Lemma~\ref{lem:readcond-mono-pub}, Lemma~\ref{lem:readcond-dc}, and assumption~\ref{pf:stack-exec:ass:readcond}.
      \item $\npair[n']{(\start,\addrend) \writeCond{}(\iota^\nwl,\local)(W')}$\\
