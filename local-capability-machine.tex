...
Define the following macros: $\mathtt{restrict}$, $\mathtt{subseg}$, and $\mathtt{lea}$ that does not overwrite the source register. A $\mathtt{store}$ that allows integers to be stored directly.
\begin{align*}
...
\begin{proof}[Proof of lemma~\ref{lem:correctness-f2}]
  ...
  \begin{align*}
    ...
    &\quad(\reg_0\update{\pcreg}{\updatePcPerm{c_\adv}}\update{r_0}{c_{\var{prp}}}\update{r_1}{c_\adv}\update{r_\stk}{c_\stk''},m'')
  \end{align*}
Where 
\[
  m''  \defeq \hs_{f2} \uplus 
...
W = \;& [0 \mapsto \iota_{\malloc,0}] \\
    ...
    & [6 \mapsto \iota^\pwl_{\start_\stk'' + 1, \addrend_\stk} (\temp)]
  \end{align*}
...
\begin{align*}
  ...
  &\iota_{\start,\addrend}^{\var{pwl}} \defeq \lambda \_ \ldotp(\temp,1,=,H^{\var{pwl}}) \\ \\
