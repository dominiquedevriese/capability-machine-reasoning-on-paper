  ...
  \begin{enumproof}[resume]
    ...
    \item $\npair{\reg_0\update{r_\stk}{c_\stk}\update{r_1}{c_{g1}}} \in \stdrr(W)$\\
            ...
            \item $\npair[n_2-i]{\reg_2} \in \stdrr(W_2)$ \\
              ...
              \begin{enumproof}
                ...
                \item $\npair[n_2-i]{c_\cls} \in \stdvr(W_2)$ \\
                  ...
                  Use Lemma~\ref{lem:anti-red-obs}, let $\ms_f''$ be given and take $\ms_r$ to be the part of $\ms_3^t$ that $\reg_3(r_\stk)$ does not govern. By the operational semantics, we know\footnote{the execution may fail, but then the confifuration is trivially in the observation relation.}
                  \[
                  ...
                  \begin{enumproof}
                    ...
                    \item Hyp-Cont\\
                      ...
                      \item $W_6 \futurewk W_5$ (as defined in the Hyp-Callee case).
                      \item $\memSat[n_4]{\ms_6}{\revokeTemp{W_6}}$
                      ...
                      \[
                        (\reg_6,\ms_6 \uplus \ms_\unused''' \uplus \ms_f''') \step[k] (\reg_7,\ms_6\update{x}{1} \uplus \ms_\unused'' \uplus \ms_f''')
                      \]
                      where
                      \begin{itemize}
                      ...
                      \[
                        \memSat[n_4-k]{\ms_6\update{x}{1}}{W_7}
                      \]
      ...
      \item \label{item:proof-cont} At this point, we apply the \texttt{scall} lemma, to get
        \[
          \npair[n_4-k]{(\reg_7,\ms_6\update{x}{1} \uplus \ms_\unused'')} \in \observations(W_7)
        \]
        show
        \begin{enumproof}
          \item $\memSat[n_4-k]{\ms_6\update{x}{1}}{\revokeTemp{W_7}}$, follows from $W_7= \revokeTemp{W_7}$.
          \item Hyp-Callee
            % Use that it is the same $r_1$
            % TODO
          \item Hyp-Cont
            % From c_stk in val rel of W_3 get that W_3 must have iota pwl region.
            % 
        \end{enumproof}
  \end{enumproof}
...
\begin{align*}
  \Regions = & \{\revoked\} \times \States \times \Rels \times (\States \fun (\Wor \monwknefun \UPred{\HeapSegments})) \uplus \\
...
\begin{mathpar}
  ...
  \and
  \inferrule{ }
  { (\revoked,s,\phi_\pub,\phi,H) \futurestr (\revoked,s,\phi_\pub,\phi,H) }
  \and
  \inferrule{ (s,s') \in \phi }
  { (\revoked,s',\phi_\pub,\phi,H) \futurestr (\temp,s,\phi_\pub,\phi,H) }
  \and
  \inferrule{ (s,s') \in \phi }
  { (\temp,s',\phi_\pub,\phi,H) \futurestr (\revoked,s,\phi_\pub,\phi,H) }
\end{mathpar}
...
\begin{mathpar}
  ...
  \and
  \inferrule{ }
  { (\revoked,s,\phi_\pub,\phi,H) \futurewk (\revoked,s,\phi_\pub,\phi,H) }
\end{mathpar}
...
\begin{align*}
  ...
  \revokeTemp{W} & \defeq \lambda r \ldotp 
                   \begin{cases}
                     (\revoked,s,\phi_\pub,\phi,H)            & \text{if }W(r) = (\temp,s,\phi_\pub,\phi,H) \\
                     W(r)                & \text{otherwise}
                   \end{cases}
\end{align*}
...
\begin{lemma}[$\mathtt{scall}$ works]
  ...
  \begin{itemize}
  \item $\memSat[n]{\ms}{\revokeTemp{W}}$ 
  \item $\dom(\ms_f) \cup (\dom(\ms_\stk \uplus \ms_{\mathit{unused}} \uplus \ms)) = \emptyset$
    \dominique{you mean $\cap$, right?}
  ...
  \item[Hyp-Callee] If
    ...
    \item $W' =
      \revokeTemp{W}[\iota^{\sta}(\temp,\ms_\stk\uplus\ms_{\mathit{act}}),\iota^{\pwl}(\dom(\ms_{\mathit{unused}}')),\iota^\sta (\temp,\ms_f)]$,
    \item $\memSat[n-1]{\ms''}{W'}$
    \item $\reg' \text{ points to stack with $\emptyset$ used and $\ms_{\mathit{unused}'}$ unused}$
    \item $\forall a \in \dom(\ms_{\var{unused}}') \ldotp \ms''(a) = 0$
  ...
  \item[Hyp-Cont] If
    \begin{itemize}
    \item $n' \leq n-2$
    \item $W'' \futurewk \revokeTemp{W}[\iota^{\sta}(\temp,\ms_\stk)][\iota^\sta(\temp,\ms_{\mathit{unused}}'')][\iota^\sta(\temp,\ms_f)]$
    \item $\memSat[n']{\ms''}{\revokeTemp{W''}}$ 
    \item for all $r$, we have that:
      \begin{equation*}
        \reg'(r)
  ...
  Then 
  \begin{itemize}
    \item $(\reg,\ms \uplus \ms_f \uplus \ms_\stk \uplus \ms_{\var{unused}}) \in \observations(W)$
  \end{itemize}
