...
\begin{definition}[Reasonable configuration]
  ...
  \item \emph{Guarantee stack base address before call} If
    ...
    \item $r_1 \neq \rtmp{1}$
    \item $\Phi'(\pcreg) + \calllen$ behaves reasonably up to $n'-i - 1$ steps
    \end{itemize}
...
\begin{definition}
  ...
  A configuration {\sourcecolor $\Phi = (\ms,\reg,\_,\_)$} is said to point to instruction $\scall{\offpc,\offsigma}{r_1}{r_2}$ in $A$ iff
  \[
