...
\begin{mathpar}
  ...
  \inference{
    ...
    \exists d_\sigma : \dom(\mscode) \rightarrow \powerset{\Seal}^3 \ldotp
    \sigrets = \biguplus_{a \in \dom(\mscode)} \phi_2(d_\sigma(a)) \tand\\
    \sigrets = \bigcup_{a \in \dom(\mscode)} \phi_1(d_\sigma(a)) \tand\\
    \sigcloss = \bigcup_{a \in \dom(\mscode)} \phi_3(d_\sigma(a)) \tand\\
    \forall a \in \dom(\mscode) \ldotp 
    \pi_1(d_\sigma(a)),\pi_2(d_\sigma(a)),\pi_3(d_\sigma(a)) \vdash_{\mathrm{comp-code}} \mscode,a
  }{
...
\begin{lemma}[purePart respects $\oplus$]
  ...
  Also, the latter is defined when the former is defined (but not necessarily vice versa).
\end{lemma}
\begin{proof}
  Follows from the definition of $\purePart{}$ and $\oplus$.
\end{proof}
...
\begin{lemma}
  \label{lem:comp-world-pure-comp-world}
  If $W_1 \oplus W_2$ is defined, then $\purePart{W_1} \oplus \purePart{W_2}$ is defined
\end{lemma}
\begin{proof}
  Follows from Lemma~\ref{lem:purePart-oplus}.
\end{proof}
...
\begin{equation*}
  H^\mathrm{code} \; \sigrets \; \sigcloss \; \code \; (\ta,\_,\gsigrets,\gsigcloss) \; \hat{W} = \left\{\npair{(\code,\code)} \middle|
    \begin{array}{l}
    \dom(\code) = [\aaddr,\baddr] \wedge \\
      (\dom(\code) \subseteq \ta \wedge \sigrets \subseteq \gsigrets \trust = \trusted) \vee \\
...
\dominique{The below is an experimental reformulation of the reasonability requirement for the FTLR and full abstraction formulation.  All remarks welcome..
  The main change is that it now says that after a call, the post-return code is also expected to behave reasonably under reasonable assumptions. 
}
...
\item If $w = ((\perm,\_),\baddr,\eaddr,\_)$ and $\perm \neq 0$ and $\gsigrets \neq \emptyset$, then $[\baddr,\eaddr] \# \dom(\ta)$
\item If $w = \sealed{\sigma,\vsc}$ and $\sigma \not\in (\gsigrets\cup \gsigcloss)$ then $\vsc$ is reasonable up to $n - 1$ steps.
\item If $w = ((\perm,\_),\baddr,\eaddr,\_)$ and $\perm \in \readAllowed{}$ and $n > 0$, then $\ms(\aaddr)$ is reasonable up to $n - 1$ steps for all $\aaddr \in ([\baddr,\eaddr] \setminus \ta)$
...
We say that an execution configuration $\Phi$ is reasonable up to $n$ steps if:
  \begin{itemize}
  \item \emph{Guarantee stack base address before call} If
    \begin{itemize}
    \item $\Phi \nstep[i]{\ta,\stkb} \Phi'$
    \item $\Phi'$ points to $\src{\scall{\offpc,\offsigma}{r_1}{r_2}}$ in $\ta$ for some $\src{r_1}$ and $\src{r_2}$
    \item $0 < i \le n$
    \end{itemize}
    Then all of the following hold:
    \begin{itemize}
    \item $\src{\Phi'}(\src{r_\stk}) = \src{\stkptr{\_,\stkb,\eaddr_\stk,\aaddr_\stk}}$
    \item $r_1 \neq \rtmp{1}$
    \item $\Phi'(\pcreg) + \calllen$ and $\Phi'(\rstk)$ behaves reasonably up to $n-i$ steps
    \end{itemize}
  \item \emph{Use return seals only for calls} If
    \begin{itemize}
    \item $\Phi \nstep[i]{\ta,\stkb} \Phi'$
  ...
  \item \emph{Don't store private stuff...} If
    ...
    \item $0 < i \le n$
    \end{itemize}
    Then $\Phi.\reg(r_2)$ is reasonable in memory $\Phi'.\mem$ up to $n$ steps
  \item \emph{Don't leak private stuff...} If
    ...
    \item $0 < i \le n$
    \end{itemize}
    Then $\Phi.\reg(r)$ is reasonable in memory $\Phi'.\mem$ up to $n$ steps for all $r$ (including $\pcreg$)
  \end{itemize}
...
We say that an executable capability $c = ((\perm,\normal),\baddr,\eaddr,\aaddr)$ behaves reasonably up to $n$ steps if for any $\Phi$ such that
  \begin{itemize}
  \item $\Phi.\reg(\pcreg) = c$
  \item $\Phi.\reg(r)$ is reasonable up to $n$ steps in memory $\Phi.\mem$ and free stack $\Phi.\ms_\stk$for all $r \not\in \{\pcreg\}$
  \item $\Phi.\mem$, $\Phi.\ms_\stk$ and $\Phi.\stk$ are all disjoint
  \end{itemize}
  We have that $\Phi$ is reasonable.

We say that a component $(\mscode,\msdata,\overline{c_{\mathrm{import}}},\overline{c_{\mathrm{export}}},\sigrets,\sigcloss,A_\linear)$ is reasonable if the following hold:
For all $\sealed{\sigma,\vsc} \in \overline{c_{\mathrm{export}}}$, with $\exec{\vsc}$, we have that $\vsc$ behaves reasonably up to any number of steps $n$.
\lau{This need to change with the proposed simplification of the reasonability update.}
...
\end{lemma}
\begin{proof}
  Take $\sigma \in (\gsigrets\cup \gsigcloss)$, $w = \seal{\sigma,\sigma,\sigma}$, $W$ such that $W(r) = \codereg{\sigrets,\sigcloss,\code,\gc}$, $\sigrets \subseteq \gsigrets$, $\sigcloss\subseteq \gsigcloss$, $\dom(\code) \subseteq \ta$ and $W$ empty otherwise.
  Then we have that $\npair[0]{(w,w)}\in \lrvg{\trusted}(W)$ but not $\npair[0]{(w,w)}\in\lrv(W)$.
\end{proof}

\begin{lemma}
  \label{lem:codereg-Hsigma-not-lrv}
  If $\dom(\code) \subseteq \ta$,  $\sigrets \subseteq \gsigrets$, $\sigcloss \subseteq \gsigcloss$, $n > 0$ and $\sigma \in (\sigrets \uplus \sigcloss)$, then $\codereg{\sigrets,\sigcloss,\code,(\ta,\stkb)}.H_\sigma~ \sigma \not\nequal (\lrv \circ \xi)$.
\end{lemma}
\begin{proof}
  ...
  \item $\sigma \in \sigrets$:
    ...
    In this case, we have that $\npair[0]{\left(\retptrd(\baddr,\eaddr),((\rw,\linear),\baddr,\eaddr,\baddr-1)\right)}$ is not in $\lrv(\xi(\hat{W}))$, but we can show that it is in $\codereg{\sigrets,\sigcloss,\code,(\ta,\stkb)}.H_\sigma~ \sigma~\hat{W}$ by taking a $\hat{W}$ such that the following hold:
    \begin{itemize}
  ...
  \item $\sigma \in \sigcloss$: This follows easily from the definition of $\codereg{}$ and Lemma~\ref{lem:lrvtrusted-not-nequal-lrvuntrusted}.
  \end{itemize}
\end{proof}

\begin{lemma}
  \label{lem:codereg-not-read-write}
  If $n > 0$, $\dom(\code) \subseteq \ta$, $\gc = (\ta,\stkb)$, $\pwheap(r_\code) \nequal \codereg{\sigrets,\sigcloss,\code,\gc}$ and $a \in \dom(\code)$ and $\memSat{\ms_S,\stk,\ms_\stk,\ms_T}{W}$ and $\sigrets \neq \emptyset$, then
  \begin{itemize}
  \item $\npair{\{a\}} \not\in \readCond{\normal,W}$
    % \item $\npair{\{a\}} \not\in \writeCond{\normal,W}$
...
\begin{proof}
  ...
  \end{itemize}
  We claim that $r_\code \in S$.
  If this is indeed the case, then we have $\pwheap(r_\code).H \nsubeq \stdreg{\dom(\code),\gc}{\pur}.H$ (we must have $R(r_\code) = \dom(\code)$ as the two regions couldn't be $\nequal$ otherwise).
  This means that for all $a \in \dom(\code)$ and $n' < n$ it must be the case that $\npair[n']{(\code(a),\code(a))} \in \lrv(W_a)$ for all $W_a$ that are compatible w.r.t.
  $\oplus$ and combine to be $W$.
  We know that $\memSat{\ms_S,\stk,\ms_\stk,\ms_T}{W}$ and $\pwheap(r_\code) \nequal \codereg{\sigrets,\sigcloss,\code,\gc}$ from which we can deduce $\npair{(\code,\code)} \in H^\mathrm{code} \; \sigrets \; \sigcloss \; \code \; (\ta,\_) \; \xi^{-1}(W_\code)$ which in particular gives us $\sigrets,\sigcloss \vdash_{\mathrm{comp-code}} \code$.
  This in turns entails that for $\sigma \in \sigrets$ there exists some $a' \in \dom(\code)$ such that $\code(a') = \seal{\sigma_\aaddr,\sigma_\baddr,\sigma_\eaddr}$ and $\sigma \in [\sigma_\baddr,\sigma_\eaddr]$.
  This means that for such a $\sigma$ and $a'$, it must be that case that
  $\npair[n']{(\seal{\sigma_\aaddr,\sigma_\baddr,\sigma_\eaddr},\seal{\sigma_\aaddr,\sigma_\baddr,\sigma_\eaddr})}
  \in \lrv(W_{a'})$ which gives us
  $\codereg{\sigrets,\sigcloss,\code,\gc}.H_\sigma  \; \sigma \nequal \lrv$
  contradicting Lemma~\ref{lem:codereg-Hsigma-not-lrv}.

  It remains to show $r_\code \in S$.
  To show this, we assume for contradiction that $r_\code \not\in S$.
  This means that there exists $r \neq r_\code$ such that $a \in R(r)$ and $\pwheap(r).H \nsubeq \stdreg{R(r),\gc}{\pur}.H$.
  ...
  In this case, both $r$ and $r_\code$ needs a memory segment that has $a$ in the domain which means that no such disjoint union can exist which gives us a contradiction.
\end{proof}
...
\begin{lemma}[Untrusted source values are reasonable]
  ...
  \item $\pwheap[W_w](r_\code) \nequal \codereg{\sigrets,\sigcloss,\code,\gc}$
  \item $\dom(\code) \subseteq \ta$
  \item $\sigrets \subseteq \gsigrets$ and $\sigcloss \subseteq \gsigcloss$
  \end{itemize}
  then, with respect to $\code,\ta,\gsigrets,\gsigcloss$, $w$ is reasonable up to $n$ steps in memory $\ms_S$ and free stack $\ms_\stk$.
\end{lemma}
\begin{proof}
  Induction over $n$.
  ...
  \item If $w = \seal{\sigma_\baddr,\sigma_\eaddr,\_}$, then $[\sigma_\baddr,\sigma_\eaddr] \mathrel{\#} (\gsigrets \cup \gsigcloss)$:
    ...
    Take $\sigma \in [\sigma_b,\sigma_e]$, we need to prove that $\sigma \not\in (\gsigrets \cup \gsigcloss)$.
    Assume that $\sigma \in (\sigrets \cup \sigcloss)$ and we will prove a contradiction.

    From $\npair{(w,\_)} \in \lrv(W_w)$, we get a $r \in \dom(\pwheap[W_w])$ such that $\pwheap[W_w](r) = (\pure,\_,H_\sigma)$ and $H_\sigma~\sigma \nequal \lrv$.
    
    From $\memSat{\ms_S,\stk,\ms_\stk,\_}{W_M}$, it follows that $\npair{(\overline{\sigma},\_,\_)} \in \lrheap(\pwheap[W_M])(W_{\var{heap}})$ for some $\overline{\sigma}$, $W_{\var{heap}}$ with $W_M = W_{\var{stack}} \oplus W_{\var{free\_stack}} \oplus W_{\var{heap}}$.

    From this, it follows that there exists a $R_\var{seal} : \dom(\activeReg{\pwheap[W_{\var{heap}}]}) \fun \powerset{\Seal}$, $\biguplus_{r \in \dom(\activeReg{\pwheap[W_{\var{heap}}]})} R_\var{seal}(r)) \subseteq \overline{\sigma}$ and $\dom(\pwheap[W_{\var{heap}}](r).H_\sigma) = R_\var{seal}(r)$ for all $r$.
   
    From $\dom(\pwheap[W_{\var{heap}}](r).H_\sigma) = R_\var{seal}(r)$ and the definedness of all the $\oplus$ operations on worlds, it follows that also $\dom(\pwheap[W_w](r).H_\sigma) = R_\var{seal}(r)$ for all $r$.

    From the facts that $\pwheap[W_w](r_\code) \nequal \codereg{\sigrets,\sigcloss,\code,(\ta,\stkb)}$, $\pwheap[W_w].H_\sigma~\sigma \nequal \lrv$, the disjointness of all the $R_{\var{seal}}(r)$, it follows that $r = r_\code$.
    However, this contradicts Lemma~\ref{lem:codereg-Hsigma-not-lrv}.

  \item If $w = ((\perm,\_),\baddr,\eaddr,\_)$ and $\perm \neq 0$, then $[\baddr,\eaddr] \# \ta$:

    Assume that $a \in [\baddr,\eaddr] \cap \ta$ and we will prove a contradiction.

    From $\npair{(w,\_)} \in \lrv(W_w)$, it follows that $\npair{\{a\}}\in\readCond{\normal,W_w}$.
    
    This contradicts Lemma~\ref{lem:codereg-not-read-write}.

  \item If $w = \sealed{\sigma,\vsc}$ and $\sigma \not\in (\gsigrets\cup \gsigcloss)$ then $\vsc$ is reasonable for $n-1$ steps: By $\npair{(w,\_)} \in \lrv(W_w)$, we get some region $r \in \dom(\pwheap)$ and a set of return seals, a set of closure seals and a code memory: $\sigrets',\sigcloss',\mscode'$ such that $\pwheap(r) = (\pure,\_,H^\mathrm{code,\square}_\sigma \; \sigrets' \; \sigcloss' \; \mscode' \; \gc)$ and $\npair[n']{(\sealed{\sigma,\vsc},\_)} \in H^\mathrm{code,\square}_\sigma \; \sigrets' \; \sigcloss' \; \mscode' \; \gc \; \sigma \; \xi^{-1}(W) \text{ for all $n' < n$}$, so in particular for $n-1$.

    It also follows that $\sigrets \uplus \sigcloss \# \sigrets' \uplus \sigcloss'$.
    ...
    \item $\dom(\code') \subseteq \ta \tand \sigcloss' \subseteq \gsigcloss \npair{(\vsc,\_)} \in \lrvtrusted(W)$
    \end{itemize}
    and in either case $\sigma \in \sigcloss'$
  ...
  \item If $w = ((\perm,\_),\baddr,\eaddr,\_)$ and $\perm \in \readAllowed{}$ and $n > 0$, then $\ms_S(\aaddr)$ is reasonable up to $n - 1$ steps for all $\aaddr \in ([\baddr,\eaddr] \setminus \ta)$:
    ...
    \item $\pwheap[W_w](r_\code) \nequal[n-1] \codereg{\sigrets,\sigcloss,\code,\gc}$

      By property of $n$-equality.
    \item $\dom(\code) \subseteq \ta$ and $\sigrets \subseteq \gsigrets$ and $\sigcloss \subseteq \gsigcloss$ and $\gc = (\ta,\stkb,\gsigrets,\gsigcloss)$
      
      Follows by assumption.
    \end{itemize}
    which gives us that, with respect to $\code,\ta,\gsigrets,\gsigcloss$, $\pi_1(R_\ms(r))(a)$ is reasonable up to $n-1$ steps in memory $\ms_S$ and free stack $\ms_\stk$.
...
\begin{lemma}[Untrusted register files are reasonable]
  ...
  \item $\sigrets \subseteq \gsigrets$ and $\sigcloss \subseteq \gsigcloss$
  \end{itemize}
  then, with respect to $\code,\ta,\gsigrets,\gsigcloss$,
  \begin{itemize}
...
\begin{lemma}[Reasonable things don't need to be trusted]
  ...
  \item Theorem~\ref{thm:ftlr} holds up to $n$ steps.
    \dominique{mention this assumption explicitly to avoid cyclic reasoning in the FTLR proof.}
  \end{itemize}
  Then
  \begin{itemize}
  \item $\npair{(w,\_)} \in \lrv(W_w)$
...
\begin{proof}
  ...
  \item 
    ...
    The latter follows by Theorem~\ref{thm:ftlr}, Lemma~\ref{lem:conds-shrinkable} and definition of $\execCond{}$.
  \end{itemize}
\end{proof}
...
\begin{theorem}[FTLR]
  ...
  \item $\trust = \trusted \vee \npair{[\baddr,\eaddr]} \in \readCond{\lin,W}$
    \dominique{This requirement is actually not needed because of Lemma~\ref{lem:xReadCond-outside-ta-implies-readCond}}
