...
\[
  \begin{array}{rcl}
    ...
    \src{i} & \defbnf &  \trg{i} \mid \scall{o}{\src{r}}{\src{r}}
  \end{array}
\]
...
\begin{definition}
  ...
  A program, $\program$ consists of two sealed capabilities, $\vsc_\code = \sealed{\sigma,c_\code}$ and $\vsc_\data = \sealed{\sigma,c_\data}$, and and two memory segments $\ms$ and $\ms_\link$ satisfying
  \begin{itemize}
...
\[
  ...
  \lrk(W) = &\left\{ \npair{\left(\array{l}\src{\retptrc(\sigma,(\perm,\lin,\baddr_c,\eaddr_c,\aaddr_c))},\\\src{\retptrd(\sigma,(\aaddr_d,\eaddr_d))},\\k_{c,T},k_{d,T}\endarray\right)} \middle|
    \begin{array}{l}
      ...
      \qquad\npair[n']{\array{l}\src{\reg_S\update{\rstk}{\stkptr{\rw,\stkb,\eaddr_d,\aaddr_d}}},\\
                               {\reg_T\update{\rstk}{((\rw,\linear),\stkb,\eaddr_d,\aaddr_d)}}\endarray}\in \lrr(\{r_\stk\})(W_\lrr) \wedge\\ 
    ...
    \end{array}
  \right\}\\ \cup
    & \left\{ \npair{\left( k_{c,S},k_{d,S},k_{c,T},k_{d,T} \right)} \middle|
      \begin{array}{l}
        \pi_\sigma(k_{c,S}) = \pi_\sigma(k_{d,S}) \wedge \pi_\sigma(k_{c,T}) = \pi_\sigma(k_{d,T}) \wedge \npair{(k_{c,S},k_{c,T}),(k_{d,S},k_{d,T})} \in \lrp(W) \\
        \forall n' \leq n, W' \pubft W \ldotp \\
        \quad \npair[n']{(k_{c,S},k_{c,T}),(k_{d,S},k_{d,T})} \in \lrp(W) \Rightarrow\\
        \qquad \forall \npair[n']{\stpair{\reg}{\reg}} \in \lrr(\{\rdata\})(W'_\lrr) \ldotp 
               \forall \memSat[n']{\src{\ms_S,\stk,\ms_\stk},\ms_T}{W'_\lrm} \ldotp\\
        \qquad \quad \npair[n']{\left(
        \arraycolsep=0pt\array{l}
          (\src{\ms_S},\src{\reg_S}\update{\pcreg,\rdata}{\pi_\vsc(\src{k_{c,S}}),\pi_\vsc(\src{k_{d,S}})},\src{\stk},\src{\ms_\stk})\\
          (\ms_T,\reg_T\update{\pcreg,\rdata}{\pi_\vsc(k_{c,T}),\pi_\vsc(k_{d,T})})
        \endarray\right)} \in \lro(W' \oplus W'_\lrr \oplus W'_\lrm)
      \end{array}
      \right\}
  \end{array}
\]
...
\[
  \lrr(R)(W) = \left\{ \npair{\stpair{\reg}{\reg}} \middle|
    \begin{array}{l}
      ...
      \quad W = \bigoplus_{r \in \RegName} S(r) \\
...
%
    ...
    \begin{array}{l}
      ...
      \qquad \qquad { \color{red} \forall (\src{c}, c') \in H_\sigma (\sigma) \ldotp \npair{\src{c,\retptrd(\sigma,\baddr,\eaddr)}, c' , \sealed{\sigma,\vsc_D}} \in \lrk(W') }%\text{\lau{This needs some work - specifically what i c and c' and do we need to seal them?}}
...
%
    ...
    \right\}\cup\\
%
    \left\{ \npair{\left(\arraycolsep=0pt\array{l} \src{\seal{\lin,\sigma,\sigma_\baddr,\sigma_\eaddr}},\\ \seal{\lin,\sigma,\sigma_\baddr,\sigma_\eaddr} \endarray \right)} 
    \right\} \cup \\
%
...
%
  ...
  \end{array}
\]
\lau{28-11-2017: Do we want the execute condition for $\rwx$ is we are doing write xor execute?}
