...
\begin{proof}
  ...
  \item If $w = ((\perm,\_),\baddr,\eaddr,\_)$ and $\perm \neq 0$, then $[\baddr,\eaddr] \# \ta$:
    ...
    \dominique{TODO: we don't know that a in dom(code)!}
  ...
  \item If $w = \sealed{\sigma,\vsc}$ and $\sigma \not\in (\gsigrets\cup \gsigcloss)$ then $\vsc$ is reasonable for $n-1$ steps:
    ...
    It also follows from $\memSat{\ms_S,\stk,\ms_\stk,\_}{W_M}$, $\sigma \not\in (\gsigrets\cup \gsigcloss)$ and $\sigrets \subseteq \gsigrets$ and $\sigcloss \subseteq \gsigcloss$ that $\sigrets \uplus \sigcloss \# \sigrets' \uplus \sigcloss'$.
    ...
    \item $\dom(\code') \subseteq \ta$, $\sigcloss' \subseteq \gsigcloss$, and $\npair{(\vsc,\_)} \in \lrvg{\trust}(W)$ for $\trust = \untrusted$ iff $\nonExec{\vsc}$.
    \end{itemize}
