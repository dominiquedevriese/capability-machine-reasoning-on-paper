...
\lau{TODO: Add $\ta$ to the source step.}
\lau{TODO: Add $\stkb$ to source term and everything else where it is necessary. Consider how $\stkb$ and $\ta$ can be added without passing around these values in LR and source semantics (solution to parameterize both with it, so they need to be provided only once.)}
\begin{align*}
...
\begin{align*}
  \var{comp} &\mathrel{::=} (\mscode,\msdata,\overline{c_{\mathrm{import}}},\overline{c_{\mathrm{export}}},\sigrets,\sigcloss)
\end{align*}
...
\begin{mathpar}
  ...
  \inference{
    ...
    A_{\mathrm{code}},A_{\mathrm{own}},A_{\mathrm{non-linear}},\overline{c_{\mathrm{import}}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} ((\perm,\normal),\baddr,\eaddr,\aaddr)
  }
  \and
  \inference{
    \permbnf \sqsubseteq \rw&
    \lin = \linear \Rightarrow \emptyset \subset [\baddr,\eaddr] \subseteq A_{\mathrm{own}} &
    \lin = \linear \Rightarrow [\baddr,\eaddr] \subseteq A_{\mathrm{non-linear}}
  }{
  ...
  \inference{
    ...
    \sigrets,\sigcloss \vdash_{\mathrm{comp-code}} \mscode \\
    \exists A_\mathrm{own} : \dom(\msdata) \rightarrow \powerset{\dom(\msdata)} & \dom(\msdata) = A_{\mathrm{non-linear}} \uplus \biguplus_{a \in \dom(\msdata)} A_{\mathrm{own}}(a) \\
...
\lau{22-3-2018: need extra rules for regions with seal interpretations}
\begin{mathpar}
...
% {\color{DarkGreen}
\lau{TODO add frame to memory satisfaction and remove it from observation relation.}
...
\[
  \lrol = \left\{ \npair{\left(\array{l}\src{(\ms_S,\reg_S,\stk_S,\ms_{\stk,S})},\\(\ms_T,\reg_T)\endarray\right)} \middle|
    \begin{array}{l}
      ...
      \quad \src{(\ms_S \uplus \ms_{f,S},\reg_S,\stk_S,\ms_{\stk,S})} \sterm[i]{\ta,\stkb} \Rightarrow (\ms_T \uplus \ms_{f,T},\reg_T) \term\\
    \end{array}
\right\}
\]
\[
  \lror = \left\{ \npair{\left(\array{l}\src{(\ms_S,\reg_S,\stk_S,\ms_{\stk,S})},\\(\ms_T,\reg_T)\endarray\right)} \middle|
    \begin{array}{l}
      \forall i \leq n, \src{\ms_{f,S}}, \ms_{f,T} \ldotp \\
      \quad (\ms_T \uplus \ms_{f,T},\reg_T) \term[i] \Rightarrow \src{(\ms_S \uplus \ms_{f,S},\reg_S,\stk_S,\ms_{\stk,S})} \sterm{\ta,\stkb}
    \end{array}
\right\}
\]
...
\lau{TODO check what conditions need to have a ``square'' i.e. need approximation}
\[
  \lrv(W) =
  \begin{array}[t]{l}
...
%
    ...
    \; \middle| \;
    \begin{array}{l}
      \exists r \in \dom(\pwheap) \ldotp \pwheap(r) =_n \codereg{\sigrets,\sigcloss,\code,\gc} \wedge\\
...
\[
  \readCond{\lin,W} = \left\{ \npair{(\baddr,\eaddr})) \middle| 
    \begin{array}{l}
      ...
      \qquad \pwheap(r) \nsubeq \stdreg{A,\gc}{\pur}
    \end{array}
  \right\}
\]
\lau{TODO: Make this condition require a set of adjacent regions. Check whether needed for the stack condition.}
...
\[
  \stackReadCond{W} = \left\{ \npair{(\baddr,\eaddr})) \middle| 
    \begin{array}{l}
      ...
      \qquad \pwfree(r) \nsubeq \stdreg{[\baddr',\eaddr'],\gc}{\pur}
    \end{array}
  \right\}
\]
where $\iota_A$ is a standard region defined in Section~\ref{sec:standard-regions}.
\[
  ...
  \right\}
\]
\lau{TODO: Make this condition require a set of adjacent regions.}
\lau{TODO: allow code regions to be merged into one. Allow for }
...
\[
  \writeCond{\lin,W} = \left\{ \npair{(\baddr,\eaddr)}) \middle| 
    \begin{array}{l}
      \exists r \in \addressable{\lin, \pwheap} \ldotp \\
      \quad \exists [\baddr',\eaddr'] \supseteq [\baddr,\eaddr] \ldotp \\
      \qquad \pwheap(r) \nsupeq[n-1] \stdreg{[\baddr',\eaddr'],\gc}{\pur} \wedge \\
...
where $\iota_A$ is a standard region defined in Section~\ref{sec:standard-regions}.
\dominique{Why $n-1$ above?  There is already a quantification over $n' <n$ in the memory satisfaction relation and another $n-1$ is used in $\stdreg{[\baddr',\eaddr'],\gc}{\pur}$. Also: readCond doesn't have the $n-1$.}
\[
...
Standard region:
\[
  \stdreg{A,\gc}{v} \defeq (v,H_A^{\mathrm{std}} \; \gc)
\]
...
\[
  H_A^{\mathrm{std}} \; \gc \; \hat{W} \defeq \left\{ \npair{\ms_S,\ms_T} \middle|
    \begin{array}{l}
...
\[
  \stareg[\stpair{\ms}{\ms},\gc]{v} = (v,H^\mathrm{sta}_{\stpair{\ms}{\ms}} \; \gc)
\]
\[
  H^\mathrm{sta}_{\stpair{\ms}{\ms}} \; \gc = \left\{ \npair{\stpair{\ms}{\ms}} \middle| 
    \begin{array}{l}
...
\[
  \codereg{\sigrets,\sigcloss,\code,\gc} \defeq (\pure,H^\mathrm{code} \; \sigrets \; \sigcloss \; \code,H^\mathrm{code}_\sigma \; \sigrets \; \sigcloss \; \code \; \gc)
\]
...
\begin{multline*}
  H^\mathrm{code}_\sigma \; \sigrets \; \sigcloss \; \code \; \gc \; \sigma \; \hat{W} = \\
  \begin{array}[t]{l}
\left\{
    \begin{array}{l}
...
\begin{definition}[World specifies component]
  ...
  \end{equation*}
  (written as $W \vdash_\gc \var{comp}$)
  iff
  \begin{equation*}
    \vdash (\mscode,\msdata,\overline{c_{\mathrm{import}}},\overline{c_{\mathrm{export}}},\sigrets,\sigcloss)
  \end{equation*}
...
\lau{TODO: components have optional main (pc/data pair)}
\lau{TODO: linking component and context requires only one main (i.e., it is undefined if both or none of the two expose main.)}
\lau{TODO: Component logical relation.}
\lau{TODO: Version of FTLR for programs.}
\lau{TODO: Component lemma for initial executable configurations.}
\lau{TODO: Executable configuration lemma.}
\lau{TODO: ... should use initial executable configuration.}
\begin{definition}
