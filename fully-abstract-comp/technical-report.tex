...
\begin{mathpar}
  ...
  \inference{
    ...
    A_{\mathrm{code}},A_{\mathrm{own}},A_{\mathrm{non-linear}},\overline{c_{\mathrm{import}}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} \sealed{\sigma,((\perm,\rx),\baddr,\eaddr,\aaddr)}
  }
  \and
  \inference{
...
\begin{lemma}[FTLR for component code-values]
  If
  \begin{itemize}
  \item $W(r_{\mathrm{code}}) = \codereg{\sigrets,\sigcloss,\mscode,\gc}$
  \item ($\trust = \untrusted$ and $\dom(\mscode \mathrel{\#} \ta)$) or ($\trust = \trusted$ and $\dom(\mscode) \subseteq \ta$)
  \item $w = ((\perm,\rx),\baddr,\eaddr,\aaddr)$
  \end{itemize}
...
\begin{proof}
  ...
  \item $w = \sealed{\sigma,((\perm,\rx),\baddr,\eaddr,\aaddr)}$, $[\baddr,\eaddr]\subseteq A_{\mathrm{code}}$, $\sigma \in \sigcloss$:
