...
\dominique{I think we need to do something about tail calls in the source language?}
...
&
                                        ...
                                        \begin{array}[t]{l}
                                          ...
                                          c = ((\decPerm{n},\lin),\baddr,\eaddr,\aaddr) \tand \\Meget
                                          w_2 = \linCons{\Phi(r_2)}
                                        \end{array}
                                        \\
                                        \sourcecolor\left.
                                   ...
                                   \begin{cases}
                                     ...
                                     \sourcecolor\left.
                                       \begin{array}[t]{l}
                                         ...
                                         \ms_{\stk,\priv}(\baddr_\stk-1) = \eaddr_\stk \tand \\
  ...
  \sem{\scall{\src{r_1}}{\src{r_2}}}(\Phi) = & 
                                               \begin{cases}
                                                 ...
                                                 \begin{array}[t]{l}
                                                   ...
                                                   \baddr_\stk \leq \aaddr_\stk < \eaddr_\stk \tand \\
...
The \emph{peristent data storage} is expected to be accessible through a normal read-write capability in $r_\data$ during normal execution (during a return the stack capability will initially be in the $r_\data$ register, but it should be spliced with the returned stack and moved to the $r_\stk$ register). There are two ways for a program to get this data capability: (1) the data part of the code/data pair (for either a callback or a global function in the linking table) should be the sealed data capability. (2) During a call, the data capability should be stored on the stack, so it can be retrieved afterwards.
\begin{figure}
  \centering
...
\begin{definition}
  ...
  \begin{itemize}
  \item We say that a memory segment $\ms$ and a list of capabilities $\bar{c}$
    are self-contained iff
  \[
    \forall c \in \bar{c} \ldotp \range{c} \subseteq \dom{\ms}
  \]
  and
  \begin{multline*}
...
Here we define what consitutes a program.

\begin{definition}
  \label{def:program}
  \lau{27-09-2017: The ``somehow''s need to be fixed.}
  A program consists of two sealed capabilities, $\vsc_\code = \sealed{c_\code,\sigma}$ and $\vsc_\data = \sealed{c_\data,\sigma}$, and and two memory segments $\ms$ and $\ms_\link$ satisfying
  \begin{itemize}
  \item $c_\code = ((\rx,\lin_\code),\baddr_\code,\eaddr_\code,\aaddr_\code)$ where $\aaddr = \baddr + 3$ and $[\baddr_\code,\eaddr_\code]\subseteq \dom(\ms)$
  ...
  \item $\ms(\baddr_\code+2) = \dots$ where $\dots$ is somehow the stack height.
  \end{itemize}
\end{definition}
...
\]
\dominique{Regarding the type signature for $[\cdot]$ above: does this mean that it maps lists of instructions to lists of instructions? Is there another function that compiles a single instruction to a list of instructions?}
...
\dominique{Is this example still relevant? Isn't it solved differently by the variable-length-instructions idea?}
...
\item Considered PORs for transition systems to: ($a \rightarrow b \Rightarrow a \rightarrow a \text{ and } b \rightarrow b$)
  \dominique{I don't remember what we meant with PORs, but perhaps quasi-reflexive partial orders.  Not sure what the point of this was again... }
  \begin{itemize}
...
One way to solve this is to make it publicly known what the end address of the stack is and always check that we receive a part of the stack that goes all the way to the end. In the above example (using the stack parts in parenthesis) this would cause the sequence of calls to fail the second time we are called, because we do not receive a stack that goes all the way to the top of the stack.
