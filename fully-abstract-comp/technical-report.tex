...
\begin{lemma}[permission-based conditions shrinkable]
  ...
  \item If $\npair{A} \in \writeCond{\lin,W}$  and $A' \subseteq A$, then
    $\npair{A'} \in \readCond{\lin,W}$.
  \item If $\npair{A} \in \xReadCond{\lin,W}$  and $A' \subseteq A$, then
    $\npair{A'} \in \xReadCond{\lin,W}$.
  \item If $\npair{A} \in \execCond{\lin,W}$  and $A' \subseteq A$, then
    $\npair{A'} \in \execCond{\lin,W}$.
  \end{itemize}
\end{lemma}
\begin{proof}
  Follows easily from the definition. TODO: check after update
\end{proof}
