...
%
    ...
    \begin{array}{l}
      ...
      \quad H_\sigma = H^\mathrm{code,\square}_\sigma \; \sigrets \; \sigcloss \; \mscode \; \gc \tand \\
...
\begin{equation*}
  H^\mathrm{code} \; \sigrets \; \sigcloss \; \code \; (\ta,\_) \; \hat{W} = \left\{\npair{(\code,\code)} \middle|
    ...
    \dom(\code) = [\aaddr,\baddr] \wedge \\
      (\dom(\code) \subseteq \ta \wedge \trust = \trusted) \vee \\
...
Take a block of code memory $\mscode$, a set of trusted addresses $\ta$ and sets of return pointer and closure seals $\sigrets$ and $\sigcloss$.
We define that a word $w$ is reasonable up to $n$ steps in memory $\ms$ if the following implications hold.
\begin{itemize}
...
\item If $w = \sealed{\sigma,\vsc}$ and $\sigma \not\in (\sigrets\cup \sigcloss)$ then $\vsc$ is reasonable.
...
We say that an execution configuration $\Phi$ is reasonable up to $n$ steps if:
  ...
  \item \emph{Use return seals only for calls} If
    ...
    \begin{itemize}
    \item $\src{\Phi'}$ is inside $\src{\scall{\offpc,\offsigma}{r_1'}{r_2'}}$ and $\sigma \in \sigrets$ 
...
\begin{lemma}
  ...
  If $\dom(\code) \subseteq \ta$, $n > 0$ and $\sigma \in (\sigrets \uplus \sigcloss)$, then $\codereg{\sigrets,\sigcloss,\code,(\ta,\stkb)}.H_\sigma \sigma \not\nequal \lrv$.
\end{lemma}
\begin{proof}
  TODO
\end{proof}
