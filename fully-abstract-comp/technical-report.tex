...
We say that an executable capability $c = ((\perm,\normal),\baddr,\eaddr,\aaddr)$ behaves reasonably up to $n$ steps if the following implication holds:
...
For all $\sealed{\sigma,\vsc},\sealed{\sigma,\vsc'} \in \overline{c_{\mathrm{export}}}$, with $\exec{\vsc}$ and $\nonExec{\vsc'}$, we have that $\vsc$ and $\vsc'$ behave reasonably with stack assumption $\emptyset$ up to any number of steps $n$.
