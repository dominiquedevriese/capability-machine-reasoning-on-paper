...
\begin{lemma}[Untrusted is trusted]
  \label{lem:untrusted-subset-trust}
  \begin{itemize}
  \item $\lrvtrusted(W) \subseteq \lrv(W)$
  \item $\lrrtrusted(W) \subseteq \lrr(W)$
  \end{itemize}
\end{lemma}
\begin{proof}
  TODO
...
\begin{definition}[Reasonable configuration]
  ...
  \item \emph{Guarantee stack base address before call} If
    ...
    \item $r_1 \neq \rtmp{1}$
    \item $\Phi'(\pcreg) + \calllen$ behaves reasonably up to $n'-i$ steps
    \end{itemize}
  \item \emph{Use return seals only for calls, use closure seals appropriately} If
    ...
    \item $\sigma \in \gsigcloss$ and one of the following holds:
      \begin{itemize}
      \item $\exec{\Phi'(r_2)}$ and $\Phi'(r_2)$ behaves reasonably up to $n$ steps.
      \item $\nonExec{\Phi'(r_2)}$ and $\Phi'(r_2)$ is reasonable up to $n$ steps in memory $\Phi'.\ms$ and free stack $\Phi'.\ms_\stk$.
      \end{itemize}
    \end{itemize}
  \item \emph{Don't store private stuff...} If
    ...
    \item $0 < i \le n'$
    \end{itemize}
    Then $\Phi.\reg(r_2)$ is reasonable in memory $\Phi'.\mem$ up to $n'-i$ steps
...
\begin{lemma}
  ...
  If $\dom(\code) \subseteq \ta$,  $\sigrets \subseteq \gsigrets$, $\sigcloss \subseteq \gsigcloss$, $n > 0$ and $\sigma \in (\sigrets \uplus \sigcloss)$, then $\codereg{\sigrets,\sigcloss,\code,(\ta,\stkb,\gsigrets,\gsigcloss)}.H_\sigma~ \sigma \not\nequal (\lrv \circ \xi)$.
\end{lemma}
\begin{proof}
  ...
  \item $\sigma \in \sigrets$:
    ...
    In this case, we have that $\npair[0]{\left(\retptrd(\baddr,\eaddr),((\rw,\linear),\baddr,\eaddr,\baddr-1)\right)}$ is not in $\lrv(\xi(\hat{W}))$, but we can show that it is in $\codereg{\sigrets,\sigcloss,\code,(\ta,\stkb,\gsigrets,\gsigcloss)}.H_\sigma~ \sigma~\hat{W}$ by taking a $\hat{W}$ such that the following hold:
    \begin{itemize}
...
\begin{proof}
  ...
  \begin{itemize}
  \item $\npair[n']{\stpair{\reg}{\reg}} \in \lrrg{\untrusted}(\{\rdata\}) (W_\lrrs )$
