...
\trg{i} &\defbnf & \tfail \mid \thalt \mid \tjmp{\trg{r}} \mid \tjnz{\trg{r}}{\trg{\rn}} \mid \tisptr{\trg{r}}{\trg{r}} \mid \tgeta{\trg{r}}{\trg{r}} \mid \tgetb{\trg{r}}{\trg{r}} \mid \\
  ...
  & & \tplus{\trg{r}}{\trg{\rn}}{\trg{\rn}} \mid \tminus{\trg{r}}{\trg{\rn}}{\trg{\rn}} \mid \tsetatob{\trg{r}} \mid \txjmp{\trg{r}}{\trg{r}} \mid \tcseal{\trg{r}}{\trg{r}}{\trg{r}} \mid \\ 
...
\begin{definition}[Use return seals only for call]
  A configuration $\src{\Phi}$ is said to use linker seals properly in $\ta$ for $n$ steps iff\\
  for all $i \leq n$, if
  \[
    \src{\Phi} \nstep[i] \src{\Phi'}
  \]
  and $\src{\Phi'}$ points to $\src{\tload{r_1}{r_2}}$ in $\ta$ and $\Phi'(r_2)
  = ((\perm,\lin),\baddr,\eaddr,\aaddr)$, $\aaddr \in [\baddr,\eaddr]$, $\aaddr
  \in \ta$ and $\Phi.\mem(\aaddr) = \seal{\sigma_b,\sigma_e}$, then
  $\src{\Phi'}$ is inside $\src{\scall{o}{r_1'}{r_2'}}$.
  \dominique{TODO: this should be allowed to be able to construct closures. Not allowed only for return pointer seals.}
\end{definition}
...
\begin{definition}[Never pass trusted seals accross the trusted-untrusted boundary]
  ...
  \]
  and $\src{\Phi'}$ is in $\ta$ and $\src{\Phi''}$ is not in $\ta$ and
  $\src{\Phi}.\reg(r_i) = \seal{\sigma_b,\sigma_e}$ and
