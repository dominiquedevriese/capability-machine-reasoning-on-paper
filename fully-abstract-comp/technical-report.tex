...
This document is a technical report accompanying a paper by the same title and authors, published at POPL 2019.
It contains proofs and details that were omitted from the paper for space and presentation reasons.
...
\[
  \begin{array}{rrcl}
   \addrbnf,\basebnf \in & \Addr & \defeq & \nats \\
    ...
    \permbnf \in& \Perm & \defbnf & \noperm \mid \readonly \mid \rw \mid \rx
                                    \mid \rwx \\
...
For the linearity encoding, we make similar assumptions: $\encLin{} : \Linear \fun \ints$ encodes linearity. $\encLin{}$ is injective and for all $\lin \in \Linear$ $\encLin{\lin} \neq -1$ (as this is used as an error value). As a capabilities linearity stays the same, we do not need a decoding function for linearity.
\begin{align*}
...
\begin{align*}
  \sem{\tmove{r}{\rn}}(\Phi) = & 
                              \begin{cases}
                                \updPcAddr{\Phi\updReg{r}{\rn}} & r \neq \pcreg \wedge \rn \in \ints \\
                                \updPcAddr{\Phi\updReg{\rn}{w}\updReg{r}{\Phi(\rn)}} & r \neq \pcreg \wedge w = \linCons{\Phi(\rn)}\\
                                \failed & \totherwise
                              \end{cases}
\end{align*}
...
\end{align*}
\clearpage
We define inductively when a component is valid ($\ta \vdash \var{comp}$) by the below inference rules:
\begin{mathpar}
  \inference{
...
{    \begin{multlined}
  ...
  \inference{
    ...
    % domi: this was only used in Lemma~\ref{lem:codereg-H-not-subs-lrv}, so not needed anymore.
...
\begin{mathpar}
  \inference{
    ...
    A_{\mathrm{code}},A_{\mathrm{own}},A_{\mathrm{non-linear}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} z
  }
  \and
  \inference{
    ...
    A_{\mathrm{code}},A_{\mathrm{own}},A_{\mathrm{non-linear}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} ((\perm,\lin),\baddr,\eaddr,\aaddr)
  }
  \and
  \inference{
    A_{\mathrm{code}},A_{\mathrm{own}},A_{\mathrm{non-linear}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} \vsc \\
    \sigma \in \sigcloss
  }{
    A_{\mathrm{code}},A_{\mathrm{own}},A_{\mathrm{non-linear}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} \sealed{\sigma,\vsc}
  }
\end{mathpar}
...
\begin{mathpar}
  \inference{
    ...
    A_{\mathrm{code}},A_{\mathrm{non-linear}},\sigrets,\sigcloss \vdash_{\mathrm{comp-export}} s \mapsto \sealed{\sigma,((\rx,\normal),\baddr,\eaddr,\aaddr)}
  }
  \and
  \inference{
    A_{\mathrm{code}},\emptyset,A_{\mathrm{non-linear}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} w
  }{
    A_{\mathrm{code}},A_{\mathrm{non-linear}},\sigrets,\sigcloss \vdash_{\mathrm{comp-export}} s \mapsto w
  }
\end{mathpar}
...
\begin{mathpar}
  ...
  \inference{
    ...
    \ta \vdash \var{comp}_0 & (\_ \mapsto c_{\mathrm{main},c}), (\_ \mapsto c_{\mathrm{main},d}) \in \overline{\var{export}}
  }{
...
\begin{definition}[Initial execution configuration]
  \begin{mathpar}
    \inference{
      ...
      \range{\ms_\stk} = \{0\}&
      \mem = \mscode \uplus \msdata \trg{\;\uplus\; \ms_\stk} \uplus \ms_\var{frame} \\
      [\baddr_\stk,\eaddr_\stk] = \dom(\ms_\stk ) &
      [\baddr_\stk-1,\eaddr_\stk+1]\mathrel{\#} (\dom(\mscode) \cup \dom(\msdata)) &
  ...
  \end{mathpar}
\end{definition}
\lau{07-07-2018: TODO why don't the above definition care about the stack base?}
...
\begin{lemma}
  \label{lem:context-trusted-addr-ampl}
  For components $\context$ and $\comp$,
  if
  \begin{itemize}
  \item $\emptyset \vdash \context$
  \item $\dom(\comp.\mscode) \vdash \comp$
  \item $\plug{\context}{\comp}$ is defined
  \end{itemize}
  Then
  \[
    \dom(\comp.\mscode) \vdash \context
  \]
\end{lemma}
\begin{proof}
...
\and
  ...
  \and
  \inferrule{ }{(\pure,H) \future (\pure, H)}
% \and
...
\begin{lemma}[Adding memory is the future]
...
\[
  ...
  \left\{
    \begin{array}{l}
      ...
      \quad \forall j < i \ldotp \forall a \in \dom(\ms_i) \ldotp \forall a' \in \dom(\ms_j) \ldotp
      a < a' < \stkb) \wedge\\
...
\[
  ...
  \left\{
    ...
    \begin{array}{l}
      ...
      \quad \dom(\pwheap(r).H_{\mathrm{seal}}) = R_\var{seal}(r)
    \end{array}
  \right.
\]
...
\lau{09-07-2018: Fix these definitions' formatting like in paper} 
\[
...
%       \quad \lin = \linear \Rightarrow \forall r \in \ldotp S |P(r)| \wedge \\
%       \quad \forall r \in S, n' < n, W'\ldotp \npair[n']{\stpair{\ms}{\ms}} \in \pwheap(r).H \; W'  \ldotp \\
...
%
    ...
    \begin{array}{l}
      ...
      \exists r \in \dom(\pwheap), \sigrets,\sigcloss,\mscode \ldotp \pwheap(r) = (\pure,\_,H_{\mathrm{seal}}) \tand \\
...
\]
where $\iota_A$ is a standard region defined in Section~\ref{sec:standard-regions}.
\dominique{22-6-2018: address-stratified is useless for length-one regions.}
...
H^\mathrm{code}_{\mathrm{seal}} \; \sigrets \; \sigcloss \; \code \; \gc)
\]
...
% \begin{lemma}
%   \label{lem:lrvtrusted-not-nequal-lrvuntrusted}
%   If $n > 0$ then $\lrv \not\nequal \lrvg{\trusted}$.
% \end{lemma}
% \begin{proof}
%   Take $\sigma \in (\gsigrets\cup \gsigcloss)$, $w = \seal{\sigma,\sigma,\sigma}$, $W$ such that $W(r) = \codereg{\sigrets,\sigcloss,\code,\gc}$, $\sigrets \subseteq \gsigrets$, $\sigcloss\subseteq \gsigcloss$, $\sigma \in (\sigrets\cup\sigcloss)$, $\dom(\code) \subseteq \ta$ and $W$ empty otherwise.
%   Then we have that $\npair[0]{(w,w)}\in \lrvg{\trusted}(W)$ but not $\npair[0]{(w,w)}\in\lrv(W)$.
% \end{proof}

% \begin{lemma}
%   \label{lem:codereg-H-not-subs-lrv}
%   If
%   \begin{itemize}
...
%     \subseteq \gsigcloss$
%   \item $n > 0$
%   \item $\sigrets,\sigcloss\vdash\var{code}$
%   \end{itemize}
%   then
%   \begin{equation*}
%     \codereg{\sigrets,\sigcloss,\code,(\ta,\stkb,\gsigrets,\gsigcloss)}.H \not\nsubeq \stdreg{\dom(\code),\gc}{\pure}.H
%   \end{equation*}
% \end{lemma}
% \begin{proof}
%   We show that $\npair[0]{(\code,\code)} \in \codereg{\sigrets,\sigcloss,\code,(\ta,\stkb,\gsigrets,\gsigcloss)}.H~\hat{W}$ but not $\npair[0]{(\code,\code)} \in \stdreg{\dom(\code),\gc}{\pure}.H~\hat{W}$ for a world with only island $\codereg{\sigrets,\sigcloss,\code,(\ta,\stkb,\gsigrets,\gsigcloss)}$ in $\pwfree$ but otherwise empty.

%   From $\sigrets,\sigcloss\vdash\var{code}$, we know that there is an $a$ such that $\code(a) = \seal{\sigma_b,\sigma_e,\_}$ with $[\sigma_b,\sigma_e] \neq \emptyset$
%   It follows easily that $\npair[0]{(\code,\code)} \not\in \stdreg{\dom(\code),\gc}{\pure}.H~\hat{W}$ since $\npair[0]{(\code(a),\code(a))} \not\in\lrv(W)$ by definition.

%   The fact that $\npair[0]{(\code,\code)} \in \codereg{\sigrets,\sigcloss,\code,(\ta,\stkb,\gsigrets,\gsigcloss)}.H~\hat{W}$ follows from Lemma~\ref{lem:ftlr-component-code-ptrs}.
% \end{proof}

% \begin{lemma}
%   \label{lem:codereg-Hsigma-not-lrv}
%   If
%   \begin{itemize}
%   \item $\dom(\code) \subseteq \ta$
%   \item $\sigrets \subseteq \gsigrets$
...
%   \item $n > 0$
%   \item $\sigrets,\sigcloss\vdash\var{code}$
%   \item $\sigma \in (\sigrets \uplus \sigcloss)$
%   \end{itemize}
%   then
%   \begin{equation*}
%     \codereg{\sigrets,\sigcloss,\code,(\ta,\stkb,\gsigrets,\gsigcloss)}.H_{\mathrm{seal}}~ \sigma \not\nequal (\lrv \circ \xi)
%   \end{equation*}
% \end{lemma}
% \begin{proof}
%   Consider the following two cases:
%   \begin{itemize}
%   \item $\sigma \in \sigrets$:

%     % In this case, we can choose a $b,e$ and construct a $\hat{W}$ such that $\npair[0]{\left(\retptrd(\baddr,\eaddr),((\rw,\linear),\baddr,\eaddr,\baddr-1)\right)}$ is not in $\lrv(\xi(\hat{W}))$, but it is in $\codereg{\sigrets,\sigcloss,\code,(\ta,\stkb)}.H_{\mathrm{seal}}~ \sigma~\hat{W}$.
%     % It suffices to make choices for $b,e$ and $\hat{W}$ such that the following hold:
%     % \begin{itemize}
%     % \item $\sigrets \subseteq \gsigrets$: assumption.
%     % \item $\dom(\code) \subseteq \ta$: assumption.
%     % \item $\exists r \in \addressable{\linear,\pwpriv[\xi(\hat{W})]}$ such that $\pwpriv[\xi(\hat{W})](r).H \nequal (\stareg[(\ms_S,\ms_T),(\ta,\stkb)]{\spao,\square}, \aaddr'+\calllen)$, $\dom(\ms_S) = \dom(\ms_T) = [\baddr,\eaddr]$, $\decInstr{\code([\aaddr',\aaddr' + \calllen-1])} = \overline{\scall{\offpc,\offsigma}{r_1}{r_2}}$, $\code(\aaddr'+\offpc) = \seal{\sigma_b,\sigma_e,\sigma_b} \tand \sigma = \sigma_b + \offsigma \in \sigrets$.
%     %   It is not difficult to construct a $\hat{W}$ with a suitable $\spatialo$ region to satisfy these conditions.
%     % \end{itemize}
%     %  DOMI: what if code has no call or no seal?
%     It suffices to observe that $\npair[0]{(0,0)} \in \lrv(\xi(\hat{W}))$ but not $\npair[0]{(0,0)} \in \codereg{\sigrets,\sigcloss,\code,(\ta,\stkb,\gsigrets,\gsigcloss)}.H_{\mathrm{seal}}~ \sigma~ \hat{W}$.

%   \item $\sigma \in \sigcloss$:
%     By definition of $\codereg{}$, it suffices to show an $\npair[0]{(\vsc,\vsc')} \in (\lrvg{\trusted}(W) \setminus \lrv(W))$ with $\exec{\vsc}$.
%     If we take $\vsc=\vsc' = ((\rx,\normal),b,e,a)$ then it suffices to show that we can take $[b,e]$ such that $[b,e] \subseteq \ta$, $\npair{[b,e]} \in\xReadCond{W}$ but not $\npair{[b,e]}\in\readCond{\normal,W}$.
%     Choosing an appropriate world, this follows by definition of $\readCond{}$ and $\xReadCond{}$ from Lemma~\ref{lem:codereg-H-not-subs-lrv}.
%   \end{itemize}
% \end{proof}
...
\begin{proof}
  ...
  \begin{equation*}
    ...
    \lrheap(\pwheap)(W_{\var{heap}})
  \end{equation*}
  for some $\ms_S' \subseteq \ms_S$ and $\ms_{T,\var{heap}}\subseteq \ms_T$.

  By definition of $\npair{(\overline{\sigma},\ms_S',\ms_\var{T,heap})} \in
  \lrheap(\pwheap)(W_{\var{heap}})$, we get an
  $R_\ms : \dom(\activeReg{\pwheap}) \fun \MemSeg \times \MemSeg$ and $\exists R_W : \dom(\activeReg{\pwheap}) \fun \World$ such that
  \begin{itemize}
  \item $W' = \oplus_{r \in \dom(\activeReg{\pwheap})} R_W(r)$ and
...
% \begin{lemma}
%   \label{lem:codereg-not-read-write}
%   If
%   \begin{itemize}
%   \item $n > 0$
%   \item $\dom(\code) \subseteq \ta$
%   \item $\gc =
%     (\ta,\stkb,\gsigrets,\gsigcloss)$
%   \item $\pwheap(r_\code) \nequal
%     \codereg{\sigrets,\sigcloss,\code,\gc}$
%   \item $a \in \dom(\code)$
%   \item $\memSat{\ms_S,\stk,\ms_\stk,\ms_T}{W}$
%   \end{itemize}
%   then
%   \begin{itemize}
%   \item $\npair{\{a\}} \not\in \readCond{\normal,W}$
%     % \item $\npair{\{a\}} \not\in \writeCond{\normal,W}$
%     % It seems unnecessary to show the part about the write condition as all non-no-perm permissions have read.
%   \end{itemize}
% \end{lemma}
% \begin{proof}
%   For the sake of contradiction assume $\npair{\{a\}} \in \readCond{\normal,W}$.
%   Then there exists $S \subseteq \addressable{\lin, \pwheap}$ and $R : S \fun
%   \powerset{\nats}$ such that
%   \begin{itemize}
%   \item $\biguplus_{r\in S} R(r) \supseteq A$
%   \item $\forall r \in S \ldotp \pwheap(r).H \nsubeq \stdreg{R(r),\gc}{\pur}.H$
%   \end{itemize}
%   Because we know $\memSat{\ms_S,\stk,\ms_\stk,\ms_T}{W}$, it must be possible to split up $\ms_S$ in disjoint memory segments that each (paired with a target memory segment) satisfies exactly one region.
%   This means that only one region can govern address $a$, so that $r_\code \in S$
%   It follows that $R(r_\code) = \dom(\code)$ and $\pwheap(r_\code).H \nsubeq \stdreg{\dom(\code),\gc}{\pur}.H$.

%   Lemma~\ref{lem:code-reg-and-mem-sat} tells us that $\npair{(\code,\code)} \in \pwheap(r_\code).H~W'$ for some $W'$ with $W = W' \oplus \_$.
%   Together with the fact that $\dom(\code)\subseteq \ta$, it then follows that $\sigrets \subseteq \gsigrets$ and $\sigcloss \subseteq \gsigcloss$ and $\sigrets,\sigcloss\vdash_{\mathrm{comp-code}} \var{code}$.
%   This now gives us a contradiction with Lemma~\ref{lem:codereg-H-not-subs-lrv}.
% \end{proof}

\begin{lemma}[Untrusted source values are reasonable]
...
\begin{proof}
  ...
  \item If $w = \sealed{\sigma,\vsc}$ and $\sigma \not\in (\gsigrets\cup \gsigcloss)$ then $\vsc$ is reasonable for $n-1$ steps:
    ...
    By $\npair{(w,\_)} \in \lrv(W_w)$, we get some region $r \in \dom(\pwheap)$ and a set of return seals, a set of closure seals and a code memory: $\sigrets,\sigcloss,\mscode$ such that $\pwheap(r) = (\pure,\_,H_{\mathrm{seal}})$ and $H_{\mathrm{seal}} \nequal H^\mathrm{code,\square}_{\mathrm{seal}} \; \sigrets \; \sigcloss \; \mscode \; \gc)$ and $\npair[n']{(\sealed{\sigma,\vsc},\_)} \in H_{\mathrm{seal}} \; \sigma \; \xi^{-1}(W)$ for all $n' < n$, so in particular for $n' = n-1$.
    It follows easily from the above that also $\npair[n-1]{(\vsc,\_)} \in H^\mathrm{code,\square}_{\mathrm{seal}} \; \sigrets \; \sigcloss \; \mscode \; \gc \; \sigma \; \xi^{-1}(W)$.
...
\begin{lemma}[Untrusted register files are reasonable]
  If
  \begin{itemize}
  \item $\gc = (\ta,\stkb,\gsigrets,\gsigcloss)$
...
\begin{lemma}[Reasonable things don't need to be trusted]
  ...
  \item Theorem~\ref{thm:ftlr} holds up to $n$ steps.
    \dominique{Note: I mention this assumption explicitly to avoid cyclic reasoning in the FTLR proof.}
  \end{itemize}
  Then
  \begin{itemize}
  \item $\npair{(w,w')} \in \lrv(W_w)$
...
\begin{lemma}[Untrusted environments produce safe closures]
...
\begin{lemma}[Safe values are safe to execute]
...
\begin{lemma}[Untrusted code regions' seals specify value safety]
  ...
  Then
  \begin{equation*}
    H^\mathrm{code,\square}_{\mathrm{seal}} \; \sigrets \; \sigcloss \; \code \; (\ta,\stkb) \; \sigma \nequal \lrv
  \end{equation*}
\end{lemma}
\begin{proof}
  Follows easily by definition.
...
\begin{lemma}[Code region sealing invariant for untrusted components implies untrusted safety]
  ...
  \item $\sigma \in \sigcloss$
  \end{itemize}
  then $\codereg{\sigrets,\sigcloss,\mscode,\gc}.H_{\mathrm{seal}} \sigma \nequal \lrv$ for all $n$
\end{lemma}
\begin{proof}
  The result follows easily by definition of $\codereg{}$ and $H^\mathrm{code,\square}_{\mathrm{seal}}$.
\end{proof}
...
\begin{proof}
  Follows easily from the definitions.
  % Note: the case for execCond where \lin = \linear does not occur
...
\begin{lemma}[permission-based conditions splicable]
...
\begin{lemma}[FTLR for component code-values]
  ...
  If
  \begin{equation*}
    \sigrets,\sigrets[\mathrm{owned}],\sigcloss,\ta\vdash_{\mathrm{comp-code}} w
  \end{equation*}
  and
  \begin{itemize}
  \item $\sigrets \subseteq \gsigrets$ and $\sigcloss \subseteq \gsigcloss$
  \item $\pwheap(r_{\mathrm{code}}) = \codereg{\sigrets,\sigcloss,\mscode,\gc}$
...
\begin{proof}
  ...
  \item
    ...
    \item $\trust = \untrusted$:
      ...
      \begin{equation*}
        \forall \sigma' \in [\sigma_\baddr,\sigma_\eaddr] \ldotp \exists r \in \dom(\pwheap) \ldotp \pwheap(r) = (\pure,\_,H_{\mathrm{seal}}) \tand H_{\mathrm{seal}} \; \sigma' \nequal \lrv
      \end{equation*}
  ...
  \item $\mscode(a) \in \ints$ and
    \begin{multline*}
      ([a \cdots a + \calllen-1] \subseteq \ta \wedge\mscode([a \cdots a + \calllen-1]) = \scall[0..\calllen-1]{\offpc,\offsigma}{r_1}{r_2}) \Rightarrow\\ (\mscode(a+\offpc) = \seal{\sigma_\baddr,\sigma_\eaddr,\sigma_\baddr} \wedge \sigma_\baddr+\offsigma \in \sigrets[\mathrm{owned}])
    \end{multline*}:
...
\begin{proof}
  ...
  \item $w = \sealed{\sigma,\vsc}$ and
    ...
    \begin{itemize}
    \item $\npair[n']{(\vsc,\vsc)} \in H_{\mathrm{seal}} \; \sigma \; \xi^{-1}(W)$ for all $n' < n$:

     By definition of $\codereg{\sigrets,\sigcloss,\mscode,\gc}$ and $H^\mathrm{code,\square}_{\mathrm{seal}}$ and because we know that $\sigma\in\sigcloss$, it suffices to prove that
   ...
   \item If $\isLinear{\vsc}$, then 
     for all $W' \future W$, $W_o$, $n' < n$ and $\npair[n']{\stpair[.]{\vsc_S'}{\vsc'_T}} \in H_{\mathrm{seal}} \; \sigma \; \xi^{-1}(W_o)$, we have that 
     \begin{equation*}
       \npair[n']{(\vsc,\vsc_S',\vsc,\vsc_T')} \in \lrexj(W'\oplus W_o))
     \end{equation*}

     By definition of $\codereg{\sigrets,\sigcloss,\mscode,\gc}$ and $H^\mathrm{code,\square}_{\mathrm{seal}}$ and because we know that $\sigma\in\sigcloss$, we know that
     one of the following holds:
     \begin{itemize}
   ...
   \item 
       If $\nonLinear{\src{\vsc_S}}$ then for all $W' \future \purePart{W}$, $W_o$, $n' < n$, $\npair[n']{(\vsc_S',\vsc'_T)} \in H_{\mathrm{seal}} \; \sigma \; \xi^{-1}(W_o)$, we have that
       \begin{equation*}
...
\begin{lemma}[FTLR for component exports]
...
\begin{proof}
  ...
  \item $w = \sealed{\sigma,\vsc}$, $\vsc = ((\rx,\normal),\baddr,\eaddr,\aaddr)$, $[\baddr,\eaddr]\subseteq \dom(\mscode)$, $\sigma \in \sigcloss$:
    ...
    By definition of $\lrv$ and by choosing island $r = r_{\mathrm{code}}$, it suffices to prove that
    \begin{itemize}
    \item $\npair[n']{(\vsc,\vsc)} \in H^\mathrm{code,\square}_{\mathrm{seal}} \; \sigrets \; \sigcloss \; \code \; \gc \; \sigma \; \xi^{-1}(W)$ for all $n' < n$:

      By definition of $\codereg{\sigrets,\sigcloss,\mscode,\gc}$ and $H^\mathrm{code,\square}_{\mathrm{seal}}$ and because we know that $\sigma\in\sigcloss$, it suffices to prove that one of the following holds: 
      \begin{itemize}
      \item $(\dom(\code) \mathrel{\#} \ta$ and $\npair[n']{(\vsc,\vsc)} \in \lrv \; \xi(\xi^{-1}(W))$
    ...
    \item
      For all $W' \future \purePart{W}$, $W_o$, $n' < n$, $\npair[n']{(\vsc_S',\vsc'_T)} \in H^\mathrm{code,\square}_{\mathrm{seal}} \; \sigrets \; \sigcloss \; \code \; \gc \; \sigma \; \xi^{-1}(W_o)$, we have that
      \begin{equation*}
        \npair[n']{(\vsc,\src{\vsc_S'},\vsc,\vsc_T')} \in \lrexj(W'\oplus W_o))
      \end{equation*}
      ...
      Since $\sigma\in\sigcloss$, it follows from $\npair[n']{(\vsc_S',\vsc'_T)} \in H^\mathrm{code,\square}_{\mathrm{seal}} \; \sigrets \; \sigcloss \; \code \; \gc \; \sigma \; \xi^{-1}(W_o)$ that
      \begin{equation*}
...
\begin{lemma}[FTLR for components]
  ...
  \begin{itemize}
  \item $\gc = (\ta,\stkb,\gsigrets,\gsigcloss)$
  \item $\var{comp}$ is a well-formed component, i.e. $\ta \vdash \var{comp}$
...
\begin{proof}
  ...
  We then know from $\ta \vdash \var{comp}$ that 
  \begin{itemize}
  \item $\dom(\mscode) = [\baddr,\eaddr]$
  \item $[\baddr-1,\eaddr+1] \mathrel{\#} \dom(\msdata)$
  \item $\mspad = [\baddr-1\mapsto 0] \uplus [\eaddr+1 \mapsto 0]$
  \item $\sigrets,\sigcloss,\ta\vdash_{\mathrm{comp-code}} \mscode$
  ...
  \item
   ...
   Take $R_\ms : \dom(\activeReg{\pwheap}) \fun \MemSeg \times \MemSeg$ such that
    ...
    \end{itemize}
    Since then $W = \oplus_{r \in \dom(\activeReg{\pwheap})} R_W'(r)$, we can use Lemma~\ref{lem:oplus-future-distr} to construct an $R_W$ with
    \begin{equation*}
      W' = \oplus_{r \in \dom(\activeReg{\pwheap})} R_W(r)
    \end{equation*}
    and $R_W(r) \future R_W'(r)$ for all $r$.
    ...
    \item $\npair[n']{(\mspad\uplus\mscode,\mspad\uplus\mscode)} \in  \codereg[\mathrm{code},\square]{\sigrets,\sigcloss,\mscode,\gc}.H~ \xi^{-1}(\purePart{W'})$ for all $n' < n$:
      ...
      \item $([\baddr-1,\eaddr+1]\subseteq \ta \wedge \trust = \trusted) \vee ([\baddr-1,\eaddr+1]\mathrel{\#} \ta \wedge \sigrets = \emptyset \wedge \trust =\untrusted)$: by assumption and choice of $\trust$.
      \item $\sigrets,\sigcloss,\ta\vdash_{\mathrm{comp-code}} \code$: by assumption.
      \item $\mspad = [\baddr-1 \mapsto 0] \uplus [\eaddr + 1 \mapsto 0]$:
        by assumption
    ...
    \item
      \begin{equation*}
        \dom(\codereg[\mathrm{code},\square]{\sigrets,\sigcloss,\mscode,\gc}.H_{\mathrm{seal}}) = \sigrets\uplus \sigcloss
      \end{equation*}
...
\lau{TODO 18-07-2018: gc should probably be specified here, so they are not just picked at random.}
\lau{TODO 18-07-2018: Where is this lemma even used?}
...
\begin{proof}
  ...
  \item $[\baddr_\stk,\eaddr_\stk] = \dom(\ms_\stk )$
  \item $[\baddr_\stk-1,\eaddr_\stk+1] \mathrel{\#} (\dom(\mscode) \cup \dom(\msdata))$
  ...
  \begin{itemize}
  \item $\{(\_ \mapsto c_{\mathrm{main},c}), (\_ \mapsto c_{\mathrm{main},d})\}\subseteq \overline{\var{export}}$ and
  \item $\npair{(\var{comp}_0,\var{comp}_0)} \in \lrcomp(W)$ with
  \item  $\var{comp}_0 = (\mscode, \msdata, \overline{a_{\mathrm{import}}\mapsfrom s_{\mathrm{import}}}, \overline{s_{\mathrm{export}}\mapsto w_{\mathrm{export}}}, \sigrets, \sigcloss,A_\linear)$
  \end{itemize}
...
\begin{definition}[Source language contextual equivalence]
  ...
  \begin{equation*}
    \forall \src{\context} \ldotp \emptyset \vdash \src{\context} \Rightarrow \src{\plug{\context}{\var{comp}_1} \sterm{\ta[,1],\stkb_1}} \Leftrightarrow \src{\plug{\context}{\var{comp}_2} \sterm{\ta[,2],\stkb_2}}
  \end{equation*}
  with $\src{\ta[,i]} = \src{\dom(\var{comp}_i.\mscode)}$
  \lau{06-07-2018: TODO: Dominique, did you resolve this?}
  \dominique{and stkbase?}
\end{definition}
...
\begin{definition}[Target language contextual equivalence]
  ...
  \begin{equation*}
    \forall \trg{\context} \ldotp \emptyset \vdash \trg{\context} \Rightarrow \trg{\plug{\context}{\var{comp}_1} \term} \Leftrightarrow \trg{\plug{\context}{\var{comp}_2} \term}
  \end{equation*}
\end{definition}

\begin{theorem}
  \label{thm:full-abstraction}
  For reasonable, well-formed components $\var{comp}_1$ and $\var{comp}_2$ (with respect to $\ta[,i] = \dom(\var{comp}_i.\mscode)$, respectively), we have
  \begin{gather*}
...
\begin{proof}
  ...
  \item Consider first the upward arrow.
    ...
    Take a $\src{\context}$ such that $\emptyset \vdash \src{\context}$, take $\src{\ta[,i]}
  ...
  \item The downward arrow is similar.
    ...
    Take a $\trg{\context}$ such that $\emptyset \vdash \trg{\context}$ and we will prove that
...
\begin{lemma}
  ...
  If $r,r' \in \dom(\pwheap)$ and $\pwheap(r) = (\pure,\_,H_{\mathrm{seal}})$
  and $\pwheap(r') = (\pure,\_,H_{\mathrm{seal}}')$ and $\sigma \in \dom(H_{\mathrm{seal}})$
  and $\sigma \in \dom(H_{\mathrm{seal}}')$ and $\memSat{\ms_S,\stk,\ms_\stk,\ms_T}{W}$,
  then
  \[
    H_{\mathrm{seal}} = H_{\mathrm{seal}}' \text{ and } r = r'
  \]
\end{lemma}
\begin{proof}
...
\dominique{Why is the below lemma needed?}
\begin{lemma}
...
% At the moment of writing this, I am not sure why we thought about them, but it was related to hiding program lengths (and stack height).
