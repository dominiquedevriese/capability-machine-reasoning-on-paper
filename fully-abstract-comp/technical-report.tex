...
\begin{definition}[Reasonable configuration]
  ...
  \item \emph{Use return seals only for calls, use closure seals appropriately} If
    ...
    \item $\sigma \in \gsigcloss$ and one of the following holds:
      \begin{itemize}
      \item $\exec{\Phi'(r_2)}$ and $\Phi'(r_2)$ behaves reasonably up to $n' - i$ steps.
      \item $\nonExec{\Phi'(r_2)}$ and $\Phi'(r_2)$ is reasonable up to $n' - i$ steps in memory $\Phi'.\ms$ and free stack $\Phi'.\ms_\stk$.
      \end{itemize}
    \end{itemize}
