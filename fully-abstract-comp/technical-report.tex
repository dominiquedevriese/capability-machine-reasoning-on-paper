...
\[
  \begin{array}{l}
    ...
    \tmove{\rtmp{1}}{\pcreg}\\
    \tcca{\rtmp{1}}{\offpc} \text{ // $\offpc$ is the offset from pc to the location of the seal for this code}\\
    \tload{\rtmp{1}}{\rtmp{1}}\\
...
\begin{definition}[Never store link seals and code capability]
  A configuration $\src{\Phi}$ is said to not store seals and code capability in $\ta$ with component parameters $\sigrets$, $\sigcloss$ for $n$ steps iff\\
  for all $i \leq n$, if
  \[
    \src{\Phi} \nstep[i] \src{\Phi'}
  \]
