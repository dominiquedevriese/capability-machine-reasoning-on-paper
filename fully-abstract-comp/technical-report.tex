...
\[
  \begin{array}{rrcl}
   \addrbnf,\basebnf \in & \Addr & \defeq & \nats \\
    ...
    &\RegName & \defbnf &\pcreg \mid \rretc \mid \rretd \mid \rstk \mid \rdata \mid \dots \\
    &\Reg & \defeq & \RegName \fun \Word\\
    &\Mem & \defeq & \Addr \fun \Word \\
    &\MemSeg & \defeq & \Addr \parfun \Word \\
    {\sourcecolor{} \sstkframe \in} & {\sourcecolor{} \StkFrame} & {\sourcecolor{} \defeq} & {\sourcecolor{} \Caps \times \Caps \times \MemSeg \times \Addr}\\
...
In the source language, $\src{\Stack}$ is a call stack that contains the data for each call. The call stack consists of a number of $\src{\StkFrame}$'s that contains 1) the old pc, 2) the old data capability, 3) caller private stack, and 4) the address of the old stack pointer.
...
\[
  \begin{array}{rcl}
    n & \in & \nats \\
    \src{r} &\in &  \RegName \setminus \{\rretc, \rretd, \rstk\}\\
...
\[
  \noRetStkReg{\reg} \defeq \forall r \in \reg \setminus \{ \pcreg, \rstk, \rretc, \rretd, \rdata \} \ldotp \reg(r) \neq \stkptr{\_}
\]
...
\begin{align*}
    ...
    \begin{cases}
      (\mem,\reg',\stk',\ms_\stk') & \arraycolsep=0pt
                                      \begin{array}[t]{l}
                                        ...
                                        \update{\rretc}{\retptrc}\\
                                        \update{\rretd}{\retptrd}\\
 ...
 \\
                                        ...
                                        \stk' = (\opc,\Phi(\rdata),\ms_\priv,\perm,\stdrng,\aaddr) :: \stk \text{ and } \\
    ...
    \end{cases}
\end{align*}
\lau{do we need to require anything on the data capability we put on the call stack?}
