...
\begin{mathpar}
  ...
  \inference{
    ...
    \vdash \var{comp}_0 & (\_ \mapsto c_{\mathrm{main},c}), (\_ \mapsto c_{\mathrm{main},d}) \in \overline{\var{export}}
  }{
...
\begin{definition}[Reasonable configuration]
  ...
  \item \emph{Guarantee stack base address before call} If
    ...
    \item $r_1 \neq \rtmp{1}$
    \item $\Phi(\pcreg) + \calllen$ behaves reasonably up to $n'-1$ steps
    \end{itemize}
  \item \emph{Use return seals only for calls, use closure seals appropriately} If
    ...
    \item $\sigma \in \gsigcloss$ and one of the following holds:
      \begin{itemize}
      \item $\exec{\Phi(r_2)}$ and $\Phi(r_2)$ behaves reasonably up to $n' - 1$ steps.
      \item $\nonExec{\Phi(r_2)}$ and $\Phi(r_2)$ is reasonable up to $n' - 1$ steps in memory $\Phi.\ms$ and free stack $\Phi.\ms_\stk$.
      \end{itemize}
    \end{itemize}
  \item \emph{Don't store private stuff...} If
    ...
    \end{itemize}
    Then $\Phi.\reg(r_2)$ is reasonable in memory $\Phi.\mem$ up to $n' -1$ step
  \item \emph{Don't leak private stuff...} If
    ...
    \item All of the following hold:
      ...
      \item $\Phi'$ is reasonable up to $n'-1$ steps
      \end{itemize}
    ...
    \item
      ...
      \item $\Phi.\reg(r)$ is reasonable in memory $\Phi.\mem$ and free stack $\Phi.\ms_\stk$ up to $n'-1$ steps for all $r \neq \pcreg$
      \end{itemize}
    \item
      \begin{itemize}
      \item $\Phi$ points to $\src{\txjmp{\offpc,\offsigma}{r_1}{r_2}}$ for some $\src{r_1}$ and $\src{r_2}$
      \item $\Phi.\reg(r)$ is reasonable in memory $\Phi.\mem$ and free stack $\Phi.\ms_\stk$ up to $n'-1$ steps for all $r \neq \pcreg$
      \end{itemize}
    \end{itemize}
  \end{itemize}
  ...
  \item
   ...
   Take $R_\ms : \dom(\activeReg{\pwheap}) \fun \MemSeg \times \MemSeg$ such that
    ...
    \item $\npair{(\msdata'|_{\{\addr\}},\msdata'|_{\{\addr\}})} \in \stdreg{\{\aaddr\},\gc}{\lin}.H~ \xi^{-1}(R_W(r_\addr))$:
      ...
      \end{itemize}
    \item $\npair{(\mspad\uplus\mscode,\mspad\uplus\mscode)} \in  \codereg[\mathrm{code},\square]{\sigrets,\sigcloss,\mscode,\gc}.H~ \xi^{-1}(\purePart{W'})$:
...
and
\[
  \callCond{\Phi,r_1,r_2,\offpc,\offsigma,\aaddr}
\]
\end{definition}
...
\begin{proof}
  ...
  \item Consider first the upward arrow.
    ...
    It follows from Lemma~\ref{lem:ftlr-comps} that $\npair{(\var{comp}_1,\var{comp}_1)} \in \lrcomp[\preceq,\gc_1](W_1)$ for some $W_1$ with $\dom(\pwfree) = \dom(\pwpriv) = \emptyset$.
    It also follows from the same Lemma~\ref{lem:ftlr-comps} that $\npair{(\context,\context)} \in \lrcomp[\preceq,\gc_1](W_1')$ for some $W_1'$ that we can choose such that $W_1 \uplus W_1'$ is defined.
    ...
    It follows from Lemma~\ref{lem:ftlr-comps} that $\npair[n']{(\var{comp}_2,\var{comp}_2)} \in \lrcomp[\succeq,\gc_2](W_2)$ for some $W_2$ with $\dom(\pwfree) = \dom(\pwpriv) = \emptyset$.
    It also follows from the same Lemma~\ref{lem:ftlr-comps} that $\npair[n']{(\context,\context)} \in \lrcomp[\succeq,\gc_2](W_2')$ for some $W_2'$ that we can choose such that $W_2 \uplus W_2'$ is defined.
  ...
  \item The downward arrow is similar.
    ...
    It follows from Lemma~\ref{lem:ftlr-comps} that $\npair{(\var{comp}_1,\var{comp}_1)} \in \lrcomp[\succeq,\gc_1](W_1)$ for some $W_1$ with $\dom(\pwfree) = \dom(\pwpriv) = \emptyset$.
    It also follows from the same Lemma~\ref{lem:ftlr-comps} that $\npair{(\context,\context)} \in \lrcomp[\succeq,\gc_1](W_1')$ for some $W_1'$ that we can choose such that $W_1 \uplus W_1'$ is defined.
    ...
    It follows from Lemma~\ref{lem:ftlr-comps} that $\npair[n']{(\var{comp}_2,\var{comp}_2)} \in \lrcomp[\preceq,\gc_2](W_2)$ for some $W_2$ with $\dom(\pwfree) = \dom(\pwpriv) = \emptyset$.
    It also follows from the same Lemma~\ref{lem:ftlr-comps} that $\npair[n']{(\context,\context)} \in \lrcomp[\preceq,\gc_2](W_2')$ for some $W_2'$ that we can choose such that $W_2 \uplus W_2'$ is defined.
