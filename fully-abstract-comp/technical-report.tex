...
\[
  \stackReadCond{W} = \left\{ \npair{A} \middle| 
    \begin{array}{l}
      ...
      \qquad \biguplus_{r\in S} R(r) \supseteq A \wedge \\
      \qquad \forall r \in S \ldotp \pwfree(r) \nsubsim \stdreg{R(r),\gc}{\pur}
    \end{array}
  \right\}
\]
where $\iota_A$ is a standard region defined in Section~\ref{sec:standard-regions}.
...
\[
  \writeCond{\lin,W} = \left\{ \npair{A} \middle| 
    \begin{array}{l}
      ...
      \qquad \biguplus_{r\in S} R(r) \supseteq A \wedge\\
      \qquad (\lin = \linear \Rightarrow \forall r\ldotp |R(r)|  = 1) \wedge\\
      \qquad \forall r \in S \ldotp \pwheap(r) \nsupsim \stdreg{R(r),\gc}{\pur} \wedge\\
...
\]
\lau{02-11-2017: I am not sure whether it should be a $\normal$ region. In previous work $\nsubsim$ ignored the ``view'' which it may also do here, and then then it does not matter which one it is.}
...
\begin{lemma}
  ...
  If $\iota \nsubsim \stdreg{A,\gc}{v}$ and $\iota \nsupsim \stdreg{A,\gc}{v}$, then $\iota \nsim {A,\gc}{v}$.
\end{lemma}
\begin{proof}
  By definition.
\end{proof}
