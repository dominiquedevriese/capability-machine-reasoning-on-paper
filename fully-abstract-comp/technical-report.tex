...
\[
  \begin{array}{rrcl}
   \addrbnf,\basebnf \in & \Addr & \defeq & \nats \\
    ...
    & & & {\sourcecolor{} \; \mid \retptrd(\basebnf,\aendbnf) \mid \retptrc(\permbnf,\linbnf,\basebnf,\aendbnf,\addrbnf)}\\
...
\trg{i} &\defbnf & \tfail \mid \thalt \mid \tjmp{\trg{r}} \mid \tjnz{\trg{r}}{\trg{\rn}} \mid \tisptr{\trg{r}}{\trg{r}} \mid \tgeta{\trg{r}}{\trg{r}} \mid \tgetb{\trg{r}}{\trg{r}} \mid \\
      ...
      & &  \tload{\trg{r}}{\trg{r}} \mid \tcca{\trg{r}}{\trg{\rn}} \mid \trestrict{\trg{r}}{\trg{\rn}} \mid \tlt{\trg{r}}{\trg{\rn}}{\trg{\rn}} \mid \\
  & & \tplus{\trg{r}}{\trg{\rn}}{\trg{\rn}} \mid \tminus{\trg{r}}{\trg{\rn}}{\trg{\rn}} \mid \tsetatob{\trg{r}} \mid \txjmp{\trg{r}}{\trg{r}} \mid \tcseal{\trg{r}}{\trg{r}} \mid \\ 
...
\[
  ...
  \begin{cases}
    ...
    \src{\baddr} & \src{\text{if $\vsc = \retptrc(\_,\_,\baddr,\_,\_)$}}
  \end{cases}
\]
\[
  \pi_\eaddr(\vsc) =
  \begin{cases}
    ...
    \src{\eaddr} & \src{\text{if $\vsc = \retptrc(\_,\_,\_,\eaddr,\_)$}}
  \end{cases}
\]
...
\[
  ...
  \begin{cases}
    ...
    \src{\lin} & \src{\text{if $\vsc = \retptrc(\_,\lin,\_,\_,\_)$}}
  \end{cases}
\]
...
\begin{align*}
  \var{comp} &\mathrel{::=} (\ms_{\mathrm{code}},\ms_{\mathrm{data}},\overline{c}_{\mathrm{import}},\overline{c}_{\mathrm{export}},\sigma_\baddr,\nrcalls,\nrclosures)
\end{align*}

We define inductively when a component is valid ($\vdash \var{comp}$) by the below inference rules:
\begin{mathpar}
  \inference{}{
    A_{\mathrm{code}},A_{\mathrm{data}},\overline{c_{\mathrm{import}}},\sigma_\baddr,\nrcalls,\nrclosures \vdash_{\mathrm{comp-value}} z
  }
  \and
  \inference{
    c \in \overline{c_{\mathrm{import}}}
  }{
    A_{\mathrm{code}},A_{\mathrm{data}},A_{\mathrm{own}},\overline{c_{\mathrm{import}}},\sigma_\baddr,\nrcalls,\nrclosures \vdash_{\mathrm{comp-value}} c
  }
  \and
  \inference{
    [\baddr,\eaddr] \subseteq A_{\mathrm{code}} & \perm \sqsubseteq \rx
  }{
    A_{\mathrm{code}},A_{\mathrm{data}},A_{\mathrm{own}},\overline{c_{\mathrm{import}}},\sigma_\baddr,\nrcalls,\nrclosures \vdash_{\mathrm{comp-value}} ((\perm,\normal),\baddr,\eaddr,\aaddr)
  }
  \and
  \inference{
    [\baddr,\eaddr] \subseteq A_{\mathrm{data}} & \permbnf \sqsubseteq \rw&
    \lin = \linear \Rightarrow \emptyset \subset [\baddr,\eaddr] \subseteq A_{\mathrm{own}}
  }{
    A_{\mathrm{code}},A_{\mathrm{data}},A_{\mathrm{own}},\overline{c_{\mathrm{import}}},\sigma_\baddr,\nrcalls,\nrclosures \vdash_{\mathrm{comp-value}} ((\perm,\lin),\baddr,\eaddr,\aaddr)
  }
  \and
  \inference{
    A_{\mathrm{code}},A_{\mathrm{data}},A_{\mathrm{own}},\overline{c_{\mathrm{import}}},\sigma_\baddr,\nrcalls,\nrclosures \vdash_{\mathrm{comp-value}} \vsc \\
    \sigma = \sigma_b + o & \nrcalls \leq o < \nrcalls + \nrclosures
  }{
    A_{\mathrm{code}},A_{\mathrm{data}},A_{\mathrm{own}},\overline{c_{\mathrm{import}}},\sigma_\baddr,\nrcalls,\nrclosures \vdash_{\mathrm{comp-value}} \sealed{\sigma,\vsc}
  }
  \and
  \inference{
    \dom(\ms_{\mathrm{code}}) = [\baddr_{\mathrm{code}},\eaddr_{\mathrm{code}}] &
    \ms_{\mathrm{code}}(\baddr_{\mathrm{code}}) = \seal{\sigma_\baddr,\sigma_\baddr+\nrcalls+\nrclosures-1,\sigma_\baddr}\\
    \forall a \in [\baddr_{\mathrm{code}}+1,\eaddr_{\mathrm{code}}]\ldotp \ms_{\mathrm{code}}(a) \in \ints\\
    \exists \var{offset\_use\_site} \in [0\cdots \nrcalls-1] \fun \dom(\ms_{\mathrm{code}}) \\
    \forall a \in [\baddr_{\mathrm{code}}+1,\eaddr_{\mathrm{code}}-\calllen+1]\ldotp \ms_{\mathrm{code}}([a\cdots a+\calllen-1]) = \scall[0..\calllen-1]{o}{r_1}{r_2} \Rightarrow \var{offset\_use\_site}(o) =a\\
    \exists A_\mathrm{own} : \dom(\ms_{\mathrm{data}}) \rightarrow \powerset{\dom(\ms_{\mathrm{data}})} & \dom(\ms_{\mathrm{data}}) = \biguplus_{a \in \dom(\ms_{\mathrm{data}})} A_{\mathrm{own}}(a) \\
    \forall a \in \dom(\ms_{\mathrm{data}})\ldotp \dom(\ms_{\mathrm{code}}),\dom(\ms_{\mathrm{data}}),A_{\mathrm{own}}(a),\overline{c_{\mathrm{import}}},\sigma_\baddr,\nrcalls,\nrclosures \vdash_{\mathrm{comp-value}}\ms_{\mathrm{data}}(a)\\
    \overline{\dom(\ms_{\mathrm{code}}), \dom(\ms_{\mathrm{data}}), \emptyset, \overline{c_{\mathrm{import}}}, \sigma_\baddr, \nrcalls, \nrclosures \vdash_{\mathrm{comp-value}} c_{\mathrm{export}}}
  }{
    \vdash (\ms_{\mathrm{code}},\ms_{\mathrm{data}},\overline{c_{\mathrm{import}}},\overline{c_{\mathrm{export}}},\sigma_\baddr,\nrcalls,\nrclosures)
  }
\end{mathpar}
...
\and
  \inferrule{ (s,s') \in \phi}
            { (\spatial,s',\phi_\pub,\phi,H) \privft (\spatialo,s,\phi_\pub,\phi,H)  }
            \and
  \inferrule{ (s,s') \in \phi }
...
\begin{mathpar}
  ...
  \inferrule{ (s,s') \in \phi_\pub}
            { (\spatial,s',\phi_\pub,\phi,H) \pubft (\spatialo,s,\phi_\pub,\phi,H)  }
            \and
  \inferrule{ (s,s') \in \phi_\pub}
...
\[
  ...
  \right\}
\]
\dominique{9-3-2018: I changed the above to require that $\ms$ contains safe values by requiring that it is in Hstd. Is this reasonable?}
...
\]
where $\iota_A$ is a standard region defined in Section~\ref{sec:standard-regions}.
\dominique{Why $n-1$ above?  There is already a quantification over $n' <n$ in the memory satisfaction relation and another $n-1$ is used in $\stdreg{[\baddr',\eaddr']}{\pur}$. Also: readCond doesn't have the $n-1$.}
\[
...
\dominique{7-3-2018: is it necessary to have $n-1$ above? there is already a quantification over $n'<n$ in the memory satisfaction relation.}
...
\[
  \codereg{\ta,\sigma_\baddr,\nrcalls,\nrclosures,\code} \defeq (\pure,*,=,=,H^\mathrm{code} \; \ta \; \sigma_\baddr \; \nrcalls \; \nrclosures \; \code,H^\mathrm{code}_\sigma \; \ta \; \sigma_\baddr \; \nrcalls \nrclosures \; \code)
\]

\begin{multline*}
  H^\mathrm{code} \; \ta \; \sigma_\baddr \; \nrcalls \; \nrclosures \; \code \; s \; \hat{W} = \\\left\{
    \npair{(\code,\code)} \middle|
  \begin{multlined}
    \dom(\code) = [\aaddr,\baddr] \tand \sigma_\eaddr = \sigma_\baddr + \nrcalls+\nrclosures - 1 \tand \\
    \code(a) = \seal{\sigma_\baddr,\sigma_\eaddr,\sigma_\baddr}
  \end{multlined}
  \right\}
\end{multline*}
...
\begin{multline*}
  H^\mathrm{code}_\sigma \; \ta \; \sigma_\baddr \; \nrcalls \; \nrclosures \; \code \; s \; \sigma \; \hat{W} = \\
  \begin{array}[t]{l}
\left\{
    \begin{array}{l}
\left. \npair{\arraycolsep=0pt\left(\array{l}\retptrc(\rx,\lin,\baddr,\eaddr,\aaddr'+\calllen - 1),\\((\rx,\lin),\baddr,\eaddr,\aaddr)\endarray\right)} \middle| \right. \\
      \\
      \begin{array}{l}
        \decInstr{\code([a',a' + \calllen-1])} = \overline{\scall{o}{r_1}{r_2}} \tand \\
        \aaddr = \aaddr' + \retoffset \tand \\
        \sigma = \sigma_\baddr + o \tand o < \nrcalls\\
        \lbrack \baddr, \eaddr \rbrack = \dom(\code) \subseteq \ta
      \end{array}
    \end{array}
      \right\} \uplus \\
\left\{
...
\left. \npair{\arraycolsep=0pt\left(\array{l}\retptrd(\baddr,\eaddr),\\((\rw,\linear),\baddr,\eaddr,\aaddr)\endarray\right)} \middle| \right. \\
      ...
      \begin{array}{l}
        ...
        \quad \decInstr{\code([a',a' + \calllen-1])} = \overline{\scall{o}{r_1}{r_2}} \tand \\
        \quad \sigma = \sigma_\baddr + o \tand o < \nrcalls \tand\\
        \quad a = b-1\tand \\
        \quad \lbrack \baddr', \eaddr' \rbrack = \dom(\code) \subseteq \ta
      \end{array}
    \end{array}
      \right\} \uplus\\
    \left\{
...
\begin{theorem}[FTLR]
  ...
  \item \begin{itemize}
    ...
    \item $\trust = \trusted$
    \item the compilation assumptions on $n$, $W$, ...
    \item the full abstraction conditions (difference with the previous point?)
    \end{itemize}
  \item
    \begin{itemize}
  ...
  \[
    \npair{\arraycolsep=0pt\array{l}((\rx,\lin),\baddr,\eaddr,\aaddr),\\
      ((\rx,\lin),\baddr,\eaddr,\aaddr)\endarray} \in \lreg{\trust}(W)
  \]
  \dominique{6-3-2018: I added $\xReadCond{\lin,W}$ since we are assuming there are no rwx pointers anywhere.}
  \begin{definition}[The compilation assumptions]
...
\begin{definition}
  ...
  \[
    \src{\Phi} \nstep[i] \src{\Phi'}
  \]
  and $\src{\Phi'}$ points to $\src{\scall{o}{r_1}{r_2}}$ in $A$ for some $\src{r_1}$ and $\src{r_2}$
...
\begin{definition}[Use return seals only for call]
  A configuration $\src{\Phi}$ is said to use linker seals properly in $\ta$ with component parameters $\compbaseseal$, $\nrcalls$, and $\nrclosures$ for $n$ steps iff\\
  for all $i \leq n$, if
  \[
    \src{\Phi} \nstep[i] \src{\Phi'}
  \]
  and $\src{\Phi'}$ points to $\src{\tcseal{r_1}{r_2}}$ in $\ta$ and $\Phi'(r_1) = \seal{\sigma_\baddr,\sigma_\eaddr,\sigma}$, then either $\src{\Phi'}$ is inside $\src{\scall{o}{r_1'}{r_2'}}$ and $\compbaseseal \leq \sigma < \compbaseseal + \nrcalls$ or $\compbaseseal + \nrcalls \leq \sigma < \compbaseseal + \nrcalls + \nrclosures$.
\end{definition}
...
\begin{definition}[Never pass trusted seals accross the trusted-untrusted boundary]
  ...
  \[
    \src{\Phi}.\reg(r) = \seal{\sigma_b,\sigma_e,\_} \Rightarrow [\compbaseseal,\compbaseseal + \nrcalls + \nrclosures - 1] \# [\sigma_b,\sigma_e]
  \]
and
%  \dominique{TODO: Also not allowed: passing a capability to a code area that points (a.o.) to a memory location containing a seal like the one above.}
  ...
  and
    \begin{multline*}
      \src{\Phi}.\reg(r) = \sealed{\sigma,\vsc} \wedge \sigma \not\in [\compbaseseal,\compbaseseal + \nrcalls + \nrclosures - 1] \Rightarrow 
      \\  \left(  
      \begin{array}{r l}
        (\vsc = \seal{\sigma_b,\sigma_e,\_} & \Rightarrow [\compbaseseal,\compbaseseal + \nrcalls + \nrclosures - 1] \# [\sigma_b,\sigma_e]) \wedge\\
...
%  \dominique{TODO: Also not allowed: storing a seal or a code capability in memory.}a
\begin{definition}[Never store link seals and code capability]
  A configuration $\src{\Phi}$ is said to not store seals and code capability in $\ta$ with component parameters $\compbaseseal$, $\nrcalls$, and $\nrclosures$ for $n$ steps iff\\
  for all $i \leq n$, if
  \[
    \src{\Phi} \nstep[i] \src{\Phi'}
  \]
  and $\src{\Phi'}$ points to $\src{\tload{r_1}{r_2}}$ in $\ta$, then 
\[
  \Phi'(r_2) = \seal(\sigma_\baddr,\sigma_\eaddr,\_) \Rightarrow [\compbaseseal, \compbaseseal + \nrcalls  + \nrclosures - 1] \# [\sigma_\baddr,\sigma_\eaddr]
\]
and
\[
  \Phi'(r_2) = ((\perm,\_),\baddr,\eaddr,) \wedge \perm \sqsubseteq \rx \Rightarrow [\baddr,\eaddr] \# \ta
\]
\end{definition}
...
\dominique{19-3-2018: components should contain information about what seals they use and for what (for building return pointers or for building closures). That should facilitate defining reasonable linking (only when seals disjoint), and for defining the condition for full abstraction and the FTLR.}
\begin{definition}
  A program $\program$ with memory segment $\ms_\program$ and $\dom(\ms_\program) = \ta$, is reasonable if for all $n$ and all contexts $\context$ the configuration $\plug{\context}{\program}$ satisfies:
  \begin{itemize}
  \item $\plug{\context}{\program}$ has reasonable calls in $\dom(\ms_\program)$ for $n$ steps.
  \item $\plug{\context}{\program}$ uses linker seals properly for $n$ steps.
  \item $\plug{\context}{\program}$ never passes trusted seals across the boundary.
  \end{itemize}
\end{definition}
