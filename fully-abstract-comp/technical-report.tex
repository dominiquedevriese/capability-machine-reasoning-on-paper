...
\begin{definition}[Reasonable configuration]
  ...
  \item \emph{Use return seals only for calls, use closure seals appropriately} If
    ...
    \item $\sigma \in \gsigcloss$ and one of the following holds:
      \begin{itemize}
      \item $\exec{\Phi(r_2)}$ and $n' = 0$ or $\Phi(r_2)$ behaves reasonably up to $n' - 1$ steps.
      \item $\nonExec{\Phi(r_2)}$ and $n' = 0$ or $\Phi(r_2)$ is reasonable up to $n' - 1$ steps in memory $\Phi.\ms$ and free stack $\Phi.\ms_\stk$.
      \end{itemize}
    \end{itemize}
  \item \emph{Don't store private stuff...} If
    ...
    \end{itemize}
    Then $n' = 0$ or $\Phi.\reg(r_2)$ is reasonable in memory $\Phi.\mem$ up to $n' -1$ step
