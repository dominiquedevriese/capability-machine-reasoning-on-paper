...
\lau{10-11-2017: The above may not work well here because we do not want a $\spatialo$ region to be taken over by a $\pure$ region (as this would allow for normal capabilities addressing an area of memory that was only addressable by linear capabilities before).}
\[
...
\[
  \Region = \left\{
  \begin{array}{l}
    \{\spatial \} \times \State \times \Rels \times (\State \fun \World \fun \UPred{\MemSeg^2}) \uplus \\
    \{\spatialo \} \times \State \times \Rels \times (\State \fun \World \fun \UPred{\MemSeg^2})\uplus \\
    \{\pure \} \times \State \times \Rels \times (\State \fun \World \fun \UPred{\MemSeg^2}) \uplus\\
    \{\revoked\}
  \end{array} \right.
\]
...
\and
  \inferrule{ (s,s') \in \phi }
            { (v,s',\phi^\pub,\phi,H) \privft (v,s,\phi^\pub,\phi,H)  }
\end{mathpar}
%
Public future regions are allowed to transition according to the public part of their transition system. \lau{15-11-2017: And what more?}
\begin{mathpar}
    \inferrule{ }
            { \revoked \pubft \revoked }
\and
  \inferrule{ (s,s') \in \phi^\pub }
            { (v,s',\phi^\pub,\phi,H) \pubft (v,s,\phi^\pub,\phi,H)  }
\end{mathpar}
...
\lars{22-11-2017: typo in $k_{d,T}$ and $k_{c,T}$ ??  Seems the
  arguments to $\sealed{}$ have been swapped}
\[
