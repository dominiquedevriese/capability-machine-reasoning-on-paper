...
\paragraph{Termination computations}
                                   ...
                                   \begin{cases}
                                     ...
                                     \sourcecolor\left.
                                       \begin{array}[t]{l}
                                         ...
                                         \Phi(r_2) = \retptrd(\sigma_2,(\aaddr_\stk,\_)) \tand \\
                                         \sigma_1 = \sigma_2 \tand\\
  ...
  \begin{cases}
    ...
    \sourcecolor\left.
      \begin{array}[t]{l}
        ...
        c_2 = \retptrd(\sigma_2,(\aaddr_\stk,\_)) \tand \\
        \sigma_1 = \sigma_2 \tand\\
        \Phi(\rstk) = \stkptr{\rw,\stkb,\eaddr_\stk,\_} \tand \\
        \Phi = (\mem,\reg,\stkf::\stk,\ms_\stk) \tand \\
        \stkf = (\opc,\ms_{\stk,\priv}) \tand \\
...
\subsubsection{\color{green} first thoughts on change to future world relation}
{\color{green}
The public part of the future world relation stays the same:
\begin{mathpar}
  \inferrule{ \exists m : \RegionName \fun \RegionName, \text{ injective}\ldotp \dom(W') \supseteq \dom(m(W)) \wedge \forall r \in \dom(W)\ldotp W'(m(r)) \pubft W(r) }
            { W' \future W }
\end{mathpar}
The private future world relation becomes:
\begin{mathpar}
  \inferrule{ \exists m : \RegionName \fun \RegionName, \text{ injective}\ldotp \dom(W') \supseteq \dom(m(W)) \wedge \forall r \in \dom(W)\ldotp W'(m(r)) \privft W(r) \\  \exists r \in \dom(W') \setminus \dom(m(W))\ldotp W'(r) \text{ is non-empty and stack region}}
            { W' \future W }
\end{mathpar}
...
\begin{definition}
  We say a region $(k,s,\phi_\pub,\phi,H)$ is a stack region on a machine with starting stack domain $[\stkb,\eaddr]$ iff
\[
  \begin{array}{l}
    \forall n, \hat{W},s'\ldotp \forall \npair{\stpair{\ms}{\ms}} \in H \; s' \; \hat{W} \ldotp \dom(\src{\ms_S}) \subseteq [\stkb,\eaddr]  \wedge \dom(\ms_T) \subseteq [\stkb,\eaddr]
  \end{array}
\]
\end{definition}
}
...
\begin{align*}
  \lrexj(W) &= \left\{ \npair{\stpair[.]{v_{c,S},v_{d,S}}{v_{c,T},v_{d,T}}} \middle| 
    \begin{array}{l}
      ...
      \qquad\Rightarrow\npair[n']{\left(\Phi_S', \Phi_T' \right)}\in \lro(W \oplus W_\lrr \oplus W_\lrm)
    \end{array}
    \right\}\\
...
\[
  \lrp(W) =
    ...
    \begin{array}{l}
      ...
      \qquad \qquad \dom(\src{\ms_S}) \subseteq [\src{\baddr},\src{\eaddr}] \wedge \dom(\ms_T) \subseteq [\baddr,\eaddr]      
    \end{array}
  \right\}
\]
...
\[
  \lrr(R)(W) = \left\{ \npair{\stpair{\reg}{\reg}} \middle|
    \begin{array}{l}
      ...
      \quad W = \bigoplus_{r \in (\RegName\setminus (\{\pcreg,\rdata \} \cup R))} S(r) \\
...
%
    ...
    \begin{array}{l}
      ...
      \forall n' \leq n \ldotp \forall W' \privft W \ldotp \\
      ...
      \quad \npair[n']{\left(\arraycolsep=0pt\array{l}\retptrc(\perm_C,\lin_C,\baddr_C,\eaddr_C,\aaddr_C),\\((\perm_C,\lin_C),\baddr_C,\eaddr_C,\aaddr_C)\endarray\right)} \in \lrv(W_C) \wedge\\
      ...
      \quad W_\var{new} = (W' \uplus W_\lrr \uplus W_\lrm \uplus W_C)[\revoked,\revoked,\stdreg{[\stkb,\eaddr]}{\spao}]\\
      \qquad \Rightarrow \\
      \qquad \quad \npair[n']{\left(\arraycolsep=0pt\array{l}
                                     \left(\arraycolsep=0pt\array{l}\ms_S,\reg_S \left[
                                       \array{l}\reg.\rstk,\pcreg,\rdata,\rtmp{1},\rtmp{2}\mapsto\\\stkptr{\rw,\stkb,\eaddr,\baddr},((\perm_C,\lin_C),\baddr_C,\eaddr_C,\aaddr_C),0,0,0\endarray \right]\\
                                     ,\stk,\ms_\stk \uplus \ms_f
                                     \endarray \right),\\
                                             \left(\ms_T,\reg_S\left[\array{l}\reg.\rstk,\pcreg,\rdata,\rtmp{1},\rtmp{2}\mapsto\\((\rw,\linear),\stkb,\eaddr,\baddr),((\perm_C,\lin_C),\baddr_C,\eaddr_C,\aaddr_C+?),0,0,0\endarray\right]\right)
                                    \endarray\right)} \in \lro(W_\var{new})
    \end{array} 
    \right\}\cup \\
%
...
%
    ...
    \begin{array}{l}
      ...
      \forall W' \future W, W_o, \stpair[.]{c_S'}{\vsc'_T} \in H_\sigma \; \sigma \; W_o \ldotp \\
      \quad \npair{\src{c_S},\src{c_S'},\sealed{\sigma,\vsc_T},\sealed{\sigma,\vsc_T'}} \in \lrexj(W'\oplus W_o) \wedge \\
      \quad \npair{\src{c_S'},\src{c_S},\sealed{\sigma,\vsc_T'},\sealed{\sigma,\vsc_T}} \in \lrexj(W' \oplus W_o)
    \end{array}
    \right\}\cup\\
%
    % \left\{ \npair{\left(\arraycolsep=0pt\array{l} \src{\seal{\sigma,\sigma_\baddr,\sigma_\eaddr}},\\ \seal{\sigma,\sigma_\baddr,\sigma_\eaddr} \endarray \right)} 
...
\begin{theorem}[FTLR]
  ...
  \[
    \begin{array}{l}
      ...
      \right)\\
      \quad\Rightarrow 
      \npair{\arraycolsep=0pt\array{l}((\perm_C,\lin_C),\baddr_C,\eaddr_C,\aaddr_C),((\perm_D,\lin_D),\baddr_D,\eaddr_D,\aaddr_D),\\
...
\dominique{I'm following the example of the above FTLR by not allowing $\rwx$, but it's not clear to me why that's the right thing to do.}
\begin{theorem}[FTLR]
  \label{thm:ftlr}
  \[
    \begin{array}{l}
      \forall n,W,\perm,\lin,\baddr,\eaddr,\aaddr \ldotp \\
      \quad \left(
      \begin{array}{l}
        \perm_C = \rx \wedge\\
        \npair{(\baddr_C,\eaddr_C)} \in \readCond{\lin_C,W_C}
      ...
      \right)\\
      \quad\Rightarrow 
      \npair{\arraycolsep=0pt\array{l}((\perm_C,\lin_C),\baddr_C,\eaddr_C,\aaddr_C),\\
                      ((\perm_C,\lin_C),\baddr_C,\eaddr_C,\aaddr_C)\endarray} \in \lre(W)
