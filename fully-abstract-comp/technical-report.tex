...
\begin{proof}
  ...
  \item $w = ((\perm,\normal),\baddr,\eaddr,\aaddr)$, $[\baddr,\eaddr]\subseteq A_{\mathrm{code}}$, $\perm \sqsubseteq \rx$:
    By definition of $\lrv(W)$, it suffices to prove that $\npair{[\baddr,\eaddr]}\in\readCond{\normal,W}$, $\npair{[\baddr,\eaddr]}\in\execCond{\normal,W}$ and $\npair{[\baddr,\eaddr]}\in\xReadCond{\normal,W}$.
    
    \dominique{TODO: readCond probably not fine: the code of a component may contain stuff that must remain private.
      We have to prevent that a component exports capabilities with authority over its code memory (directly or indirectly)?}
    TODO
