...
\[
  \begin{array}{rcl}
    o & \in & \nats \\
    \src{i} & \defbnf &  \trg{i} \mid \scall{\offpc,\offsigma}{r}{r}
  \end{array}
\]
...
\begin{align*}
  \var{comp} &\mathrel{::=} (\ms_{\mathrm{code}},\ms_{\mathrm{data}},\overline{c}_{\mathrm{import}},\overline{c}_{\mathrm{export}},\sigrets,\sigcloss)
\end{align*}

We define inductively when a component is valid ($\vdash \var{comp}$) by the below inference rules:
\begin{mathpar}
  \inference{
    \mscode(a) = \seal{\sigma_\baddr,\sigma_\eaddr,\sigma_\aaddr} & [\sigma_\baddr,\sigma_\eaddr] \subseteq (\sigrets \cup \sigcloss)
  }{
    \sigrets,\sigcloss \vdash_{\mathrm{comp-code}} \mscode,a
  }
  \and
  \inference{
    \mscode(a) \in \ints\\
    \mscode([a \cdots a + \calllen-1]) = \scall[0..\calllen-1]{\offpc,\offsigma}{r_1}{r_2} \Rightarrow (\mscode(a+\offpc) = \seal{\sigma_\baddr,\sigma_\eaddr,\sigma_\aaddr} \wedge \sigma_\baddr+\offsigma \in \sigrets)
  }{
    \sigrets,\sigcloss \vdash_{\mathrm{comp-code}} \mscode,a
  }
  \and
  \inference{
    ...
    d_\sigma(a),\sigcloss \vdash_{\mathrm{comp-code}} \mscode,a
  }{
  ...
  \inference{
    ...
    A_{\mathrm{code}},A_{\mathrm{data}},\overline{c_{\mathrm{import}}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} z
  }
  \and
  \inference{
    c \in \overline{c_{\mathrm{import}}}
  }{
    A_{\mathrm{code}},A_{\mathrm{data}},A_{\mathrm{own}},\overline{c_{\mathrm{import}}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} c
  }
  \and
  \inference{
    [\baddr,\eaddr] \subseteq A_{\mathrm{code}} & \perm \sqsubseteq \rx
  }{
    A_{\mathrm{code}},A_{\mathrm{data}},A_{\mathrm{own}},\overline{c_{\mathrm{import}}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} ((\perm,\normal),\baddr,\eaddr,\aaddr)
  }
  \and
  \inference{
    ...
    A_{\mathrm{code}},A_{\mathrm{data}},A_{\mathrm{own}},\overline{c_{\mathrm{import}}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} ((\perm,\lin),\baddr,\eaddr,\aaddr)
  }
  \and
  \inference{
    A_{\mathrm{code}},A_{\mathrm{data}},A_{\mathrm{own}},\overline{c_{\mathrm{import}}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} \vsc \\
    \sigma \in \sigcloss
  }{
    A_{\mathrm{code}},A_{\mathrm{data}},A_{\mathrm{own}},\overline{c_{\mathrm{import}}},\sigrets,\sigcloss \vdash_{\mathrm{comp-value}} \sealed{\sigma,\vsc}
  }
  \and
  \inference{
    \dom(\mscode) \mathrel{\#} \dom(\msdata)\\
    \sigrets,\sigcloss \vdash_{\mathrm{comp-code}} \mscode \\
    \exists A_\mathrm{own} : \dom(\msdata) \rightarrow \powerset{\dom(\msdata)} & \dom(\msdata) = \biguplus_{a \in \dom(\msdata)} A_{\mathrm{own}}(a) \\
    \forall a \in \dom(\msdata)\ldotp \dom(\mscode),\dom(\msdata),A_{\mathrm{own}}(a),\overline{c_{\mathrm{import}}},\sigrets,\sigcloss \vdash \msdata(a)\\
    \overline{\dom(\mscode), \dom(\msdata), \emptyset, \overline{c_{\mathrm{import}}}, \sigrets,\sigcloss \vdash_{\mathrm{comp-value}} c_{\mathrm{export}}}\\
  }{
...
\begin{mathpar}
  \inference{
    \var{comp}_1 = (\mscode[1], \msdata[1], \overline{c_{\mathrm{import},1}}, \overline{c_{\mathrm{export},1}}, \sigrets[1], \sigcloss[1])\\
    \var{comp}_2 = (\mscode[2], \msdata[2], \overline{c_{\mathrm{import},2}}, \overline{c_{\mathrm{export},2}}, \sigrets[2], \sigcloss[2])\\
    \var{comp}_3 = (\mscode[3], \msdata[3], \overline{c_{\mathrm{import},3}}, \overline{c_{\mathrm{export},3}}, \sigrets[3], \sigcloss[3])\\
...
\begin{definition}[Programs and Contexts]
  We define a program as a $(\var{comp},c_{\mathrm{main}})$ component with an empty import list and $c_{\mathrm{main}}$ is one of the exported symbols.

  Given a component $\var{comp}$, a context is a tuple $(\var{comp'},c_{\mathrm{main}})$ such that $\var{comp'}$ is a component and $(\var{comp} \bowtie \var{comp'},c_{\mathrm{main}})$ is a program.
\end{definition}
...
\[
  \comp{\scall{\offpc,\offsigma}{r_1}{r_2}} = 
  \begin{array}[t]{l}
...
\[
  \codereg{\ta,\sigrets,\sigcloss} \defeq (\pure,*,=,=,H^\mathrm{code} \; \ta \; \sigrets \; \sigcloss \; \code,H^\mathrm{code}_\sigma \; \ta \; \sigrets \; \sigcloss \; \code)
\]
...
\begin{theorem}[FTLR]
  ...
  \item \begin{itemize}
    ...
    \item For all $\npair{\stpair{\reg}{\reg}} \in \lrrtrusted(W)$ and $\memSat{\src{\ms_S},\src{\ms_\stk},\src{\stk},\ms_T}{W}$ and $\ms_f$, we have $\src{\Phi} = (\src{\ms_S} \uplus \ms_f, \src{reg_S}, \src{\stk}, \src{\ms_\stk})$ and 
      \[
        \sigrets = \bigcup_{r \in \dom(\pwheap)} \{\sigma \mid \pwheap(r) = \codereg{\ta',\sigrets',\_} \wedge \sigma \in \sigrets'  \wedge \ta' \subseteq \ta\}
      \]
      and
      \[
        \sigcloss = \bigcup_{r \in \dom(\pwheap)} \{\sigma \mid \pwheap(r) = \codereg{\ta',\_,\sigcloss'} \wedge \sigma \in \sigcloss'  \wedge \ta' \subseteq \ta\}
      \]
      \begin{itemize}           
\item $\Phi$ checks $\stkb$ before calls in $\ta$ for $n$ steps (Definition~\ref{def:check-stack-addr-before-call}).
  \item $\Phi$ uses link seals properly in $\ta$ with component parameters $\sigrets$ and $\sigcloss$ for $n$ steps (Definition~\ref{def:use-return-seals-call}).
  \item $\Phi$ handle trusted seals properly in $\ta$ with components parameters $\sigrets$ and $\sigcloss$ for $n$ steps (Definition~\ref{def:handle-trusted-seals-properly}).
  \item $\Phi$ does not store link seals and code capabilities in $\ta$ with component parameters $\sigrets$ and $\sigcloss$ for $n$ steps (Definition~\ref{def:never-store-seal-code-cap}).
      \end{itemize}
    \end{itemize}
  \item
    \begin{itemize}
