...
\begin{multline*}
  ...
  (\State \fun \Seal \parfun \World \monnefun \URel{\Caps \times \SealableCaps})
\end{multline*}
and
\[
  \Regions = \left\{
...
%
    ...
    \begin{array}{l}
      ...
      \quad \forall W' \future W, W_o, n' < n, \stpair[.]{\vsc_S'}{\vsc'_T} \in H_\sigma \; s \; \sigma \; W_o \ldotp \\
...
\[
  \codereg{\ta,\sigma_\baddr,\sigma_\eaddr,\code} \defeq (\pure,*,=,=,H^\mathrm{code} \; \ta \; \sigma_\baddr \; \sigma_\eaddr \; \code,H^\mathrm{code}_\sigma \; \ta \; \sigma_\baddr \; \sigma_\eaddr \; \code)
\]

\[
  H^\mathrm{code} \; \ta \; \sigma_\baddr \; \sigma_\eaddr \; \code \; s \; \hat{W} = \{\npair{(\code,\code)} | \dom(\code) = [\aaddr,\baddr] \tand \code(a) = \seal{\sigma_\baddr,\sigma_\baddr,\sigma_\eaddr}\}
\]
\lau{19-03-2018: Add the link capability to the above? code in ta?}
\[
  H^\mathrm{code}_\sigma \; \ta \; s \; \sigma \; \hat{W} = 
  \begin{array}[t]{l}
\left\{
    \begin{array}{l}
\left. \npair{\arraycolsep=0pt\left(\array{l}\sealed{\sigma,\retptrc(\rx,\lin,\baddr,\eaddr,\aaddr'+\calllen - 1)},\\((\rx,\lin),\baddr,\eaddr,\aaddr)\endarray\right)} \middle| \right. \\
      \\
      \begin{array}{l}
...
\left\{
    \begin{array}{l}
\left. \npair{\arraycolsep=0pt\left(\array{l}\sealed{\sigma,\retptrd(\baddr,\eaddr)},\\((\rx,\linear),\baddr,\eaddr,\aaddr)\endarray\right)} \middle| \right. \\
      \\
      \begin{array}{l}
        \exists r \in \dom(\pwpriv[\xi(\hat{W})]) \ldotp \pwpriv[\xi(\hat{W})](r) \nequal (\stareg[(\ms,\ms)]{\spa}, ((\rx,\lin),\baddr',\eaddr',\aaddr')) \tand \\
        \quad \dom(\ms) = [\baddr,\eaddr] \tand\\
        \quad \decInstr{\code([a',a' + \calllen]-1)} = \overline{\scall{o}{r_1}{r_2}} \tand \\
        \quad \aaddr = \aaddr' + \calllen \tand \\
        \quad \sigma = \sigma_\baddr + o \tand\\
    ...
    \left\{
    \begin{array}{l}
\left. \npair{\arraycolsep=0pt\left(\array{l}
      \sealed{\sigma,\vsc}, \vsc' \endarray\right)} \middle| \right. \\
      \\
      \begin{array}{l}
        \sigma \in [\sigma_\baddr,\sigma_\eaddr] \tand\\
...
Similarly, rather than invoking \sreturn{}, one would ccall \lau{now xjmp?} the $\retptrd$ and $\retptrc$ sealed code/data pair.
