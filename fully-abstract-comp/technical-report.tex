...
\item clear the temporaryu register.
...
\lau{06-11-2017: If we need to introduce revoked regions like we hade for the local capabilities, we may consider the following: For the future world relation require the existance of a bijection from region names to region names. For all regions in the present world the future region is given by the bijection. This would mean that reinstantiation would happen simply by giving a bijection that maps to the new region. In other words, the weird shadowing would not be necessary.
  \begin{mathpar}
...
\lau{03-11-2017: TODO take a look at linearity of this definition - in paticular the recovered pc and stack pointer.}
\[
  \lrk(W) = \left\{ \npair{\left(\array{l}\src{\retptrd(\sigma,(\aaddr_d,\eaddr_d))},\\\src{\retptrc(\sigma,(\perm,\lin,\baddr_c,\eaddr_c,\aaddr_c))},\\k_{d,T},k_{c,T}\endarray\right)} \middle|
    \begin{array}{l}
      ...
      \qquad\dom(\ms_\stk) = [\baddr_\stk,\eaddr_\stk] \wedge \\
      \qquad\eaddr_\stk = \aaddr_d-1\\
      \qquad\Rightarrow\\
      \qquad\npair[n']{\left(\array{l}\src{\left(\arraycolsep=0pt\array{l}\ms_S,\reg_S\updReg{\pcreg,\rstk}{\opc,\stkptr{\rw,\baddr_\stk,\eaddr_d,\_}},\\\stk, \ms_\stk \uplus \ms_{\stk,f}\endarray\right)},\\
                                          (\ms_T,\reg_T\updReg{\pcreg,\rstk}{\opc,((\rw,\lin),\baddr_\stk,\eaddr_d,\_)})\endarray\right)}
...
\[
  ...
  \begin{cases}
    \{ r \mid W(r) = (\normal,\_) \} & \text{if $\lin = \normal$} \\
    \{ r \mid W(r) = (\pureo,\_) \}  & \text{otherwise (i.e. $\lin = \linear$)} \\
  \end{cases}
\]
...
\lau{01-11-2017: TODO Write down address-stratified condition.}
