...
\begin{mathpar}
  ...
  \inference{
    ...
    \overline{c_{\mathrm{export}}} \mathrel{\#} \overline{c_{\mathrm{import}}}
  }{
...
\[
  \execCond{\lin,W} = \left\{ \npair{(\baddr,\eaddr)} \middle|
    \begin{array}{l}
      ...
      \quad \forall n' < n, W' \future W_l, \aaddr \in [\baddr',\eaddr'] \subseteq [\baddr,\eaddr] \ldotp\\
...
\begin{definition}[World specifies component]
  ...
  \begin{itemize}
  \item $\pwheap(r_i) = \stdreg{A_i,\gc}{\lin}$ with $\lin = \pure$ or $\lin = \spatialo$
...
\begin{lemma}[Compatibility lemma for linking]
  If
  \begin{itemize}
  \item $\npair{(\var{comp}_1,\var{comp}_1)} \in \lrcomp(W)$
  \item $\npair{(\var{comp}_2,\var{comp}_2)} \in \lrcomp(W)$
  \end{itemize}
  then
  \begin{itemize}
  \item $\npair{(\var{comp}_1 \bowtie \var{comp}_2,\var{comp}_1\bowtie\var{comp}_2)} \in \lrcomp(W)$.
  \end{itemize}
\end{lemma}
\begin{proof}
...
\begin{lemma}[readCond shrinkable]
  \label{lem:readCond-shrinkable}
  If $\npair{A} \in \readCond{\lin,W}$  and $A' \subseteq A$, then
...
\begin{lemma}[readXCond shrinkable]
  \label{lem:readXCond-shrinkable}
  If $\npair{A} \in \xReadCond{\lin,W}$  and $A' \subseteq A$, then
...
\begin{proof}
  ...
  \item $\trust = \untrusted$ and $\dom(\mscode \mathrel{\#} \ta)$:
    ...
    \item $\npair{[b,e]}\in\xReadCond{\normal,W}$:
      follows from the definition of $\xReadCond$, together with the assumption that $\pwheap(r_{\mathrm{code}}) = \codereg{\sigrets,\sigcloss,\mscode,\gc}$.
    \item $\npair{[b,e]}\in\execCond{\normal,W}$:
      take $n' < n$, $W' \future \purePart{W}$, $\aaddr' \in [\baddr',\eaddr'] \subseteq [\baddr,\eaddr]$ and $w' = ((\rx,\normal),\baddr',\eaddr',\aaddr')$.
      Then we need to show that $\npair[n']{(w',w')} \in \lre(W')$.
      This now follows immediately from the (regular) FTLR (Theorem~\ref{thm:ftlr}), using the two above points, Lemma~\ref{lem:downwards-closed} and Lemmas~\ref{lem:readCond-shrinkable} and~\ref{lem:readXCond-shrinkable}.
    \end{itemize}

  \item $\trust = \trusted$ and $\dom(\mscode) \subseteq \ta$:
    By definition of $\lrvg{\trusted}(W)$, it suffices to show that
    $\npair{[b,e]}\in\xReadCond{\normal,W}$.

    This follows immediately from the definition of $\xReadCond$, together with the assumption that $\pwheap(r_{\mathrm{code}}) = \codereg{\sigrets,\sigcloss,\mscode,\gc}$.
  \end{itemize}
\end{proof}
...
\begin{lemma}[FTLR for component data-values]
  ...
  \item $\npair[n']{(c_{\mathrm{import}},c_{\mathrm{import}})} \in \lrv(W)$ for all $n' < n$
...
\begin{proof}
  ...
  We know that $W \vdash \var{comp}$ by assumption, so it remains to prove that 
  for all  $W' \future W$ with 
  \begin{equation*}
    \overline{\npair[n']{(c_{\mathrm{import}},c_{\mathrm{import}})}} \in \lrv(\purePart{W'}) \text{ for all $n' < n$}
  \end{equation*}
  we have that
  ...
  TODO
  \dominique{Something missing: we should be able to subdivide the world according to how linear authority is distributed in the component well-formedness judgement, but the world-specifies-component relation does not give us this.}
\end{proof}
...
\section{Lemmas}
\subsection{Well-behaved configurations}
\begin{lemma}
  \label{lem:check-stack-addr-before-call-dc}
For $n' \leq n$, if a configuration $\src{\Phi}$ is said to check $\stkb$ before calls in $\ta$ for $n$ steps, then $\src{\Phi'}$ is said to check $\stkb$ before calls in $\ta$ for $n'$ steps
\end{lemma}
\begin{proof}
  \lau{TODO}
\end{proof}
...
\begin{lemma}
  \label{lem:use-return-seals-call-dc}
  For $n' \leq n$, if a configuration $\src{\Phi}$ is said to use linker seals properly in $\ta$ with component parameters $\sigrets$, $\sigcloss$ for $n$ steps, then $\src{\Phi}$ is said to use linker seals properly in $\ta$ with component parameters $\sigrets$, $\sigcloss$ for $n'$ steps.
\end{lemma}
\begin{proof}
  \lau{TODO}
\end{proof}
...
\begin{lemma}
  \label{lem:handle-trusted-seals-prop-dc}
  For $n' \leq n$, if a configuration $\src{\Phi}$ is said to handle trusted seals properly in $\ta$ with component parameters $\sigrets$, $\sigcloss$ for $n$ steps, then $\src{\Phi}$ is said to handle trusted seals properly in $\ta$ with component parameters $\sigrets$, $\sigcloss$ for $n'$ steps.
\end{lemma}
\begin{proof}
  \lau{TODO}
\end{proof}
...
\begin{lemma}
  \label{lem:never-store-seal-code-cap-dc}
  For $n' \leq n$, if a configuration $\src{\Phi}$ is said to not store link seals and code capability in $\ta$ with component parameters $\sigrets$, $\sigcloss$ for $n$ steps, then $\src{\Phi}$ is said to not store link seals and code capability in $\ta$ with component parameters $\sigrets$, $\sigcloss$ for $n'$ steps.
\end{lemma}
\begin{proof}
  \lau{TODO}
\end{proof}
