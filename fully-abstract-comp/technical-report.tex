...
\begin{definition}[Reasonable configuration]
  ...
  \item \emph{Use return seals only for calls} If
    \begin{itemize}
    \item $\Phi \nstep[i]{\ta,\stkb} \Phi'$
    \item $\src{\Phi'}$ points to $\src{\tcseal{r_1}{r_2}}$ in $\ta$ and
      $\Phi'(r_1) = \seal{\sigma_\baddr,\sigma_\eaddr,\sigma}$
    \item $0 < i \le n$
    \end{itemize}
  ...
  \item \emph{Don't leak private stuff...} If
    ...
    \item $0 < i \le n$
    \end{itemize}
    Then $\Phi.\reg(r)$ is reasonable in memory $\Phi'.\mem$ up to $n$ steps for
