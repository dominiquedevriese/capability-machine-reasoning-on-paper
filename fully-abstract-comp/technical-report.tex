...
\begin{mathpar}
  \inferrule{ \exists m : \RegionName \fun \RegionName, \text{ injective}\ldotp \dom(W') \supseteq \dom(m(W)) \wedge \forall r \in \dom(W)\ldotp W'(m(r)) \privft W(r) \\  \exists r \in \dom(W') \cap \dom(W)\ldotp W'(r) \text{ is non-empty and stack region}}
            { W' \future W }
\end{mathpar}
...
\[
  \begin{array}{l}
    \forall n, \hat{W},s'\ldotp \forall \npair{\stpair{\ms}{\ms}} \in H \; s' \; \hat{W} \ldotp \dom(\src{\ms_S}) \subset [\stkb,\eaddr]  \wedge \dom(\ms_T) \subset [\stkb,\eaddr]
  \end{array}
\]
\end{definition}
