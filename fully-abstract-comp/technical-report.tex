...
\newcommand{\stpair}[3][]{
\ifthenelse{\equal{#1}{}}
{\left(\src{#2}_S,#3_T\right)}
{\left(\src{#2},#3\right)}}


\newcommand{\memSat}[4][n]{#2 :_{#1}^{#3}#4}
...
\[
  \lre(W) = \left\{ \npair{\stpair{v}{v}} \middle| 
    \begin{array}{l}
      ...
      \quad\src{\reg_S}(\rretd,\rretc,\pcreg) = \src{k_{d,S}}, \src{k_{c,S}}, \src{v_S} \wedge\\
      \quad\reg_T(\rretd,\rretc,\pcreg) = k_{d,S}, k_{c,S}, v_T \wedge\\
      \quad\npair[n']{\stpair[.]{(k_{d,S},k_{c,S})}{(k_{d,T},k_{c,T})}} \in \lrk(W) \wedge \\
      \quad\stpair{\reg}{\reg}\ldotp \npair[n']{
      \quad\left(\array{l}\src{\reg_S\updReg{\pcreg,\rretd,\rretc}{v_S,k_{d,S},k_{c,S}}},\\\reg_T\updReg{\pcreg,\rretd,\rretc}{v_T,k_{d,T},k_{c,T}}\endarray\right)} \in \lrr(W) \wedge\\
      \quad\src{\ms_S},\ms_T \ldotp \memSat[n']{\stpair[.]{\ms}{\ms}}{\stpair{\reg}{\reg}}{W} \\
      \quad\Rightarrow\\
      \quad\npair[n']{\left(\array{l}\src{(\reg_S\updReg{\pcreg}{v_S}\updReg{r_\ret}{k_V},\ms_S)},\\(\reg_T\updReg{\pcreg}{v_T}\updReg{r_\ret}{k_T},\ms_T)\endarray\right)}
      \in \lro(W)
    \end{array}
            \right\}
\]
...
\[
  \lro(W) = \left\{ \npair{\stpair[.]{(\reg_S,\ms_S,\stk_S,\ms_{\stk,S})}{\reg_R,\ms_R}} \middle|
    \begin{array}{l}
      \forall i \leq n, \stpair{\mem}{\mem} \ldotp \\
      \quad \src{(\reg_S,\ms_S \cup \mem_S,\stk_S,\ms_{\stk,S})} \term[i] \Rightarrow (\reg_R,\ms_R) \term\\
      \quad \tand\\
      \quad (\reg_R,\ms_R) \term[i] \Rightarrow \src{(\reg_S,\ms_S \cup \mem_S,\stk_S,\ms_{\stk,S})} \term
    \end{array}
\right\}
\]
...
\[
  \lrk(W) = \left\{ \npair{\stpair[.]{\retptrd{\sigma},\retptrc{\sigma}}{k_{d,T},k_{c,T}}} \middle|
    \begin{array}{l}
