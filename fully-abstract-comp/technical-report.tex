...
\begin{proof}
  ...
  From the fact that $\npair{(\code,\code)} \in \pwheap(r_\code).H~W'$ for some $W'$ with $W = W' \oplus \_$, and the fact that $\dom(\code)\subseteq \ta$, it also follows that $\sigrets \subseteq \gsigrets$ and $\sigcloss \subseteq \gsigcloss$ and $\sigrets,\sigcloss\vdash_{\mathrm{comp-code}} \var{code}$.
