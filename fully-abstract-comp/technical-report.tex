...
\left. \npair{(\vsc, \vsc' )} \middle| \right. \\
      \begin{array}{l}
        ...
        (\dom(\code) \subseteq \ta \tand \sigcloss \subseteq \gsigcloss \npair{(\vsc,\vsc')} \in \lrvtrusted \; \xi(\hat{W}))
      \end{array}
    \end{array}
  \right\}\\
...
\begin{lemma}
  ...
  If $\dom(\code) \subseteq \ta$, $n > 0$ and $\sigma \in (\sigrets \uplus \sigcloss)$ and $\sigrets \subseteq \gsigrets$ and $\sigcloss \subseteq \gsigcloss$, then $\codereg{\sigrets,\sigcloss,\code,(\ta,\stkb,\gsigrets,\gsigcloss)}.H_\sigma \sigma \not\nequal \lrv$.
\end{lemma}
\begin{proof}
  TODO
\end{proof}
...
\begin{lemma} 
  ...
  $\npair{(\code,\code)} \in H^\mathrm{code} \; \sigrets \; \sigcloss \; \code \; \gc \; \xi^{-1}(W')$
\end{lemma}
\begin{proof}
  TODO
\end{proof}
