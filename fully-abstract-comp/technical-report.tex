...
\begin{lemma}
  ...
  \item $n > 0$
  \end{itemize}
  then, with respect to $\code,\ta,\sigrets,\sigcloss$,
  $w$ is reasonable up to $n$ steps in memory $\Phi.\mem$.
\end{lemma}
\begin{proof}
  We need to prove four implications:
  \begin{itemize}
  \item If $w = \seal{\sigma_\baddr,\sigma_\eaddr,\_}$, then $[\sigma_\baddr,\sigma_\eaddr] \mathrel{\#} (\sigrets \cup \sigcloss)$:

    Take $\sigma \in [\sigma_b,\sigma_e]$, we need to prove that $\sigma \not\in (\sigrets \cup \sigcloss)$.
  ...
  \item If $w = \sealed{\sigma,\vsc}$ and $\sigma \not\in (\sigrets\cup \sigcloss)$ then $\vsc$ is reasonable: By $\npair{(w,\_)} \in \lrv(W_w)$, we get some region $r \in \dom(\pwheap)$ and a set of return seals, a set of closure seals and a code memory: $\sigrets',\sigcloss',\mscode'$ such that $\pwheap(r) = (\pure,\_,H^\mathrm{code,\square}_\sigma \; \sigrets' \; \sigcloss' \; \mscode' \; \gc)$ and $\npair[n']{(\sealed{\sigma,\vsc},\_)} \in H^\mathrm{code,\square}_\sigma \; \sigrets' \; \sigcloss' \; \mscode' \; \gc \; \sigma \; \xi^{-1}(W) \text{ for all $n' < n$}$.
    ...
    \begin{itemize}
    \item $\dom(\code') \mathrel{\#} \ta \tand \npair[n']{(\vsc,\vsc')} \in \lrv \;
      \xi(\hat{W})$; or
    \item $\dom(\code') \subseteq \ta \tand \npair[n']{(\vsc,\vsc')} \in \lrvtrusted \; \xi(\hat{W})$
    \end{itemize}
    \lau{At this point, it seems like should have done the proof by induction,
      but this would not be possible with the $n>0$ assumption (at least not if
      we want to use the IH). In the latter case, it is probably difficult to
      show that the something is in the trusted value-relation. If we assume
      $\ta = \dom(\code)$, then this may be discharged by contradiction (we may
      somewhere need to assume that $\ta \neq \emptyset$).}
    TODO
