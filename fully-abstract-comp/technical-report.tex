...
\item If $w = ((\perm,\_),\baddr,\eaddr,\_)$ and $\perm \in \readAllowed{}$ and $n > 0$, then $\ms(\aaddr)$ is readable up to $n - 1$ steps for all $\aaddr \in ([\baddr,\eaddr] \setminus \ta)$
\end{itemize}

We say that an executable capability $c = ((\perm,\normal),\baddr,\eaddr,\aaddr)$ and data capability $c_{\mathrm{data}}$ behaves reasonably with stack assumption $\ms_\stk'$ up to $n$ steps if the following implication holds:

For any $\Phi$ such that
  \begin{itemize}
  \item $\Phi.\reg(\pcreg) = c$
  \item $\Phi.\reg(\rdata) = c_{\mathrm{data}}$
  ...
  \item $\Phi.\ms_\stk|_{\dom(\ms_\stk')} = \ms_\stk'$
  ...
  \item \emph{Guarantee stack base address before call} If
    ...
    \item $r_1 \neq \rtmp{1}$
    \item $\Phi'(\pcreg) + \calllen$ and $\Phi'(\rstk)$ behave reasonably with stack assumption $\Phi.\ms_\stk|_{[\eaddr_\stk,\aaddr_\stk]}$ up to $n-i$ steps
    \end{itemize}
