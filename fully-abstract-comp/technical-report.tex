...
\begin{definition}[Programs and Contexts]
  We define a program to be either a component $\var{comp}$ or a tuple $(\var{comp}, c_{\mathrm{main},c}, c_{\mathrm{main},d})$ with component $\comp$ with an empty import list and $c_{\mathrm{main},c}$ and $c_{\mathrm{main},d}$ are two of the exported symbols.

A context is a program defined as above.
...
Two programs $\var{comp}$ and $(\var{comp}', c_{\mathrm{main},c}, c_{\mathrm{main},d})$ can be linked into a new program $(\var{comp} \bowtie \var{comp'}, c_{\mathrm{main},c}, c_{\mathrm{main},d})$.
\end{definition}
\dominique{A program is intuitively a component that is ready to be run.  From that point of view, I don't see the point of a program without main. I would propose changing the definitions: a program should be a component that is ready to be executed, i.e. it must have an empty import list and a pair of capabilities to be used as main. A component can be either a component with a main (i.e. a program that still needs to be linked with library implementations) or one without (i.e. a library implementation that will be used by other components). A context for a given component is any other component that can be linked with it to produce a program.}
...
\lau{TODO: Component logical relation. Rough sketch is in white-board-notes/leuven-23-3-2018-3.jpg (top left).}
\dominique{TODO: Compatibility lemma for linking: if $c_1$ and $c_1'$ are related and $c_2$ and $c_2'$, then so should be $c_1\bowtie c_2$ and $c_1' \bowtie c_2'$ (if defined).}
\lau{TODO: Version of FTLR for programs.}
\lau{TODO: Compatibility lemma for initial executable configuration construct, i.e. a lemma that says ``if two programs are related, then their initial executable configurations will be related''.}
\lau{TODO: A logical relation for executable configurations.}
\lau{TODO: contextual equivalence should be defined in terms of initial executable configuration.}
