...
\begin{definition}[Use return seals only for call]
  ...
  \[
    \src{\Phi} \nstep[i] \src{\Phi'}
  \]
  and $\src{\Phi'}$ points to $\src{\tcseal{r_1}{r_2}}$ in $\ta$ and $\Phi'(r_1) = \seal{\sigma_\baddr,\sigma_\eaddr,\sigma}$, then either $\src{\Phi'}$ is inside $\src{\scall{o}{r_1'}{r_2'}}$ and $\sigma_\baddr \leq \sigma \leq \sigma_\baddr + \nrcalls$ or $\sigma_\baddr + \nrcalls < \sigma \leq \sigma_\baddr + \nrcalls + \nrclosures+1$.
\end{definition}
