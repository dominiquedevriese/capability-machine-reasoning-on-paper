...
Take a block of code memory $\mscode$, a set of trusted addresses $\ta$ and sets of return pointer and closure seals $\gsigrets$ and $\sigcloss$.
We define that a word $w$ is reasonable up to $n$ steps in memory $\ms$ if $n=0$
...
\item If $w = ((\perm,\_),\baddr,\eaddr,\_)$ and $\perm \neq 0$ and $\gsigrets \neq \emptyset$, then $[\baddr,\eaddr] \# \dom(\mscode)$
...
We say that an executable capability $c = ((\perm,\normal),\baddr,\eaddr,\aaddr)$ behaves reasonably up to $n$ steps if for any $\Phi$ such that
  ...
  \item $\Phi.\reg(r)$ is reasonable up to $n$ steps in memory $\Phi.\mem$ for all $r \not\in \{\pcreg\}$
  \item $\Phi.\ms_\stk$ is reasonable up to $n-1$ steps in memory $\Phi.\mem$ for all $a$
...
\begin{lemma}
  ...
  $w$ is reasonable up to $n$ steps in memory $\Phi.\mem$.
\end{lemma}
\begin{proof}
  Induction over $n$.
  ...
  \item If $w = ((\perm,\_),\baddr,\eaddr,\_)$ and $\perm \in \readAllowed{}$ and $n > 0$, then $\ms(\aaddr)$ is reasonable up to $n - 1$ steps for all $\aaddr \in ([\baddr,\eaddr] \setminus \ta)$:
    ...
    From $\npair[n]{[b,e]} \in \readCond{\perm,\lin}$ we get $S\subseteq
    \addressable{\lin,W}$ and $R : S \fun \powerset{\nats}$ such that
    \begin{itemize}
    ...
    By $\memSat{\ms_S,\stk,\ms_\stk,\_}{W_M}$, we get $R_\ms : \dom(\activeReg{\pwheap}) \fun \MemSeg \times \MemSeg$ such that $\ms_T = \biguplus_{r \in \dom(\activeReg{\pwheap})} \pi_2(R_\ms(r))$ and $\ms_S = \biguplus_{r \in \dom(\activeReg{\pwheap})} \pi_1(R_\ms(r))$.
    ...
    This means that for $n'' < n'$, we have $\npair[n'']{\pi_1(R_\ms(r))(a)} \in \lrv(R_W'(a))$ where $R_W' : [b,e] \fun \World$ such that $\biguplus_{a \in \dom(\pi_1(R_\ms(r)))} R_W'(a) = R_W(r)$.

    \lau{At this point we would like to use the IH, but we do not have $R_W'(a) \uplus W_M$ defined because $R_W'(a)$ is a split of $W_M$ (in the case where the capability is $\linear$, in the other case we are fine.).}
...
\begin{proof}
  ...
  \item $\Phi.\reg(r)$ is reasonable up to $n$ steps in memory $\Phi.\mem$ for all $r \not\in \{\pcreg\}$:
    ...
    We have that $W = \bigoplus_{r \in \RegName \setminus \{\pcreg\}} S(r)$ and $\npair{(\reg_S(r),\reg_T(r))} \in \lrv(S(r))$ for all $r \not\in \{\pcreg\}$.
