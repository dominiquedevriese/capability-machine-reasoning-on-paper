...
\usepackage[a3paper,margin=0.7in]{geometry}
...
\[
  \Worldh = \RegionName \fun (\Regions + \Regionh)
\]
and
\[
  \Worlds = \RegionName \fun (\Regions \times \SealableCaps)
\]
and
\[
  \Worldfs = \RegionName \fun \Regions
\]
where $\RegionName = \nats$.
\[
  \Regionh = 
...
\[
  ...
  \left\{
    \begin{array}{l}
      ...
      \quad \memSatStack{\stk,\ms_\var{T,stack}}{W} \wedge \\
      \quad \memSatFStack{\ms_\stk,\ms_\var{T,free\_stack}}{W} \wedge \\
      \quad \memSatHeap{\ms_S,\ms_\var{T,heap}}{W}
    \end{array}
  \right.
\]
...
\[
  ...
  \left\{
    \begin{array}{l}
      W_\var{stack} = \pi_2(W) \wedge \\
      \stk = (\opc_0,\ms_0), \dots (\opc_m,\ms_m) \wedge \\
      \exists R_\ms : \dom(W_\var{stack}) \fun \MemSeg \times \SealableCaps \times \MemSeg \ldotp \\
      \quad \ms_T = \biguplus_{r \in \dom(W_\var{stack})} \pi_3(R_\ms(r)) \wedge \\
      \quad \\
      \quad \exists R_W : \dom(W_\var{stack}) \fun World \ldotp \\
      \qquad \forall r \in \dom(W_\var{stack}), n' < n \ldotp \\
      \qquad \quad \npair[n']{(\pi_1(R_\ms(r)),\pi_3(R_\ms(R))} \in W_\var{stack}(r).H \; W_\var{stack}(r).s \; \xi^{-1}(R_W(r)) \\
      \qquad \quad \pi_2(R_\ms(r)) = W_\var{stack}(r).\opc
...
\[
  \memSatHeap{ms_\stk,\ms_T}{W} \text{ iff } 
  \left\{
    \begin{array}{l}
      \dots
    \end{array}
