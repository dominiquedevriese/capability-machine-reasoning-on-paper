...
\begin{proof}
  ...
  \end{itemize}
  \item $\overline{\npair{(c_{\mathrm{export}},c_{\mathrm{export}})}} \in \lrv(\purePart{W'})$: TODO
...
\[
  \lrec(W) = \left\{ \npair{\left(
    ...
    \begin{array}{l}
      ...
      \npair{\stpair{\reg}{\reg}} \in \lrrg{\trust}(\{\rdata\})(W_R)
    \end{array}
            \right\}
\]
...
\begin{proof}
  ...
  \begin{itemize}
  \item 
    $\npair{( (\reg_S(\pcreg),\reg_S(\rdata)), (\reg_T(\pcreg),\reg_T(\rdata)) )} \in \lrexj(W_\pcreg)$:
    TODO
  \item $\memSat{\ms_S,\ms_\stk,\stk,\ms_T}{W_M}$:
    ...
    \begin{itemize}
    \item $\memSatStack{\emptyset,\emptyset}{\purePart{W_M}}}$
    \item $\memSatFStack{\ms_\stk,\ms_\stk}{\purePart{W_M}}$
    \end{itemize}
    ...
    \begin{equation*}
      \npair{(\ms_\stk|_{\{a\}},\ms_\stk|_{\{a\}})} \in \stdreg{\{a\},\gc}{\spatialo}.H~ \xi^{-1}(\purePart{W_M})
    \end{equation*}
    By definition, it suffices to show that for all $n' < n$, we have that:
    \begin{equation*}
      \npair{(\ms_\stk(a),\ms_\stk(a)) \in \lrv(\purePart{W_M})}
    \end{equation*}
