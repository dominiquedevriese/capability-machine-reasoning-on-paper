...
%   then
%   \begin{equation*}
%     \codereg{\sigrets,\sigcloss,\code,(\ta,\stkb,\gsigrets,\gsigcloss)}.H_\sigma~ \sigma \not\nequal (\lrv \circ \xi)
%   \end{equation*}
% \end{lemma}
% \begin{proof}
...
%     % In this case, we can choose a $b,e$ and construct a $\hat{W}$ such that $\npair[0]{\left(\retptrd(\baddr,\eaddr),((\rw,\linear),\baddr,\eaddr,\baddr-1)\right)}$ is not in $\lrv(\xi(\hat{W}))$, but it is in $\codereg{\sigrets,\sigcloss,\code,(\ta,\stkb)}.H_\sigma~ \sigma~\hat{W}$.
...
%     It suffices to observe that $\npair[0]{(0,0)} \in \lrv(\xi(\hat{W}))$ but not $\npair[0]{(0,0)} \in \codereg{\sigrets,\sigcloss,\code,(\ta,\stkb,\gsigrets,\gsigcloss)}.H_\sigma~ \sigma~ \hat{W}$.
