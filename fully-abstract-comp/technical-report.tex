...
\newcommand{\pureo}{\mathrm{pure\_owned}}
\newcommand{\normReg}{\mathrm{normal}}
...
\dominique{26-9-2017: I think we should allow $\rstk$ in the source language too.}
\lau{28-09-2017: I can see from the commit history that this was something when we had push/pop instructions as well as a return instruction. I have removed the restriction on $\rstk$.}
...
\lau{27-09-2017: In what sense? Do you want to allow them (I do not thinkn they are allowed at the moment?)}
...
The $\decInstr{}$ function must be surjective and injective for all non-$\tfail$ instructions. The $\encInstr{}$ function must be injective. Futher $\decInstr{}$ must be the left inverse of $\encInstr{}$ that is for all $i \in \Instr$
\[
...
\[
  ...
  \begin{array}{l}
    ...
    \{\pureo \} \times \State \times \Rels \times (\State \fun \World \fun \UPred{\MemSeg^2})\\
    \{\normReg \} \times \State \times \Rels \times (\State \fun \World \fun \UPred{\MemSeg^2})
  \end{array} \right.
\]
where $\pure$ and $\pureo$ are regions governing segments of memory addressed by linear capabilities. $\pureo$ signifies that this region is addressable. $\pure$ signifies that the region is not owned and can thus not be addressed. Finally, $\normReg$ signifies that the region is only addressed by non-linear capabilities. Notice that no region allows for both linear and non-linear capabilities to address it.
\lau{26-20-2017: We may also want to build the infrastructure for the stack more explicitely into the worlds (but maybe we can handle it in the memory satisfaction).}
...
Disjoint union of worlds. Joins together two alike worlds with strictly disjoint ownership over $\pureo$-regions.
\[
  W_1 \uplus W_1 = W
  \begin{array}[t]{l}
...
\begin{align*}
  (\normReg,\var{sm}) \oplus (\normReg,\var{sm}) =  & \; (\normReg,\var{sm}) \\
  (\pureo,\var{sm}) \oplus (\pure,\var{sm}) = & \; (\pure,\var{sm}) \oplus (\pureo,\var{sm})\\
                                           =  & \; (\pureo,\var{sm}) \\
  (\pure,\var{sm}) \oplus (\pure,\var{sm}) =  & \; (\pure,\var{sm}) \\
\end{align*}
and for all other cases $\oplus$ is undefined. Specifically, $\oplus$ is not defined when both sides are a $\pureo$-region. It is further not defined if the two sides do not agree on region type and state machine. Here $\var{sm}$ is the state machine of the region.
...
\[
  \memSat{\ms_S,\ms_\stk,\stk}{W} \text{ iff } 
  \left\{
    \begin{array}{l}
      ...
      \quad \qquad W = \biguplus_{r \in \dom(W)} R_W(r) \wedge \\
      \quad \qquad \forall r \in \dom(W) , n' < n \ldotp \\
      \qquad \qquad \npair[n']{R(r)} \in W(r).H \; (W(r).s) \; (\xi^{-1} (R_W (r))) 
    \end{array}
  \right.
\]
In the definition of memory satisfaction, $R_\ms$ acts as a partitioning in the two related memories. As the source language is a special interpretation of the target language, part of the target memory needs to relate to the stack in the source language. We also need to split up the world which is done by $R_W$.We split up the world in order to make sure that the linearity constraints impossed by the world are satisfied.


\subsection{Relation}
\lau{19-10-2017: Rest of source configuration needs to come from somewhere in $\lre$.}
\lau{19-10-2017: Maybe linearity should be taken care of in just one place (instead of in $\lrr$ relation and mem staisfaction). That is, add a predicate that checks whether the configuration satisfies linearity (such a condition may be to syntactic to work?).}
...
,T},k_{c,T},k_{d,T}})\endarray\right)}
      \in \lro(W\uplus W_\lrk \uplus W_\lrr \uplus W_\lrm)
    \end{array}
    \right\}
\]
\lau{24-10-2017: Dominique, I do not see how the splitting of worlds is going to work out. I can see how it will enforce linearity, but is you give a "pure" region (the kind of region that is only allowed to go to one part of a split) to the $\lrr$-relation, then the memory satisfation does not have it, and it cannot find a memory segment for this part. In other words, the memory satisfaction will not be able to find a concrete memory segment for segments governed by a linear capability in the register file. Maybe the memory satisfaction should have all of the parts, so it can represent them (but also limit what linear capabilities can be used).}
...
\lau{19-10-2017: $\lrk$ may indicate that $\lre$ should really be a relation between a \src{source pc} and a pair of a target pc and stack pointer? Otherwise, what happens with the data part of the return pointer?}
\[
  \lrk(W) = \left\{ \npair{\left(\array{l}\src{\retptrd(\sigma,\_)},\\\src{\retptrc(\sigma,\_)},\\k_{d,T},k_{c,T}\endarray\right)} \middle|
    \begin{array}{l}
      k_{d,T} \text{ is a stack pointer with correct end and such with everything related to the source stack} \\
      k_{c,T} \text{ relates to the old pc on the stack}\\
      ...
      \qquad\memSat[n']{\stpair[.]{\ms_S,\stk,\ms_\stk}{\ms_T}}{W_\lrm} \\
      \qquad\Rightarrow\\
      \qquad\npair[n']{\left(\array{l}\src{(\ms_S,\reg_S\updReg{\pcreg,\rstk}{\opc,\stkptr{\dots}},\stk, \ms_\stk)},\\
                                          (\ms_T,\reg_T\updReg{\pcreg,\rstk}{\dots,\dots})\endarray\right)}
      \in \lro(W' \uplus W_\lrr \uplus W_\lrm)
    \end{array}
  \right\}
\]
...
\[
  \lrr(W) = \left\{ \npair{\stpair{\reg}{\reg}} \middle|
    \begin{array}{l}
      ...
      \quad \forall r \in \RegName \setminus \{\pcreg, \rretd,\rretc \}\ldotp\\
      \qquad\npair{\stpair[.]{\src{\reg_S(r)}}{\reg_T(r)}} \in \lrv(S(r))
    \end{array}
            \right\}
\]
Where $S\text{ is a separation of $W$}$ means that no two partion share the same ``pure'' region (whatever that is).
...
\[
  ...
  \begin{array}[t]{l}
    ...
    \forall n'<n, W' \pubft W, W_\lrv \npair[n']{\stpair[.]{\retptrd(\sigma,\_)}{\sealed{\sigma,\vsc_D}}} \in \lrv(W_\lrv) \ldotp\\
      \npair[n']{\retptrc(\sigma,\_),\retptrd(\sigma,\_),\sealed{\sigma,\vsc_C},\sealed{\sigma,\vsc_D}} \in \lrk(W' \uplus W_\lrv)
    \end{array}
    \right\}\cup\\
    ...
    \forall n'<n, W' \pubft W, W_\lrv \npair[n']{\stpair[.]{\retptrc(\sigma,\_)}{\sealed{\sigma,\vsc_C}}} \in \lrv(W_\lrv) \ldotp\\
      \npair[n']{\src{\retptrc(\sigma,\_)},\src{\retptrd(\sigma,\_)},\sealed{\sigma,\vsc_C},\sealed{\sigma,\vsc_D}} \in \lrk(W' \uplus W_\lrv)
    \end{array}
    \right\}\cup \\
    \left\{ \npair{\left(\array{l}\src{\sealed{\sigma,\vsc_S}},\\ \sealed{\sigma,\vsc_T} \endarray\right)} \middle|
    \begin{array}{l}
      
    \end{array}
    \right\}\cup\\
    \left\{ \npair{\left(\arraycolsep=0pt\array{l} \src{\stkptr{\perm,\baddr,\eaddr,\aaddr}},\\ ((\perm,\linear),\baddr,\eaddr,\aaddr) \endarray \middle| \right)}
    \right\}
  \end{array}
\]
...
\item Considered PORs for transition systems to: ($a \rightarrow b \Rightarrow a \rightarrow a \text{ and } b \rightarrow b$)
  ...
  \lau{28-09-2017: I think the next three points is the answer. We basically wanted a formalism in which we could design one region that would govern the stack. I do not remeber why it was made possible with PORs.}
  \begin{itemize}
...
One way to solve this problem is to make the "top addess" of the stack known. There are many ways to do this, but we have chosen to do the following:
\begin{itemize}
