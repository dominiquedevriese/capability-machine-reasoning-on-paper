...
\begin{proof}
  ...
  Take $\aaddr\in\dom(\mscode)$, $n'<n$.
  By Lemmas~\ref{lem:purePart-duplicable}, \ref{lem:oplus-future} and \ref{lem:monotonicity}, it now suffices to prove that $\npair[n']{(\ms_S(\aaddr),\ms_T(\aaddr))} \in \lrv(\purePart{\hat{W}})$.

  From $\sigrets,\sigcloss \vdash_{\mathrm{comp-code}} \mscode$ and the fact that $\sigrets = \emptyset$, we know that for any $\aaddr\in\dom(\mscode)$, we have that $\emptyset,\emptyset,\sigcloss \vdash_{\mathrm{comp-code}} \mscode,\aaddr$.
  By definition, this last judgement implies that one of the following cases holds:
  \begin{itemize}
  \item $\mscode(a) \in\ints$ and an omitted extra fact.  It follows trivially that $\npair[n']{(\ms_S(\aaddr),\ms_T(\aaddr))} \in \lrv(\purePart{\hat{W}})$.
  \item 
    $\mscode(a) = \seal{\sigma_\baddr,\sigma_\eaddr,\sigma_\baddr}$ and $[\sigma_\baddr,\sigma_\eaddr] \subseteq \sigcloss$.

    By definition of $\lrv$, it suffices to show that $\exists r \in
    \dom(\pwheap)$ such that $\pwheap(r) = (\pure,\_,H_\sigma)$ and $H_\sigma \;
    \sigma' \nequal \lrv$.

    \dominique{TODO: we need to know that our code region is in $\hat{W}$?}

    Then we can use Lemma~\ref{lem:untrusted-codereg-sealed-vals-safe}.
  \end{itemize}
\end{proof}
