...
\newcommand{\pureReg}{\mathrm{pure}}
\newcommand{\normalReg}{\mathrm{normal}}
\newcommand{\State}{\mathrm{State}}
\newcommand{\Rels}{\mathrm{Rels}}
\newcommand{\UPred}[1]{\mathrm(#1)}

\newcommand{\future}{\sqsupseteq}
\newcommand{\privft}{\future^{\mathrm{priv}}}
\newcommand{\pubft}{\future^{\mathrm{pub}}}
...
\subsection{Worlds}
\[
  \World = \RegionName \parfun 2 \times \Region
\]
where $\RegionName = \nats$. $2$ indicates ownership of the region which decides whether a capability is allowed to reference this part of memory.
\lau{26-10-2017: Maybe we want to place the ownership directly on the $\pureReg$-regions as we only want to enforce that linear capabilities don't have any aliases.}
\[
  \Region = \left\{
  \begin{array}{l}
    \{\pureReg \} \times \State \times \Rels \times (\State \fun \World \fun \UPred{\MemSeg^2})\\
    \{\normalReg \} \times \State \times \Rels \times (\State \fun \World \fun \UPred{\MemSeg^2})
  \end{array} \right.
\]
...
Disjoint union of worlds. Joins together two alike worlds with strictly disjoint ownership over $\pureReg$-regions.
\[
  W_1 \uplus W_1 = W
  \begin{array}[t]{l}
...
\begin{align*}
  (i,\normalReg,\var{sm}) \oplus (j,\normalReg,\var{sm}) =  & \; (\max(i,j),\normalReg,\var{sm}) \\
  (1,\pureReg,\var{sm}) \oplus (0,\pureReg,\var{sm}) =      & \; (1,\pureReg,\var{sm}) \\
  (0,\pureReg,\var{sm}) \oplus (1,\pureReg,\var{sm}) =      & \; (1,\pureReg,\var{sm}) \\
  (0,\pureReg,\var{sm}) \oplus (0,\pureReg,\var{sm}) =      & \; (0,\pureReg,\var{sm}) \\
\end{align*}
and for all other cases $\oplus$ is undefined. Specifically, $\oplus$ is not defined when both sides are an owned $\pureReg$-region. It is further not defined if the two sides do not agree on region type and state machine.
...
\[
  \lrr(W) = \left\{ \npair{\stpair{\reg}{\reg}} \middle|
    \begin{array}{l}
      ...
      \quad \forall r \in \dom(\reg_T) \setminus \{\pcreg, \rretd,\rretc \}\ldotp\\
