...
Copy paste from work on ftlr proof case: 
    ...
    In the latter case, we get $S \subseteq \addressable{\linear,\pwheap{R_R(r_3)}}$ and $R : S \fun \powerset{R_R(r_3)}$ such that $\forall r \in S \ldotp |R(r)| = 1$ and $\biguplus_{r\in S} R(r) \supseteq [\baddr,\eaddr]$.
    ...
    \item if $\perm \in \readAllowed$, then
      \begin{itemize}
      \item $\npair{[\baddr,s]} \in \readCond{\linear,R_R'(r_1)}$
      \item $\npair{[s+1,\eaddr]} \in \readCond{R_R'(r_2)}$
      \end{itemize}
      Both follow from the assumption $\npair{[\baddr,\eaddr]} \in \readCond{\linear,R_R(r_3)}$, and the fact that $R_R'(r_i), i=1,2$ owns the appropriate regions.
    \item if $\perm \in \writeAllowed$, then
      \begin{itemize}
      \item $\npair{[\baddr,s]} \in \writeCond{\linear,R_R'(r_1)}$
      \item $\npair{[s+1,\eaddr]} \in \writeCond{\linear,R_R'(r_2)}$
      \end{itemize}
      Both follow from the assumption $\npair{[\baddr,\eaddr]} \in \writeCond{\linear,R_R(r_3)}$, and the fact that $R_R'(r_i), i=1,2$ owns the appropriate regions.
    \item $\perm = \rwx$
    ...
    We also trivially have $\npair[n-1]{(0,0)} \in \lrv{R_R'(r_3)}$ which shows the content of register $r_3$ to be safe.
    ...
    \item if $\perm \in \readAllowed$, then
      \begin{itemize}
      \item $\npair[n-1]{[\baddr,s]} \in \readCond{\normal,R_R'(r_1)}$
      \item $\npair[n-1]{[s+1,\eaddr]} \in \readCond{R_R'(r_2)}$
      \end{itemize}
      This follows by assumption, Lemma~\ref{lem:conds-shrinkable}, Lemma~\ref{lem:non-linear-pure}, and Lemma~\ref{lem:monotonicity}.
    \item if $\perm \in \writeAllowed$, then
      \begin{itemize}
      \item $\npair[n-1]{[\baddr,s]} \in \writeCond{\normal,R_R'(r_1)}$
      \item $\npair[n-1]{[s+1,\eaddr]} \in \writeCond{\normal,R_R'(r_2)}$
      \end{itemize}
      This follows by assumption, Lemma~\ref{lem:conds-shrinkable}, Lemma~\ref{lem:non-linear-pure}, and Lemma~\ref{lem:monotonicity}.
    \item $\perm \neq \rwx$

      By assumption.
    \item if $\perm = \rx$, then 
      \begin{itemize}
      \item $\npair[n-1]{[\baddr,s]} \in \xReadCond{R_R'(r_1)}$
      \item $\npair[n-1]{[s+1,\eaddr]} \in \xReadCond{R_R'(r_2)}$
      \item $\npair[n-1]{[\baddr,s]} \in \execCond{R_R'(r_1)}$
      \item $\npair[n-1]{[s+1,\eaddr]} \in \execCond{R_R'(r_2)}$
      \end{itemize}
      All of the above follow by assumption, Lemma~\ref{lem:conds-shrinkable}, Lemma~\ref{lem:non-linear-pure}, and Lemma~\ref{lem:monotonicity}.
    \end{itemize}

    Finally, we have
    \[
      \npair[n-1]{\Phi_S'(r_3),\Phi_T'(r_3)}\in \lrv{R_R'(3)}
    \]
...
TODO
  ...
  copy paste from FTLR proof

      We need to redistribute the ownership previously owned by the world used for $r_3$ to $r_1$ and $r_2$.

    From $\npair{(\Phi_S(r_3),\Phi_S(r_3))} \in \lrvg{\trust}(R_R(r_3))$, we get that either $\perm = \noperm$, in which case we can pick the ownership distribution to be the same as before, or $\npair{[\baddr,\eaddr]} \in \stackReadCond{R_R(r_3)}$.

    In the latter case, we get $S \subseteq \addressable{\linear,\pwfree{R_R(r_3)}}$ and $R : S \fun \powerset{R_R(r_3)}$ such that $\forall r \in S \ldotp |R(r)| = 1$ and $\biguplus_{r\in S} R(r) \supseteq [\baddr,\eaddr]$.
    ...
    \item if $\perm \in \readAllowed$, then
      \begin{itemize}
      \item $\npair{[\baddr,s]} \in \stackReadCond{\linear,R_R'(r_1)}$
      \item $\npair{[s+1,\eaddr]} \in \stackReadCond{R_R'(r_2)}$
      \end{itemize}
      Both follow from the assumption $\npair{[\baddr,\eaddr]} \in \stackReadCond{\linear,R_R(r_3)}$, and the fact that $R_R'(r_i), i=1,2$ owns the appropriate regions.
    \item if $\perm \in \writeAllowed$, then
      \begin{itemize}
      \item $\npair{[\baddr,s]} \in \stackWriteCond{\linear,R_R'(r_1)}$
      \item $\npair{[s+1,\eaddr]} \in \stackWriteCond{\linear,R_R'(r_2)}$
      \end{itemize}
      Both follow from the assumption $\npair{[\baddr,\eaddr]} \in \stackWriteCond{\linear,R_R(r_3)}$, and the fact that $R_R'(r_i), i=1,2$ owns the appropriate regions.
    \end{itemize}
    ...
    \[
      \npair[n-1]{(\Phi_S'(r), \Phi_T'(r))} \in \lrvg{\trust}(R(r))
    \]
    by assumption and Lemma~\ref{lem:downwards-closed}.
...
\begin{proof}
  ...
  \dominique{How do we know $\ms_\code'$ contains [b,e] entirely?
    It does seem plausible that it contains the call entirely as well as $a + \offpc$ thanks to the code region's well-formedness.
  }
...
TODO
  \item Case \ref{case:ftlr:move-cca-etc:splice-normal}:
    ...
    If the permission is $\noperm$, then it is trivial to show.

    Assuming the permission is not $\noperm$, we know $\perm \in \readAllowed{}$.

    If we further assume $\isLinear{\Phi_S(r_2)}$
    \begin{itemize}
    \item $\npair{[\baddr_2,\eaddr_2]} \in \readCond{\linear,R_R(r_2)}$
    \item $\npair{[\eaddr_2+1,\eaddr_3]} \in \readCond{\linear,R_R(r_3)}$
    \end{itemize}
    From this we get $S_2 \subseteq \addressable{\linear,\pwheap{R_R(r_2)}}$ and
    $R_2 : S_2 \fun \powerset{\nats}$ such that $\forall r \in S_2 \ldotp
    |R_2(r)| = 1$ (and so on) as well as $S_3 \subseteq \addressable{\linear,\pwheap{R_R(r_3)}}$ and
    $R_3 : S_3 \fun \powerset{\nats}$ such that $\forall r \in S_3 \ldotp
    |R_3(r)| = 1$ (and so on).

    In $R_R'$ give $r_1$ ownership of $R_2^{-1}([\baddr_2,\eaddr_2])$ and $R_3^{-1}([\baddr_2+1,\eaddr_3])$ and removed it from the respective regions that owned it before.
    The remaining ownership remains unchanged.
    
    Now we need to show
    \[
      \npair[n-1]{[\baddr_2,\eaddr_3]} \in \readCond{\linear,R_R(r_2)}
    \]
    which we do by picking $S' = S_2 \cup S_3$ and $R'(r) = R_2(r)$ for $r \in S_2$ and $R'(r) = R_3(r)$ for $r \in S_3$.
    With these choices, the rest follow by the readCondition assumptions and downwards-closure.

    If $\perm \in \writeAllowed{}$, then we need to show
    \[
      \npair[n-1]{[\baddr_2,\eaddr_3]} \in \writeCond{\linear,R_R(r_2)}
    \]
    which is done in a similar fashion\footnote{We do technically need to argue that the readCondition and the writeCondition agree on the set of regions we got from the readCondition.
      This is done using the memory satisfaction assumption.}.

    Using the same ownership distribution, it is trivial to show
    \[
      \npair[n-1]{(\Phi_S'(r_i),\Phi_T'(r_i))} \in \lrv(R'_R(r_i)) \text{ for $i
        \in \{1,2\}$}
    \]
    as the pair is $(0,0)$.

    For $\neg \isLinear{\Phi_S(r_2)}$, we use $R_R' = R_R$ as the distribution function.
    In this case, for $r_1$ it works like before, i.e.\ the result follows from the assumptions.
    However, in the case where the permission is $\rx$, we also have to show the following:
    \begin{itemize}
    \item $\npair[n-1]{[\baddr_2,\eaddr_3]} \in \xReadCond{R_R'(r_1)}$

      By assumption we have
      \begin{itemize}
      \item $\npair{[\baddr_2,\eaddr_2]} \in \xReadCond{\linear,R_R(r_2)}$
      \item $\npair{[\eaddr_2+1,\eaddr_3]} \in \xReadCond{\linear,R_R(r_3)}$
      \end{itemize}
      From which the result follows.
    \item $\npair[n-1]{[\baddr_2,\eaddr_3]} \in \execCond{R_R'(r_1)}$
