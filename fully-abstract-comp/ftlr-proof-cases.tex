...
\begin{proof}
  ...
  Further by $\memSat{\ms_S,\stk,\ms_\stk,\ms_T}{W}$, we know that for one $r \in S$ there is a $\mscode'\uplus\mspad \subseteq \mscode$, call this $r$ for $r_{\mscode'}$,\footnote{Follows from $H^\var{code}$ requiring $\dom(\mscode') \subseteq \ta$ and $\dom(\mscode) = \ta$ and finally that $\mscode$ is part of $\ms_S$.}
  such that
  \begin{equation}
    \npair{(\mscode'\uplus\mspad,\mscode'\uplus\mspad)} \in H^{\var{code}} \; \sigrets' \; \sigcloss' \; \mscode' \; (\ta,\_,\gsigrets,\gsigcloss)\label{eq:ftlr:rretd:Hcode}
  \end{equation}
...
\item $\dom(\mscode'\uplus\mspad) \subseteq \ta$
  ...
  Otherwise we would have $\dom(\mscode') \# \ta$ which would contradict $\dom(\mscode'\uplus\mspad) \subseteq \dom(\mscode) \subseteq\ta$
