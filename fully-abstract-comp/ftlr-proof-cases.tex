...
\begin{lemma}
  ...
  \item $\memSat{\Phi_S.\mem,\Phi_S.\ms_\stk,\Phi_S.\stk,\Phi_T.\mem}{W_M}$
...
\begin{proof}
  ...
  By general induction on $n$, i.e. we can assume that the lemma already holds for $n' < n$.
  ...
  \lau{TODO: look through this case and find the place where the ''no seal cross
    boundary thing'' should have been used. (I have done this, and it does not
    seem necessary (?))}
...
\lau{TODO I think this case should follow from $H^\var{code}$ and the comp-code judgement, but it does not.
    ...
    \item $\reg_S^{(3)}(\rstk) = \stkptr{\perm_\var{ret\_\stk},\baddr_\var{ret\_\stk},\eaddr_\var{ret\_\stk},\_}$ and $\baddr_\var{ret\_\stk} = \stkb$ and $\eaddr_\var{ret\_\stk} +1 = \baddr_\stk' $ and $\perm_\var{ret\_\stk} = \rw$ and $\baddr_\var{ret\_\stk} \leq \eaddr_\var{ret\_\stk}$.
      ...
      \item
        ...
        \[
          \npair[n''']{(((\rx,\normal),\baddr,\eaddr,\aaddr+\calllen),((\rx,\normal),\baddr,\eaddr,\aaddr+\calllen))} \in \lreg[\trusted](W''')
        \]
...
\begin{proof}[Proof of Theorem~\ref{thm:ftlr}]
  Note first that if $\trust = \untrusted$, then $\npair{[\baddr,\eaddr]} \in \readCond{\lin,W}$ follows by Lemma~\ref{lem:xReadCond-outside-ta-implies-readCond}.

  By general induction over $n$\footnote{if $n=0$, then we have a contradiction with $\Phi_S\term[i]$ and $\Phi_T\term[i]$ when we get to $\lro$.}.
  Assume
  \begin{itemize}
...
By Lemma~\ref{lem:ftlr-internal-lemma}, it suffices to show that:
\begin{itemize}
\item $\Phi_S$ is reasonable up to $n'$ steps:
  TODO
\item $\trust = \trusted \vee \npair[n']{[\baddr,\eaddr]} \in \readCond{\lin,W_\pcreg}$:
  TODO
\item $\Phi_S(\pcreg) = \Phi_T(\pcreg) = ((\rx,\lin),\baddr,\eaddr,\aaddr)$:
  TODO
\item $\npair[n']{[\baddr,\eaddr]} \in \xReadCond{W_\pcreg}$:
  TODO
