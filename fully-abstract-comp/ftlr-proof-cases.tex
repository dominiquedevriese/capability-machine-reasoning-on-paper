...
\begin{lemma}[Non-expansiveness of relations]
  ...
  If $W \nequal W'$, then
  \begin{itemize}
  \item $\xReadCond{\lin,W} \nequal \xReadCond{\lin,W'}$.
  \item ...
  \end{itemize}
\end{lemma}
...
\begin{proof}
  ...
  \nsubsim \stdreg{R(r),\gc}{\pur}$ for all $r \in S$.
  ...
  Because $\pwheap(r) \nsubsim \stdreg{R(r),\gc}{\pur}$ and $W\oplus W_M = W \oplus (W_\var{heap} \oplus W_\var{free\_stack} \oplus W_\var{stack})$ is defined, it follows that
  also $\pwheap[W_\var{heap}](r) \nsubsim \stdreg{R(r),\gc}{\pur}$. This
  means that also $\npair{R_\ms(r)} \in
  H^{\mathrm{std}}_{R(r)}\; \xi^{-1}(R_W(r))$.\dominique{have we defined
    $\nsubsim$ somewhere?} \lau{Yes, it is in subsection~\ref{subsec:worlds}}
...
\begin{proof}
  ...
  Because $\pwfree(r) \nsubsim \stdreg{R(r),\gc}{\pur}$ and $W\oplus W_M = W \oplus (W_\var{free\_stack} \oplus W_\var{free\_stack} \oplus W_\var{stack})$ is defined, it follows that also $\pwfree[W_\var{free\_stack}](r) \nsubsim \stdreg{R(r),\gc}{\pur}$.
  This means that also $\npair{R_\ms(r)} \in H^{\mathrm{std}}_{R(r)}\; \xi^{-1}(R_W(r))$.\dominique{have we defined $\nsubsim$ somewhere?}
  \lau{Yes, it is in subsection~\ref{subsec:worlds}}
...
\begin{array}[t]{l}
  ...
  \updReg{\rretc}{\sealed{\sigma',((\perm,\lin),\baddr,\eaddr,\aaddr+\var{off\_return})}} \\
  \updReg{\rtmp{1}}{0}\\
...
\end{array}
\]
where $\Phi_T(r_i) = \sealed{\sigma,c_i'} \text{ for $i\in\{1,2\}$}$ and $\var{off\_return} = 15$ which is the offset to the return code.
...
By Lemma~\ref{lem:nsub-and-nsup-std} and the safety assumption on the register-file, there exists $S \supseteq [\baddr_\stk, \eaddr_\stk]$ such that for some $R : S \fun \powerset{\nats}$ we have $\biguplus_{r \in S} R(r) \supseteq [\baddr_\stk,\eaddr_\stk]$ and for all $r$ in $S$ $\pwfree[W_{R,1}](r) \nsim \stdreg{R(r),\gc}{\pur}$ and $|R(r)| = 1$ and $\pwfree[W_{R,1}](r) \text{ is address-stratified}$.
Now define
\[
  W_{R,1}' = W_{R,1}[\free.R^{-1}([\aaddr_\stk,\eaddr_\stk]) \mapsto \revoked][\priv.r_\var{priv\_stk} \mapsto \stareg[(\ms_{\stk\_\priv,S},\Phi_T''.\mem|_{[\aaddr_\stk,\eaddr_\stk]}),\gc]{\spa,\square}]
\] 
...
\item[Case $\rretd$:] We have to show
  \[
    \npair[n'-1]{(\sealed{\sigma',\retptrc(\aaddr_\stk,\eaddr_\stk)},\sealed{\sigma',((\rw,\linear),\aaddr_\stk,\eaddr_\stk,\aaddr_\stk)})} \in \lrv(W_{R,\rretd}')
  \]
  where $W_{R,\rretd}'$ is the part of $W_R'$ with ownership over $r_\var{priv\_stk}$.
  From assumption $W \vdash_{(\ta,\stkb)} (\mscode,\_,\_,\_,\sigrets,\sigcloss)$\footnote{This must be the other possible assumption contains $[\baddr,\eaddr] \# \ta$ which leads to a contradition with $[\baddr,\eaddr] \subseteq \ta$}, we know there is a code-region for the
  
\item[Case $\rretc$:] \lau{TODO}
\end{description}

\begin{lemma}
  If $W \vdash_{(\ta,\stkb)} (\mscode,\_,\_,\_,\sigrets,\sigcloss,\_)$ and
  $[\baddr,\eaddr] \subseteq \dom(\mscode)$ and
  $\exists \aaddr \ldotp \mscode(\aaddr) = \sigma$ and
  $\sigma \in \sigrets$ and
  $\memSat{\dots}{W}$, then
  \[
    \begin{array}{l}
      \exists \mscode' r \in \dom(\pwheap) \ldotp \\
      \quad \pwheap(r) = \codereg{\sigrets',\sigcloss',\mscode',(\ta,\stkb)} \text{ and } \\
      \quad [\baddr,\eaddr] \subseteq \dom(\mscode') \text{ and } \\
      \quad \sigma \in \sigrets' \subseteq \sigrets \text{ and } \\
      \quad \sigclos \subseteq \sigclos'
    \end{array}
  \]
\end{lemma}
