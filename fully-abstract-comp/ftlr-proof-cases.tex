...
\begin{proof}
  ...
  It is also easy to show that if $\Phi_S \sterm{i + \min(j_S,j_T)}$, then $\Phi_S' \sterm{i}$ and if $\Phi_T \term[i+\min(j_S,j_T)]$, then $\Phi_T' \term[i]$.
...
\begin{proof}
  ...
  \item One of the following sets of requirements holds:
    ...
    \item $\trust = \untrusted$ and $\npair{[\baddr,\eaddr]} \in \readCond{\lin,W_\pcreg}$
    \end{itemize}:
    Follows from the corresponding assumption of this lemma and the fact that $\Phi_S \nstep{1} \Phi_S'$ with Lemma~\ref{lem:ec-reasonable-antired}.
    \dominique{not actually true: reasonability definitions need to be made downward closed using quantification over smaller $n'$?}
  ...
  \item[Case $\rretc$:]
    ...
    In this case, $\xjumpResult{}{}{}$ will produce failed configurations which are trivially in the observation relation.
    ...
    \item $\Phi_S(\rstk) = \stkptr{\perm_\var{ret\_\stk},\baddr_\var{ret\_\stk},\eaddr_\var{ret\_\stk},\_}$ and $\baddr_\var{ret\_\stk} = \stkb$ and $\eaddr_\var{ret\_\stk} +1 = \baddr_\stk' $ and $\perm_\var{ret\_\stk} = \rw$ and $\baddr_\var{ret\_\stk} \leq \eaddr_\var{ret\_\stk}$.
      ...
      \item
        ...
        \]
        Here we need to use our assumption that Theorem~\ref{thm:ftlr} holds for all $n' < n$.
        TODO: continue
        \lau{TODO: the reasonability requirements of the FTLR seems to be an issue here.}
        \dominique{should now be fine?}
      \end{itemize}
    \end{itemize}
