...
\begin{proof}
  ...
  Because $\pwheap(r).H \nsubeq \stdreg{R(r),\gc}{\pur}.H$ and $W\oplus W_M = W \oplus (W_\var{heap} \oplus W_\var{free\_stack} \oplus W_\var{stack})$ is defined, it follows that
  also $\pwheap[W_\var{heap}](r).H \nsubeq \stdreg{R(r),\gc}{\pur}.H$. This
  means that also $\npair{R_\ms(r)} \in
  ...
  We have that $\npair{(R_\ms(r).1\update{a}{0},R_\ms(r).2\update{a}{0})} \in \pwheap[W_\var{heap}](r).H \; \xi^{-1}(W_r')$ because $\pwheap[W_\var{heap}](r)$ is address-stratified and $\pwheap[W_\var{heap}](r).H \nsupeq \stdreg{R'(r),\gc}{\pur}.H$.
...
\item[Case $\rretd$:] We have to show 
  ...
  From the assumption $\npair{[\baddr,\eaddr]} \in \xReadCond{W}$ we get a
  set of regions $R \subseteq \addressable{\normal, \pwheap}$ 
and
  $\memSat[n']{\ms_S,\stk,\ms_\stk,\ms_T}{W}$, 
...
\begin{lemma}
  ...
  If $\npair{[\baddr,\eaddr]} \in \xReadCond{W}$ and
  $\memSat{\ms_S \uplus \mscode, \stk,\ms_\stk,\ms_T}{W}$
  $[\baddr,\eaddr] \subseteq \dom(\mscode)$ and
  \[
    \begin{array}{l}
      \exists r \in \dom(\pwheap) ,\mscode' \ldotp \\
