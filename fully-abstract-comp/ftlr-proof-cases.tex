...
\begin{lemma}
  \label{lem:stareg-ms-subeq-stdreg}
  If
  \begin{itemize}
  \item $R \nequal \stareg[(\ms,\ms)]{\pur}$
  \item $\npair{(\ms,\ms)} \in H^{\mathrm{std}}_{\dom(\ms)} \; * \; W$
  \item $\dom(\ms) \supseteq [\baddr,\eaddr]$
  \end{itemize}
  Then
  $R \nsubeq \stdreg{\dom(\ms)}{\pur}$
\end{lemma}
\begin{proof}
  \dominique{TODO: define nsubeq and friends first?}
\end{proof}

\begin{lemma}[readXCondition implies readCondition]
  \label{lem:readXCond-readCond}
  If $\npair{(\baddr,\eaddr)}\in\xReadCond{\lin,W}$, then $\npair{(\baddr,\eaddr)}\in\readCond{\lin,W}$.
\end{lemma}
\begin{proof}
  From $\npair{(\baddr,\eaddr)}\in\xReadCond{\lin,W}$, we get by definition an $r \in \addressable{\lin, \pwheap}$ and an $\ms$ with $\dom(\ms) \supseteq [\baddr,\eaddr]$,  $\npair{(\ms,\ms)} \in H^{\mathrm{std}}_{\dom(\ms)} \; * \; W$ and $\pwheap(r) \nequal \stareg[(\ms,\ms)]{\pur}$.

  The result then follows directly using Lemma~\ref{lem:stareg-ms-subeq-stdreg}.
\end{proof}
...
\begin{lemma}[$\lro$ closed under antireduction (generalised previous lemma)]
  ...
  \begin{itemize}
  \item $\Phi_S \steps[j_S] \Phi_S'$
  \item $\Phi_T \steps[j_T] \Phi_T'$
  \item $\npair{(\Phi_S',\Phi_T')} \in \lro$
\end{itemize}
then
\[
...
\begin{enumerate}
\item $\npair[n']{[(c,c)]} \in \lrp(W)$ \dominique{Weird that this is an assumption here.  Shouldn't this be a proof obligation? Or rather: I think this should not be here at all but in the value relation instead? Remove this from the definition of $\lre$?}
...
From Lemmas~\ref{lem:readcond-writecond-work} and~\ref{lem:readXCond-readCond}, using $\memSat[n']{\ms_S,\stk,\ms_\stk,\ms_T}{W_M}$ (from assumption~\ref{item:mem-ass}) and $W \oplus W_R \oplus W_M$ is well-defined (assumption~\ref{item:worlds-def}), we get (for any $a' \in [\baddr,\eaddr]$) that $\npair[n']{(\ms_S(a'),\ms_T(a'))} \in \lrv(W')$ for some $W'$ such that $W_M = W' \oplus W_M'$.
...
Similar to the case for $\tisptr{\trg{r}}{\trg{r}}$.
