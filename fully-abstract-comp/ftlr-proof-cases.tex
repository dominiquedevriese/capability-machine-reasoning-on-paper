...
\begin{lemma}[Downwards closure of relations]
  ...
  If $n' \leq n$, then
  \begin{itemize}
  \item If $\npair{\lin,\baddr,\eaddr} \in \lrp(W)$, then $\npair[n']{\lin,\baddr,\eaddr} \in \lrp(W)$.
...
\end{lemma}
\begin{proof}
  The properties for $\lrp$, $\readCond{}$, $\stackReadCond{}$, $\writeCond{}$, $\stackWriteCond{}$, $\execCond{}$, $\xReadCond{}$ follow easily by definition.
...
\end{lemma}
\begin{proof}
  Direct from the definition of $\sqsubseteq$ and $\lrvg{\trust}$, observing that $\npair{[b,e]} \in \readCond{\lin,W}$ implies $\npair{(\lin,b,e)}\in\lrp(W)$ by definition.
\end{proof}
...
\end{lemma}
\begin{proof}
  \dominique{Doesn't hold: lrp doesn't make sense for stack regions!}
  TODO
\end{proof}
...
\begin{lemma}
  ...
  \item One of the following sets of requirements holds:
    ...
    \item $\trust = \untrusted$ and $\npair{[\baddr,\eaddr]} \in \readCond{\lin,W_\pcreg}$
    \end{itemize}
  \item $\Phi_S(\pcreg) = \Phi_T(\pcreg) = ((\rx,\lin),\baddr,\eaddr,\_)$
...
\begin{proof}
  ...
  \item \label{case:ftlr:move-cca-etc} All of the following hold: (includes cap-manipulation cases: move, cca,
    ...
    \item One of the following holds:
      ...
      \item \label{case:ftlr:move-cca-etc:move}(move) $w_1 = \Phi_S(r_2)$, $w_1' = \Phi_S'(r_2)$, and $\isLinear{w_1} \Rightarrow w_2 = w_2' = 0$, and $\neg \isLinear{w_1} \Rightarrow (w_2 = \Phi_S(r_2) \wedge w_2' = \Phi_T(r_2)$) and $r_1 \neq r_2$ and $k = 2$.
      \item \label{case:ftlr:move-cca-etc:move-eq}(move) $w_1 = \Phi_S(r_1)$, $w_1' = \Phi_S'(r_1)$ and $k = 1$.
  ...
  \item One of the following sets of requirements holds:
    ...
    \item $\trust = \untrusted$ and $\npair[n-1]{[\baddr,\eaddr]} \in \readCond{\lin,W_\pcreg}$
    \end{itemize}: Follows from the corresponding assumption of this lemma and
    the fact that $\Phi_S \nstep[1]{\ta,\stkb} \Phi_S'$ and
    Lemma~\ref{lem:ec-reasonable-antired},
    \ref{lem:ec-reasonable-downwards-closed}, and \ref{lem:downwards-closed}.
  \item $\Phi_S'(\pcreg) = \Phi_T'(\pcreg) = ((\rx,\lin),\baddr,\eaddr,\_)$:
  ...
  \item $\npair[n-1]{(\Phi_S.\reg,\Phi_T.\reg)} \in \lrrg{\trust}(W_R)$:
    Follows from the corresponding assumption of this lemma using Lemma~\ref{lem:downwards-closed} and the fact that integers are always in $\lrvg{\trust}$ by definition.
  ...
  \[
    \npair[n-1]{(\Phi_S',\Phi_T'')} \in \lro
  \]
  By assumption, we have $\npair[n]{(\reg_S(r_i),\reg_T(r_i))} \in \lrrg{\trust}(W_{R,i})$ for some $W_{R,i}$ with $i \in \{1,2\}$.
...
\end{itemize}
  ...
  \item[Case $\rretc$:]
    ...
    cannot be the case as $\sigma' \in \sigrets'$ and we have $\sigrets',\sigcloss' \vdash_{\mathrm{comp-code}} \mscode'$.
\lau{TODO I think this case should follow from $H^\var{code}$ and the comp-code judgement, but it does not.
      Specifically, the comp-code judgement (for now) does not require $\sigrets'$ and $\sigcloss'$ to be disjoint.}
    ...
    Now consider the following cases: \lau{TODO: Perhaps make a lemma that says if source fail and target step to fail, then the two configurations are in $\lro$.}
    \begin{itemize}
    \item $\reg_S^{(3)}(\rstk) \neq \stkptr{\_,\_,\_,\_}$\\
    ...
    \item $\reg_S^{(3)}(\rstk) = \stkptr{\perm_\var{ret\_\stk},\baddr_\var{ret\_\stk},\eaddr_\var{ret\_\stk},\_}$ and $\baddr_\var{ret\_\stk} = \stkb$ and $\eaddr_\var{ret\_\stk} +1 = \baddr_\stk' $ and $\perm_\var{ret\_\stk} = \rw$ and $\baddr_\var{ret\_\stk} \leq \eaddr_\var{ret\_\stk}$.
      ...
      \item
        ...
        \]
        and $W'''_R$ with the ownership of $W''_R$ as well as for the regions $r_{b_\stk'}\dots r_{e_\stk'}$,
        and $W'''_M$ with the ownership of $W''_M$,
        we show the following:
        \begin{enumproof}
        \item $\npair[n''']{(\Phi_S^{(4)}.\reg,\Phi_T^{(5)}.\reg)} \in \lrv(W'''_R)$
        \item $\memSat[n''']{\Phi_S^{(4)}.\mem,\Phi_S^{(4)}.\stk,\Phi_S^{(4)}.\ms_\stk,\Phi_T^{(5)}.\mem}{W'''_M}$ 
        \item $W''' \oplus W'''_R \oplus W'''_M$ defined
        \end{enumproof}
        Assuming the above, we use our assumption that Theorem~\ref{thm:ftlr} holds for all $n' < n$ (note that from assumption ``$\Phi_S$ reasonable up to $n$ steps'' and Lemma~\ref{lem:ec-reasonable-downwards-closed} and $n''' \le n'' < n - 1$ we get ``$\Phi_S$ reasonable up to $n'''+1$ steps'' from which it follows that ``$\Phi_S(\pcreg) + \calllen$ reasonable up to $n'''$ steps''.
        ) To get
        \[
          \npair[n''']{(((\rx,\normal),\baddr,\eaddr,\aaddr+\calllen),((\rx,\normal),\baddr,\eaddr,\aaddr+\calllen))} \in \lre[\trusted](W''')
        \]
        ...
        Show:
        \[
          \npair[n''']{(\Phi_S^{(4)}.\reg,\Phi_T^{(5)}.\reg)} \in \lrrtrusted(W'''_R)
        \]
        Using Lemma~\ref{lem:untrusted-subset-trust}, it suffices to show $\npair[n''']{(\Phi_S^{(4)}.\reg,\Phi_T^{(5)}.\reg)} \in \lrrtrusted(W'''_R)$.
        ...
        \item Case $r \not\in \{\rdata,\rtmp{1},\rtmp{2},\pcreg,\rstk\}$:
          ...
          We already know:
          \[\npair[n''']{(\reg^{(3)}_S(r),\reg^{(3)}_T(r))} \in \lrr(W''_{R,r}),\] so by Lemma~\ref{lem:monotonicity}, we get
          \[
        ...
        \item Case $\rstk$:
          ...
          The $\var{stackReadCondition}$ is showed in the same way, but we also use Lemma~\ref{lem:stdreg-singleton-addr-strat} to argue that the new regions are address-stratified.
        \end{itemize}
        ...
        \item $\npair{(\overline{\sigma},\ms_S',\ms_\var{T,heap})} \in \lrheap(\pwheap)(W_{\var{heap}}''')$
          ...
          Follows by region interpretation monotonicity and the heap satisfaction assumption.
        \end{itemize}
...
% Case: \ref{case:ftlr:scall:mem}, we need to show

% \begin{equation}
%   \memSat[n-1]{\Phi_S''.\mem,\Phi_S''.\stk,\Phi_S''.\ms_\stk,\Phi_T''.\mem}{W_M'}\label{eq:ftlr:call:memsat-case}
% \end{equation}
% We have the following memory satisfaction on the initial memory:
% \begin{equation}
%   \memSat[n]{\ms_S,\stk,\ms_\stk,\ms_T}{W_M}\label{eq:ftlr-call-og-mem-sat}
% \end{equation}
% By definition of memory satisfaction, this gives us:
% \begin{itemize}
% \item $\stk = (\opc_1,\ms_1) :: \dots :: (\opc_m,\ms_m)$
% \item $\ms_1 \# \dots \# \ms_m \# \ms_S \# \ms_\stk$
% \item $W = W_{\var{stack}} \oplus W_{\var{free\_stack}} \oplus W_{\var{heap}}$
% \item $\ms_\var{T,stack}, \ms_\var{T,free\_stack}, \ms_\var{T,heap}, \ms_{T,f}, \ms_{S,f}, \ms_S',\overline{\sigma}$ such that
%   \begin{itemize}
%   \item $\ms_S = \ms_{f,S} \uplus \ms_S'$
%   \item $\ms_T = \ms_\var{T,stack} \uplus \ms_\var{T,free\_stack} \uplus \ms_\var{T,heap} \uplus \ms_{T,f}$
%   \item $\memSatStack{\stk,\ms_\var{T,stack}}{W_{\var{stack}}}$
%   \item $\memSatFStack{\ms_\stk,\ms_\var{T,free\_stack}}{W_{\var{free\_stack}}}$
%   \item $\npair{(\overline{\sigma},\ms_S',\ms_\var{T,heap})} \in \lrheap(\pwheap)(W_{\var{heap}})$
%   \end{itemize}
% \end{itemize}
% We need to consider the memories of $\Phi_S''$ and $\Phi_T''$.
% The memories of the initial configurations have only changed little:
% \[
%   \Phi_T''.\mem = \Phi_T''.\mem\update{\aaddr_\stk}{42}
% \]
% and 
% \[
% \Phi_S''.\ms_\stk = \Phi_S.\ms_\stk - \Phi_S.\ms_\stk|_{[\aaddr_\stk,\eaddr_\stk]}
% \]
% and
% \[
%   \Phi_S''.\stk = ((\aaddr+\calllen),\ms_{\stk\_\priv,S}) :: \Phi_S.\stk
% \]
% with $\ms_{\stk\_\priv,S} = \Phi_S.\ms_\stk |_{[\aaddr_\stk,\eaddr_\stk]}\update{\aaddr_\stk}{42}$.
% We need to show the memory satisfaction in \ref{eq:ftlr:call:memsat-case}. Say
% $(\opc_0,\ms_0) = ((\aaddr+\calllen),\ms_{\stk\_\priv,S})$. In this case we have
% to argue 
% \[
%   \ms_{\stk\_\priv,S} \# \ms_1 \# \dots \# \ms_m \# \ms_S \# \Phi_S.\ms_\stk - \Phi_S.\ms_\stk|_{[\aaddr_\stk,\eaddr_\stk]}
% \]
% which follows from Assumption~\ref{eq:ftlr:call:memsat-case} and the fact that the memory we remove from $\Phi_S.\ms_\stk$ is exactly what we add to the private stack.
% \lau{This is a bit more involved than I make it to be here, but the fact that we also have a safety assumption on the register-file with the stack pointer means that all of the memory removed from the free stack is actually there.}
...
%   This concludes case \ref{case:ftlr:scall:reg}
  ...
  \item $\baddr_\stk = \stkb$
    ...
    Further, from $\callCond{\Phi_S,r_1,r_2,a}$ and $b \leq a$ and $a + \calllen -1 \leq e$ and $[b,e] \subseteq \ta$ we can conclude that $\Phi_S$ points to $\src{\scall{\offpc,\offsigma}{r_1}{r_2}}$ in $\ta$.
  ...
  \item $\npair{(\overline{\sigma},\ms_S',\ms_\var{T,heap})} \in \lrheap(\pwheap)(W_{M,\var{heap}})$
  \end{itemize}
  ...
  \item $\memSatStack{((\aaddr+\calllen),\ms_{\stk\_\priv,S}) :: \Phi_S,\ms_\var{T,stack} \uplus \ms_\var{T,free\_stack}|_{[\aaddr_\stk,\eaddr_\stk]}\update{\aaddr_\stk}{42}}{W_{M,\var{stack}}'}$:
    ...
    By assumption we know $\npair{(\Phi_S.\reg,\Phi_T.\reg)} \in \lrrg{\trust}(W_R)$ which entails $\npair{(\Phi_S.\reg(\rstk),\Phi_T.\reg(\rstk))} \in \lrrg{\trust}(W_{R,\rstk})$.
    ...
    which entails
    \[
      \npair[n-1]{R_\ms(r)} \in \stdreg{R_{\rstk}(r),\gc}{\spao} \; \xi^{-1}(R_W(r))
    \]
    which entails
    \[
  ...
  \item Case \ref{case:ftlr:move-cca-etc:cseal} and $\isLinear{\Phi_S(r_1)}$ and $r_1 = \pcreg$.
  \end{itemize}
  ...
  \item Case \ref{case:ftlr:move-cca-etc:move}:
    ...
    \lau{remember to change this if we do not manage to prove that the pc is safe.}
...
TODO
  ...
  \[
    \npair[n-1]{\Phi_S',\Phi_T'} \in \lro
  \]
  which follows from the induction hypothesis in the case where $\Phi_S'.\reg(\pcreg) = ((\rx,\lin),\baddr,\eaddr,\_)$.
...
\begin{lemma}
  ...
  If $\pwheap = \pwheap[W']$ and $W' \future W$ and $\npair{(\overline{\sigma},\ms_S,\ms_T)} \in \lrheap(W)$, then $\npair{(\overline{\sigma},\ms_S,\ms_T)} \in\lrheap(W')$.
\end{lemma}
\begin{proof}
  TODO
\end{proof}
