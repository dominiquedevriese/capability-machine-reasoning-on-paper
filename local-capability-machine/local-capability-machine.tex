...
\author{Lau Skorstengaard\\Aarhus University\\\texttt{lau@cs.au.dk}
        \and 
        Dominique Devriese\\Vrije~Universiteit~Brussel\\\texttt{dominique.devriese@vub.be}
        \and 
...
\begin{lstlisting}
        ...
        plus r_t1 r_t1 1
        move r_t2 pc
        lea r_t2 ... // ... is the offset to continue
        move r_t3 pc
        lea r_t3 ... // ... is the offset to iter
iter:
        jnz r_t2 r_t1
        lea pc ... // ... if the offset to end
continue:
        store r_t 0
        lea r_t 1
        minus r_t1 r_t1 1
...
\begin{proof}
  ...
  \item Define $\ms_{\var{frame}} = \left(\biguplus_{r'\in\activeReg{W'},r'\neq
    ...
    \ms_{\var{frame}}$ and $\npair[n'']{P(r)} \in W'(r).H~(W'(r).s)~(\xi^{-1}(W'))$.
    Easy from the previous point.
  \item $\npair[n'']{P(r)} \in W'(r).H~(W'(r).s)~(\xi^{-1}([r\mapsto W'(r)]))$, i.e.
  ...
  \item Case: $\reg(r_1) \in \ints$ and $\reg(r_1) \geq 0$
    ...
    \item Define $\ms' = \left(\biguplus_{r'\in\activeReg{W'},r'\neq
      ...
      public monotonicity and downwards closedness of all regions, and finally
      the definition of $W''$.
    \item $\npair[n''-j+1]{w_{\var{ret}}} \in \stdvr(W'')$:\\
      By Lemma~\ref{lem:stdvr-mono-wk}, the fact that $W''\futurewk W'$,
      Lemma~\ref{lem:stdvr-dc}, and the fact that
    ...
    \item $i > j$ and $\Phi'\step[i-j]
      ...
      (\halted,\mem')$ with
      $(\reg\update{\pcreg}{((\exec,\glob),\start,\addrend,\addr)},\ms\uplus\ms_f)
      \step[j] \Phi'$ using Lemma~\ref{lem:determinacy}.
...
\begin{proof}
  ...
  \item Case $\var{conf'} = \failed$: contradiction, since it is not possible
    that $\failed \step[i'] (\halted,\mem')$. 
  \item Case $\var{conf'} = (\halted,\mem)$:
    \begin{enumproof}
    \item Then $i' = 0$ and $\mem' =
      \mem$\\
      Follows from $(\halted, \mem) \step[i'] (\halted,\mem)$
    \item For $W' = W$, $\ms_r = \emptyset$ and $\ms' = \ms$, we have that
      $\mem = \hs' \uplus \hs_r \uplus \ms_f$ and
  ...
  \item Case $\var{conf'} = \updateReg[\Phi'']{\pcreg}{\var{newPc}}$, and additionally, one of the following
    ...
    \item $\exists W' \futurestr W$, $\hs_r$, $\hs' \ldotp$
      ...
      evaluation $\var{conf'}\step[i'] (\halted,\mem')$ from
...
\subsubsection{Stack helper lemmas}
\begin{lemma}
